/* ==========================================
   Shopping Support
   Version 0.1.0

   app.js

   アプリ初期設定
   ========================================== */

"use strict";

/* ==========================================
   編集中商品ID
   ========================================== */

let editingProductId = null;

/*
 商品登録後の戻り先

 normal
 → 通常の商品登録

 outingAdd
 → お出かけ＋追加へ戻る
*/

let productRegistrationReturnMode =
    "normal";

/*
 お出かけ作成中の
 持ち物一時保存
*/

let outingDraftItems = [];


/*
 保存済みのお出かけ
*/

let outings = [];

/*
 お出かけチェック
 現在表示している状態

 pending = 準備前
 ready   = 準備済
 hold    = 保留
*/

let outingViewMode =
    "pending";


/*
 現在開いている
 お出かけID
*/

let currentOutingId =
    null;


/*
 商品を後から関連付ける
 お出かけ持ち物ID
*/

let outingLinkTargetItemId =
    null;


/*
 お出かけ
 商品選択専用モード

 false
 → 通常の＋追加

 true
 → 既存の購入する物から
    商品を選んでいる
*/

let outingProductSelectMode =
    false;


/*
 お出かけ一覧
 現在表示している種類

 upcoming = 予定
 routine  = いつも使う
 past     = 過去
*/

let outingListViewMode =
    "upcoming";

let productFilterCategoryId = "";

/* ==========================================
   購入予定
   ========================================== */

const SHOPPING_STORAGE_KEY =
    "cocartlyShoppingItems";

let shoppingItems =
    loadShoppingItems();

let pendingShoppingItemId =
    null;

/*
 商品詳細で現在表示している
 購入予定商品ID
*/

let shoppingDetailItemId =
    null;

/*
 購入予定の商品絞り込み

 all
 favorite
 categoryId
*/

let shoppingProductFilter =
    "all";

/* ==========================================
   店頭確認履歴
   ========================================== */

const STORE_CHECK_STORAGE_KEY =
    "cocartlyStoreChecks";


let storeChecks =
    loadStoreChecks();

/* ==========================================
   編集中店舗ID
   ========================================== */

let editingStoreId = null;

/* ==========================================
   編集中商品ジャンルID
   ========================================== */

let editingCategoryId = null;

/* ==========================================
   マスター管理画面を開いた元の画面
   ========================================== */

let masterReturnScreen = "settings";

/* ==========================================
   今回の購入 一時入力保存
   ========================================== */

let priceRecordTempStoreId = "";
let priceRecordTempPrice = "";
let priceRecordTempQuantity = "1";

/* ==========================================
   価格計算情報
   ========================================== */

let productPriceCalculation = null;

let purchasePriceCalculation = null;

/* ==========================================
   商品登録画面の選択値保持
   ========================================== */

let productSelectedCategoryId = "";

let productSelectedStoreId = "";

/* ==========================================
   アプリ起動
   ========================================== */

/*
  ページの読み込みが完了したら実行する。

  Version 0.1.0では、
  ボタンが正しく取得できるかを確認するだけ。

  将来ここに
  ・データ読込
  ・設定読込
  ・画面初期化
  などを追加していく。
*/

document.addEventListener("DOMContentLoaded", initializeApp);

/* ==========================================
   PWA
   Service Worker 登録
   ========================================== */

if ("serviceWorker" in navigator) {

    window.addEventListener(
        "load",
        () => {

            navigator.serviceWorker
                .register("./service-worker.js")
                .then(
                    (registration) => {

                        console.log(
                            "Cocartly Service Worker 登録成功",
                            registration.scope
                        );

                    }
                )
                .catch(
                    (error) => {

                        console.error(
                            "Cocartly Service Worker 登録失敗",
                            error
                        );

                    }
                );

        }
    );

}

/* ==========================================
   初期化
   ========================================== */

function initializeApp() {

    console.log("Shopping Support 起動");

    loadProducts();

    loadCategories();

    createDefaultCategories();

    initializeStores();

    loadPriceHistory();

    /*
     買い物セッションを復元
    */

    loadShoppingSession();

    loadOutings();

    setupButtons();
    displayCategorySelect();
    displayProducts();

    /*
     現在の表示言語を
     ホーム画面へ反映
    */

    updateHomeScreenLanguage();

    /*
     初回設定画面の表示判定
    */

    displayInitialSetupScreen();

}

/* ==========================================
   初回設定画面 表示判定
   ========================================== */

function displayInitialSetupScreen() {

    /*
     初回設定済みなら
     通常のホーム画面を使用
    */

    if (
        isCocartlySetupCompleted()
    ) {

        return;

    }


    /*
     初回設定前なら
     一度すべての画面を閉じる
    */

    hideAllScreens();


    /*
     初回設定画面を表示
    */

    const setupScreen =
        document.getElementById(
            "setupScreen"
        );


    if (!setupScreen) {

        return;

    }


    setupScreen.hidden =
        false;


    /*
     保存されている表示言語を
     初回設定画面のプルダウンへ反映
    */

    const setupLanguageSelect =
        document.getElementById(
            "cmbSetupLanguage"
        );

    if (setupLanguageSelect) {

        setupLanguageSelect.value =
            getCocartlyLanguage();

    }

    /*
     保存されている
     普段買い物する国・地域を
     初回設定画面のプルダウンへ反映
    */

    const setupCountrySelect =
        document.getElementById(
            "cmbSetupCountry"
        );

    const savedBaseCountry =
        loadCocartlyBaseCountry();

    if (setupCountrySelect) {

        setupCountrySelect.value =
            savedBaseCountry || "JP";

    }

    /*
     初回設定画面の表示言語を反映
    */

    updateSetupScreenLanguage();

}

/* ==========================================
   初回設定画面の表示言語を反映
   ========================================== */

function updateSetupScreenLanguage() {

    const welcomeTitle =
        document.getElementById(
            "setupWelcomeTitle"
        );

    const welcomeGuide =
        document.getElementById(
            "setupWelcomeGuide"
        );

    const languageSelect =
        document.getElementById(
            "cmbSetupLanguage"
        );

    const countrySelect =
        document.getElementById(
            "cmbSetupCountry"
        );

    const languageLabel =
        document.querySelector(
            'label[for="cmbSetupLanguage"]'
        );

    const countryLabel =
        document.querySelector(
            'label[for="cmbSetupCountry"]'
        );

    const completeButton =
        document.getElementById(
            "btnCompleteSetup"
        );


    if (welcomeTitle) {

        welcomeTitle.textContent =
            t("setup.welcome");

    }


    if (welcomeGuide) {

        welcomeGuide.textContent =
            t("setup.countryGuide");

    }

    if (languageLabel) {

        languageLabel.textContent =
            t("setup.languageLabel");

    }


    if (countryLabel) {

        countryLabel.textContent =
            t("setup.countryLabel");

    }

    if (completeButton) {

        completeButton.textContent =
            t("common.start");

    }


    /*
     表示言語の選択肢
    */

    if (languageSelect) {

        const languageOptions = [
            "ja",
            "en",
            "ko",
            "zh-CN",
            "zh-TW",
            "fr"
        ];

        for (
            const language
            of languageOptions
        ) {

            const option =
                languageSelect.querySelector(
                    `option[value="${language}"]`
                );

            if (option) {

                option.textContent =
                    t(
                        "language." +
                        language
                    );

            }

        }

    }


    /*
     国・地域の選択肢
    */

    if (countrySelect) {

        const countryOptions = [
            "JP",
            "US",
            "CA",
            "AU",
            "KR",
            "CN",
            "TW",
            "OTHER"
        ];

        for (
            const country
            of countryOptions
        ) {

            const option =
                countrySelect.querySelector(
                    `option[value="${country}"]`
                );

            if (!option) {

                continue;

            }


            const countryIcons = {

                JP: "🇯🇵 ",
                US: "🇺🇸 ",
                CA: "🇨🇦 ",
                AU: "🇦🇺 ",
                KR: "🇰🇷 ",
                CN: "🇨🇳 ",
                TW: "🇹🇼 ",
                OTHER: "🌐 "

            };


            option.textContent =
                countryIcons[country] +
                t(
                    "country." +
                    country
                );

        }

    }

}

/* ==========================================
   初回設定 完了
   ========================================== */

function completeInitialSetup() {

    /*
     初回設定画面で選択された
     表示言語と国・地域を取得
    */

    const languageSelect =
        document.getElementById(
            "cmbSetupLanguage"
        );

    const countrySelect =
        document.getElementById(
            "cmbSetupCountry"
        );


    /*
     表示言語を保存
    */

    if (languageSelect) {

        setCocartlyLanguage(
            languageSelect.value
        );

    }


    /*
     普段買い物する国・地域を保存
    */

    if (countrySelect) {

        setCocartlyBaseCountry(
            countrySelect.value
        );

    }


    /*
     初回設定完了として保存
    */

    completeCocartlySetup();


    /*
     一度すべての画面を閉じる
    */

    hideAllScreens();


    /*
     ホーム画面を表示
    */

    const homeScreen =
        document.getElementById(
            "homeScreen"
        );


    if (!homeScreen) {

        return;

    }


    /*
     選択した表示言語を
     ホーム画面へ反映
    */

    updateHomeScreenLanguage();

    homeScreen.hidden =
        false;


    /*
     画面上部へ戻す
    */

    window.scrollTo(
        0,
        0
    );

}

/* ==========================================
   購入予定 状態
   ========================================== */

/*
 pending   = 購入前
 purchased = 購入済み（一時）
 hold      = 保留
*/

let shoppingViewMode =
    "pending";


/*
 現在の買い物セッション
 買い物終了までは一時状態として保持
*/

let shoppingSession = {

    id: null,

    startedAt: null,

    completedAt: null,

    active: false

};

/* ==========================================
   買い物セッション 保存
   ========================================== */

function saveShoppingSession() {

    localStorage.setItem(

        "shoppingSupportShoppingSession",

        JSON.stringify(
            shoppingSession
        )

    );

}


/* ==========================================
   買い物セッション 読み込み
   ========================================== */

function loadShoppingSession() {

    try {

        const saved =
            localStorage.getItem(
                "shoppingSupportShoppingSession"
            );

        if (!saved) {

            return;

        }

        const parsed =
            JSON.parse(saved);

        if (!parsed) {

            return;

        }

        shoppingSession = {

            id:
                parsed.id || null,

            startedAt:
                parsed.startedAt || null,

            completedAt:
                parsed.completedAt || null,

            active:
                parsed.active === true

        };

    } catch (error) {

        console.error(
            "買い物セッションを読み込めませんでした。",
            error
        );

    }

}

/* ==========================================
   購入予定 読み込み
   ========================================== */

function loadShoppingItems() {

    try {

        const saved =
            localStorage.getItem(
                SHOPPING_STORAGE_KEY
            );

        if (!saved) {

            return [];

        }

        const data =
            JSON.parse(saved);

        if (!Array.isArray(data)) {

            return [];

        }


        /*
         旧データにはstatusが無いため
         購入前として補完
        */

        return data.map(

            item => {

                return {

                    ...item,

                    status:
                        item.status ||
                        "pending",

                    updatedAt:
                        item.updatedAt ||
                        item.createdAt ||
                        new Date().toISOString()

                };

            }

        );

    } catch (error) {

        console.error(
            "購入予定読み込みエラー：",
            error
        );

        return [];

    }

}

/* ==========================================
   店頭確認履歴 読み込み
   ========================================== */

function loadStoreChecks() {

    try {

        const saved =
            localStorage.getItem(
                STORE_CHECK_STORAGE_KEY
            );

        if (!saved) {

            return [];

        }

        const data =
            JSON.parse(saved);

        return Array.isArray(data)
            ? data
            : [];

    } catch (error) {

        console.error(
            "店頭確認履歴の読み込みエラー：",
            error
        );

        return [];

    }

}

/* ==========================================
   店頭確認履歴 保存
   ========================================== */

function saveStoreChecks() {

    localStorage.setItem(

        STORE_CHECK_STORAGE_KEY,

        JSON.stringify(
            storeChecks
        )

    );

}

/* ==========================================
   店頭確認ID作成
   ========================================== */

function createStoreCheckId() {

    return (
        "store-check-" +
        Date.now() +
        "-" +
        Math.random()
            .toString(36)
            .slice(2, 8)
    );

}

/* ==========================================
   購入予定 保存
   ========================================== */

function saveShoppingItems() {

    localStorage.setItem(

        SHOPPING_STORAGE_KEY,

        JSON.stringify(
            shoppingItems
        )

    );

}

/* ==========================================
   購入予定 商品ジャンル切り替え表示
   ========================================== */

function displayShoppingCategoryTabs() {

    const tabs =
        document.getElementById(
            "shoppingCategoryTabs"
        );

    if (!tabs) {

        return;

    }


    /*
     ⭐よく使う・すべて
     は毎回ここで作り直す
    */

    tabs.innerHTML = `

    <button
        type="button"
        class="shopping-category-button"
        data-shopping-category="favorite">

        ${t(
        "shopping.favorite"
    )}

    </button>


    <button
        type="button"
        class="shopping-category-button"
        data-shopping-category="all">

        ${t(
        "shopping.all"
    )}

    </button>

`;


    /*
     登録済みジャンルを追加
    */

    const activeCategories =
        categories.filter(

            category =>
                category.active !== false

        );


    for (
        const category of activeCategories
    ) {

        const button =
            document.createElement(
                "button"
            );


        button.type =
            "button";


        button.className =
            "shopping-category-button";


        button.dataset.shoppingCategory =
            category.id;


        const defaultCategoryTranslationKeys = {

            "食品": "category.default.food",
            "飲料": "category.default.drink",
            "お菓子": "category.default.snack",
            "お酒": "category.default.alcohol",
            "日用品": "category.default.daily",
            "洗剤・衛生用品": "category.default.hygiene",
            "医薬品": "category.default.medicine",
            "化粧品・美容": "category.default.beauty",
            "衣料品": "category.default.clothing",
            "ベビー用品": "category.default.baby",
            "ペット用品": "category.default.pet",
            "文具・雑貨": "category.default.stationery",
            "家電・電池": "category.default.electronics",
            "その他": "category.default.other"

        };

        const translationKey =
            defaultCategoryTranslationKeys[
            category.name
            ];

        button.textContent =
            translationKey
                ? t(translationKey)
                : category.name;


        tabs.appendChild(
            button
        );

    }


    /*
     クリック処理
    */

    const buttons =
        tabs.querySelectorAll(
            ".shopping-category-button"
        );


    buttons.forEach(

        button => {

            button.addEventListener(

                "click",

                () => {

                    shoppingProductFilter =
                        button.dataset.shoppingCategory;


                    updateShoppingCategoryTabs();

                    displayShoppingProductOptions();

                }

            );

        }

    );


    updateShoppingCategoryTabs();

}

/* ==========================================
   購入予定 商品ジャンル 選択表示
   ========================================== */

function updateShoppingCategoryTabs() {

    const buttons =
        document.querySelectorAll(
            ".shopping-category-button"
        );


    buttons.forEach(

        button => {

            const category =
                button.dataset.shoppingCategory;


            button.classList.toggle(

                "active",

                category ===
                shoppingProductFilter

            );

        }

    );

}

/* ==========================================
   購入予定 商品候補表示
   ========================================== */

function displayShoppingProductOptions() {

    const select =
        document.getElementById(
            "cmbShoppingProduct"
        );

    if (!select) {

        return;

    }


    select.innerHTML = `

         <option value="">
             ${t(
        "shopping.selectProduct"
    )}
         </option>

    `;


    /*
     表示対象商品
    */

    let targetProducts =
        products.filter(

            product =>
                product.active !== false

        );


    /*
     ジャンルで絞り込み

     all
     → 全商品

     favorite
     → まだ③〜⑤で実装するため
       今は0件にする

     それ以外
     → categoryIdで絞る
    */

    if (
        shoppingProductFilter ===
        "favorite"
    ) {

        targetProducts =
            targetProducts.filter(

                product =>
                    product.favorite ===
                    true

            );

    } else if (
        shoppingProductFilter !==
        "all"
    ) {

        targetProducts =
            targetProducts.filter(

                product =>
                    product.categoryId ===
                    shoppingProductFilter

            );

    }


    /*
     商品名順
    */

    targetProducts.sort(

        (a, b) =>

            a.name.localeCompare(
                b.name,
                "ja"
            )

    );


    /*
     商品候補追加
    */

    for (
        const product of targetProducts
    ) {

        const option =
            document.createElement(
                "option"
            );


        option.value =
            product.id;


        /*
         商品に登録されている
         購入先から国を取得
        */

        const store =
            product.storeId
                ? findStore(
                    product.storeId
                )
                : null;

        const country =
            store
                ? store.country || "JP"
                : "JP";

        const countryFlag =
            getCountryFlag(
                country
            );


        /*
         現在は日本語表示なので
         日本の商品は国旗を表示しない
        
         海外商品の場合だけ
         国旗を表示する
        */

        const countryPrefix =
            country === "JP"
                ? ""
                : `${countryFlag} `;


        option.textContent =
            `${countryPrefix}${product.name}`;


        select.appendChild(
            option
        );

    }


    /*
     該当商品がない場合
    */

    if (
        targetProducts.length === 0
    ) {

        const option =
            document.createElement(
                "option"
            );

        option.value =
            "";

        option.disabled =
            true;


        if (
            shoppingProductFilter ===
            "favorite"
        ) {

            option.textContent =
                t(
                    "shopping.empty.favorite"
                );

        } else {

            option.textContent =
                t(
                    "shopping.empty.category"
                );

        }


        select.appendChild(
            option
        );

    }

}


/* ==========================================
   購入予定を追加
   ========================================== */

function addShoppingItem() {

    const select =
        document.getElementById(
            "cmbShoppingProduct"
        );

    const newNameInput =
        document.getElementById(
            "txtShoppingNewProductName"
        );

    const newJanInput =
        document.getElementById(
            "txtShoppingJanCode"
        );

    const quantityInput =
        document.getElementById(
            "txtShoppingQuantity"
        );

    const message =
        document.getElementById(
            "shoppingMessage"
        );


    const productId =
        select
            ? select.value
            : "";


    const newProductName =
        newNameInput
            ? newNameInput.value.trim()
            : "";


    const newJanCode =
        newJanInput
            ? newJanInput.value.trim()
            : "";


    const quantity =
        Number(
            quantityInput.value
        );


    /*
     登録済み商品も
     未登録商品名も入力されていない
    */

    if (
        productId === "" &&
        newProductName === ""
    ) {

        message.textContent =
            t(
                "shopping.validation.selectOrName"
            );

        return;

    }


    /*
     数量チェック
    */

    if (
        !Number.isFinite(quantity) ||
        quantity < 1
    ) {

        message.textContent =
            t(
                "shopping.validation.quantity"
            );

        quantityInput.focus();

        return;

    }


    /*
     未登録商品として追加しようとしていて、
     商品コードが入力されている場合だけ
     登録済み商品の商品コードと重複していないか確認
    */

    if (
        productId === "" &&
        newJanCode !== ""
    ) {

        const existingJanProduct =
            products.find(

                product =>

                    product.active !== false &&

                    (
                        product.janCode ===
                        newJanCode ||

                        product.jan ===
                        newJanCode
                    )

            );


        if (existingJanProduct) {

            message.textContent =
                t(
                    "shopping.validation.duplicateCode"
                );


            alert(
                t(
                    "shopping.validation.duplicateCodeDetail"
                ).replace(
                    "{name}",
                    existingJanProduct.name
                )
            );


            /*
             登録済み商品を
             自動選択する
            */

            if (select) {

                select.value =
                    existingJanProduct.id;

            }


            return;

        }

    }


    /*
     登録済み商品を選択した場合
    */

    if (
        productId !== ""
    ) {

        const existing =
            shoppingItems.find(

                item =>
                    item.productId ===
                    productId

            );


        if (existing) {

            existing.quantity =
                quantity;

            /*
             もう一度追加した場合は
             購入前へ戻す
            */

            existing.status =
                "pending";

            existing.holdStartedAt =
                null;

            existing.updatedAt =
                new Date().toISOString();

        } else {

            const now =
                new Date().toISOString();


            shoppingItems.push({

                id:
                    "shopping-" +
                    Date.now() +
                    "-" +
                    Math.random()
                        .toString(36)
                        .slice(2, 8),

                productId:
                    productId,

                quantity:
                    quantity,

                status:
                    "pending",

                holdStartedAt:
                    null,

                createdAt:
                    now,

                updatedAt:
                    now

            });

        }

    } else {

        /*
         未登録商品を追加
        */

        const now =
            new Date().toISOString();


        /*
         同じ未登録商品が
         すでに購入予定にあるか確認
        */

        const existing =
            shoppingItems.find(

                item =>

                    !item.productId &&

                    (
                        item.productName ||
                        ""
                    ).trim() ===
                    newProductName

            );


        if (existing) {

            existing.quantity =
                quantity;

            /*
             商品コードを後から入力した場合は更新
            */

            if (
                newJanCode !== ""
            ) {

                existing.janCode =
                    newJanCode;

            }


            existing.status =
                "pending";

            existing.holdStartedAt =
                null;

            existing.updatedAt =
                now;

        } else {

            shoppingItems.push({

                id:
                    "shopping-" +
                    Date.now() +
                    "-" +
                    Math.random()
                        .toString(36)
                        .slice(2, 8),

                /*
                 まだ商品登録済みではないため
                 productIdは持たない
                */

                productId:
                    null,

                productName:
                    newProductName,

                janCode:
                    newJanCode,

                quantity:
                    quantity,

                /*
                 買い物終了時に
                 商品登録が必要
                */

                needsRegistration:
                    true,

                status:
                    "pending",

                holdStartedAt:
                    null,

                createdAt:
                    now,

                updatedAt:
                    now

            });

        }

    }


    /*
     購入予定保存
    */

    saveShoppingItems();


    /*
     入力欄リセット
    */

    if (select) {

        select.value =
            "";

    }


    if (newNameInput) {

        newNameInput.value =
            "";

    }


    if (newJanInput) {

        newJanInput.value =
            "";

    }


    quantityInput.value =
        "1";


    /*
     メッセージ
    */

    message.textContent =
        productId !== ""
            ? t(
                "shopping.message.added"
            )
            : t(
                "shopping.message.unregisteredAdded"
            );


    /*
     追加後は購入前一覧へ戻す
    */

    shoppingViewMode =
        "pending";


    const addArea =
        document.getElementById(
            "shoppingAddArea"
        );


    if (addArea) {

        addArea.hidden =
            true;

    }


    /*
     ＋追加で非表示にしていた
     買い物終了を戻す
    */

    const finishButton =
        document.getElementById(
            "btnFinishShopping"
        );


    if (finishButton) {

        finishButton.style.display =
            "";

    }


    updateShoppingViewButtons();

    updateShoppingCounts();

    displayShoppingItemsByCurrentView();

}


/* ==========================================
   購入予定から商品判断画面へ
   ========================================== */

function openShoppingProductDecision(
    itemId
) {

    const item =
        shoppingItems.find(

            item =>
                item.id === itemId

        );

    if (!item) {

        return;

    }

    pendingShoppingItemId =
        item.id;

    openRegisteredJanInfo(

        item.productId,

        "shopping"

    );

}


/* ==========================================
   購入予定から削除
   ========================================== */

function removeShoppingItem(
    itemId
) {

    shoppingItems =
        shoppingItems.filter(

            item =>
                item.id !== itemId

        );

    saveShoppingItems();

    updateShoppingCounts();

    displayShoppingItemsByCurrentView();

}

/* ==========================================
   購入予定 状態変更
   ========================================== */

function changeShoppingItemStatus(
    shoppingItemId,
    status
) {

    const item =
        shoppingItems.find(

            item =>
                item.id ===
                shoppingItemId

        );

    if (!item) {

        return false;

    }


    const allowedStatuses = [

        "pending",

        "purchased",

        "hold"

    ];


    if (
        !allowedStatuses.includes(
            status
        )
    ) {

        return false;

    }


    item.status =
        status;

    item.updatedAt =
        new Date().toISOString();


    /*
     保留にした日時を記録
    */

    if (status === "hold") {

        item.holdStartedAt =
            new Date().toISOString();

    } else {

        item.holdStartedAt =
            null;

    }


    saveShoppingItems();

    updateShoppingCounts();

    displayShoppingItemsByCurrentView();

    return true;

}

/* ==========================================
   購入予定 表示切り替え
   ========================================== */

function changeShoppingView(
    viewMode
) {

    shoppingViewMode =
        viewMode;


    const addArea =
        document.getElementById(
            "shoppingAddArea"
        );

    const finishButton =
        document.getElementById(
            "btnFinishShopping"
        );


    /*
     ＋追加
    */

    if (
        viewMode === "add"
    ) {

        if (addArea) {

            addArea.hidden =
                false;

        }


        if (finishButton) {

            finishButton.style.display =
                "none";

        }

    } else {

        if (addArea) {

            addArea.hidden =
                true;

        }


        if (finishButton) {

            finishButton.style.display =
                "";

        }

    }


    updateShoppingViewButtons();

    updateShoppingCounts();

    displayShoppingItemsByCurrentView();

}

/* ==========================================
   買い物終了
   最終確認
   ========================================== */

function finishShopping() {

    const pendingItems =
        shoppingItems.filter(

            item =>
                item.status ===
                "pending"

        );


    const purchasedItems =
        shoppingItems.filter(

            item =>
                item.status ===
                "purchased"

        );


    const holdItems =
        shoppingItems.filter(

            item =>
                item.status ===
                "hold"

        );


    /*
     購入済み商品がない場合
    */

    if (
        purchasedItems.length === 0
    ) {

        alert(
            t(
                "shopping.finish.noPurchased"
            )
        );

        return;

    }


    /*
     購入前の商品が残っている場合
    */

    if (
        pendingItems.length > 0
    ) {

        const continueFinish =
            confirm(

                t(
                    pendingItems.length === 1
                        ? "shopping.finish.pendingWarningOne"
                        : "shopping.finish.pendingWarning"
                ).replace(
                    "{count}",
                    pendingItems.length
                )

            );


        if (!continueFinish) {

            /*
             購入前へ戻す
            */

            changeShoppingView(
                "pending"
            );

            return;

        }

    }


    /*
 最終確認
*/

    const purchasedText =
        t(
            purchasedItems.length === 1
                ? "shopping.finish.itemCountOne"
                : "shopping.finish.itemCount"
        ).replace(
            "{count}",
            purchasedItems.length
        );

    const pendingText =
        t(
            pendingItems.length === 1
                ? "shopping.finish.itemCountOne"
                : "shopping.finish.itemCount"
        ).replace(
            "{count}",
            pendingItems.length
        );

    const holdText =
        t(
            holdItems.length === 1
                ? "shopping.finish.itemCountOne"
                : "shopping.finish.itemCount"
        ).replace(
            "{count}",
            holdItems.length
        );


    const confirmed =
        confirm(

            t(
                "shopping.finish.confirm"
            )
                .replace(
                    "{purchasedText}",
                    purchasedText
                )
                .replace(
                    "{pendingText}",
                    pendingText
                )
                .replace(
                    "{holdText}",
                    holdText
                )

        );


    if (!confirmed) {

        return;

    }

    /*
     購入済み商品の
     店・価格情報を確認
    */

    const missingPurchaseInfoItems =
        purchasedItems.filter(

            item =>
                !getShoppingPurchaseStoreCheck(
                    item
                )

        );


    if (
        missingPurchaseInfoItems.length > 0
    ) {

        const names =
            missingPurchaseInfoItems

                .map(

                    item => {

                        const product =
                            products.find(

                                product =>
                                    product.id ===
                                    item.productId

                            );


                        return product
                            ? product.name
                            : (
                                item.productName ||
                                t(
                                    "shopping.productNameUnset"
                                )
                            );

                    }

                )

                .join("\n・");


        alert(

            t(
                "shopping.finish.missingInfo"
            ).replace(
                "{names}",
                names
            )

        );


        return;

    }

    /*
     購入済み商品を正式確定
    */

    let savedHistoryCount =
        0;

    let registeredProductCount =
        0;


    /*
     未登録商品だった店頭情報を
     正式productIdへ付け替えたかどうか
    */

    let storeChecksChanged =
        false;


    for (
        const shoppingItem of
        purchasedItems
    ) {

        /*
         お出かけチェックから追加された商品なら
         購入完了として元の持ち物を準備済にする
        */

        completeOutingItemsFromShopping(
            shoppingItem
        );

        /*
         今回購入した店・価格
        */

        const storeCheck =
            getShoppingPurchaseStoreCheck(
                shoppingItem
            );


        if (!storeCheck) {

            continue;

        }


        /*
         元々未登録商品だったか
        */

        const wasUnregistered =
            !shoppingItem.productId;


        /*
         正式商品を取得または作成
        */

        const product =
            registerShoppingItemProduct(
                shoppingItem,
                storeCheck
            );


        if (!product) {

            continue;

        }


        if (wasUnregistered) {

            registeredProductCount++;

        }


        /*
         未登録商品の店頭情報は
         shoppingItemId参照から
         productId参照へ変更
        */

        if (wasUnregistered) {

            for (
                const check of
                storeChecks
            ) {

                if (
                    check.shoppingItemId ===
                    shoppingItem.id
                ) {

                    check.productId =
                        product.id;

                    check.shoppingItemId =
                        null;

                    storeChecksChanged =
                        true;

                }

            }

        }


        /*
         購入履歴へ正式保存
        */

        const priceCalculation = {

            originalPrice:
                storeCheck.taxExcludedPrice,

            taxExcludedPrice:
                storeCheck.taxExcludedPrice,

            priceType:
                "taxExcluded",

            taxRate:
                storeCheck.taxRate,

            roundingMode:
                storeCheck.roundingMode ||
                "round",

            discountType:
                storeCheck.discountType ||
                "none",

            discountValue:
                Number(
                    storeCheck.discountValue ||
                    0
                )

        };


        const history =
            addPriceHistory(

                product.id,

                storeCheck.storeId,

                storeCheck.taxIncludedPrice,

                shoppingItem.quantity ||
                1,

                "",

                "",

                priceCalculation

            );


        if (history) {

            savedHistoryCount++;

        }

    }


    /*
     新しく作った商品を保存
    */

    if (
        registeredProductCount > 0
    ) {

        saveProducts();

    }


    /*
     店頭情報の参照先変更を保存
    */

    if (storeChecksChanged) {

        saveStoreChecks();

    }


    /*
     購入済み商品だけ
     購入予定から削除
    
     購入前・保留は残す
    */

    shoppingItems =
        shoppingItems.filter(

            item =>
                item.status !==
                "purchased"

        );


    saveShoppingItems();


    /*
     表示更新
    */

    shoppingViewMode =
        "pending";


    updateShoppingViewButtons();

    updateShoppingCounts();

    displayShoppingItemsByCurrentView();


    /*
     完了メッセージ
    */

    const historyText =
        t(
            savedHistoryCount === 1
                ? "shopping.finish.itemCountOne"
                : "shopping.finish.itemCount"
        ).replace(
            "{count}",
            savedHistoryCount
        );

    const registeredText =
        t(
            registeredProductCount === 1
                ? "shopping.finish.itemCountOne"
                : "shopping.finish.itemCount"
        ).replace(
            "{count}",
            registeredProductCount
        );


    alert(

        t(
            "shopping.finish.completed"
        )
            .replace(
                "{historyText}",
                historyText
            )
            .replace(
                "{registeredText}",
                registeredText
            )

    );

}

/* ==========================================
   購入予定 切り替えボタン表示
   ========================================== */

function updateShoppingViewButtons() {

    const buttons = {

        pending:
            document.getElementById(
                "btnShoppingPending"
            ),

        purchased:
            document.getElementById(
                "btnShoppingPurchased"
            ),

        hold:
            document.getElementById(
                "btnShoppingHold"
            ),

        add:
            document.getElementById(
                "btnShoppingAddView"
            )

    };


    for (
        const [
            mode,
            button
        ] of
        Object.entries(buttons)
    ) {

        if (!button) {

            continue;

        }

        button.classList.toggle(

            "active",

            shoppingViewMode ===
            mode

        );

    }

}

/* ==========================================
   購入予定 件数更新
   ========================================== */

function updateShoppingCounts() {

    const pendingCount =
        shoppingItems.filter(

            item =>
                item.status === "pending"

        ).length;


    const purchasedCount =
        shoppingItems.filter(

            item =>
                item.status === "purchased"

        ).length;


    const holdCount =
        shoppingItems.filter(

            item =>
                item.status === "hold"

        ).length;


    const pendingCountElement =
        document.getElementById(
            "shoppingPendingCount"
        );

    const purchasedCountElement =
        document.getElementById(
            "shoppingPurchasedCount"
        );

    const holdCountElement =
        document.getElementById(
            "shoppingHoldCount"
        );

    const remainingCountElement =
        document.getElementById(
            "shoppingRemainingCount"
        );


    if (pendingCountElement) {

        pendingCountElement.textContent =
            pendingCount;

    }


    if (purchasedCountElement) {

        purchasedCountElement.textContent =
            purchasedCount;

    }


    if (holdCountElement) {

        holdCountElement.textContent =
            holdCount;

    }


    if (remainingCountElement) {

        remainingCountElement.textContent =
            pendingCount;

    }

}

/* ==========================================
   購入予定 状態別一覧
   ========================================== */

function displayShoppingItemsByCurrentView() {

    const shoppingList =
        document.getElementById(
            "shoppingList"
        );

    if (!shoppingList) {

        return;

    }


    shoppingList.innerHTML =
        "";


    /*
     ＋追加画面では
     商品一覧を表示しない
    */

    if (
        shoppingViewMode === "add"
    ) {

        return;

    }


    const targetItems =
        shoppingItems.filter(

            item =>
                item.status ===
                shoppingViewMode

        );


    if (
        targetItems.length === 0
    ) {

        let emptyMessage =
            t(
                "shopping.empty.pending"
            );


        if (
            shoppingViewMode ===
            "purchased"
        ) {

            emptyMessage =
                t(
                    "shopping.empty.purchased"
                );

        } else if (
            shoppingViewMode ===
            "hold"
        ) {

            emptyMessage =
                t(
                    "shopping.empty.hold"
                );

        }


        shoppingList.innerHTML = `

            <p class="shopping-empty-message">

                ${emptyMessage}

            </p>

        `;

        return;

    }


    for (
        const item of targetItems
    ) {

        const product =
            products.find(

                product =>
                    product.id ===
                    item.productId

            );


        /*
         登録済み商品なら
         商品マスターの商品名を利用

         将来の未登録商品にも対応
        */

        const productName =

            product
                ? product.name
                : (
                    item.productName ||
                    item.name ||
                    t(
                        "shopping.productNameUnset"
                    )
                );

        /*
         お出かけチェック経由の商品マーク
        */

        const outingMark =

            Array.isArray(
                item.outingLinks
            ) &&
                item.outingLinks.length > 0

                ? "🎒 "
                : "";

        const quantity =
            item.quantity || 1;


        /*
         前回購入店
        */

        let previousStoreText =
            t(
                "shopping.previousPurchaseNone"
            );


        if (product) {

            const latestHistory =
                getLatestPriceHistory(
                    product.id
                );


            if (latestHistory) {

                previousStoreText =
                    t(
                        "shopping.previousPurchase"
                    ).replace(
                        "{store}",
                        getStoreDisplayName(
                            latestHistory.storeId
                        )
                    );

            }

        }


        const itemRow =
            document.createElement(
                "div"
            );


        itemRow.className =
            "shopping-item-row";


        itemRow.innerHTML = `

            <button
                type="button"
                class="shopping-item-main"
                data-shopping-id="${item.id}">

                <span class="shopping-item-name">

                    ${outingMark}${productName}

                </span>

                <span class="shopping-item-sub">

                    ${previousStoreText}

                </span>

                <span class="shopping-item-quantity">

                    ${t(
            "shopping.quantityDisplay"
        ).replace(
            "{quantity}",
            quantity
        )}

                </span>

                <span class="shopping-item-arrow">

                    〉

                </span>

            </button>

        `;

        const itemButton =
            itemRow.querySelector(
                ".shopping-item-main"
            );


        if (itemButton) {

            itemButton.addEventListener(

                "click",

                () => {

                    openShoppingItemDetail(
                        item.id
                    );

                }

            );

        }

        shoppingList.appendChild(
            itemRow
        );

    }

}

/* ==========================================
   購入予定詳細 販売店候補
   ========================================== */

function displayShoppingDetailStoreOptions() {

    const select =
        document.getElementById(
            "cmbShoppingDetailStore"
        );

    const previousArea =
        document.getElementById(
            "shoppingDetailPreviousStoreQuickArea"
        );

    const previousContainer =
        document.getElementById(
            "shoppingDetailPreviousStoreQuick"
        );

    const recentArea =
        document.getElementById(
            "shoppingDetailRecentStoresArea"
        );

    const recentContainer =
        document.getElementById(
            "shoppingDetailRecentStores"
        );


    if (!select) {

        return;
    }


    /*
     すべての販売店を表示
    */
    select.innerHTML = `

         <option value="">
            ${t(
        "shopping.store.select"
    )}
         </option>

    `;


    for (const store of stores) {

        if (
            store.active === false
        ) {

            continue;
        }


        const option =
            document.createElement(
                "option"
            );

        option.value =
            store.id;

        option.textContent =
            getStoreDisplayName(
                store.id
            );


        select.appendChild(
            option
        );

    }


    /*
     現在開いている購入予定商品
    */
    const shoppingItem =
        shoppingItems.find(
            item =>
                item.id ===
                shoppingDetailItemId
        );


    const product =
        shoppingItem &&
            shoppingItem.productId
            ? products.find(
                product =>
                    product.id ===
                    shoppingItem.productId
            )
            : null;


    /*
     前回購入した店舗
    */
    let previousStoreId =
        "";


    if (product) {

        const latestHistory =
            getLatestPriceHistory(
                product.id
            );


        if (
            latestHistory &&
            latestHistory.storeId
        ) {

            const previousStore =
                findStore(
                    latestHistory.storeId
                );


            if (
                previousStore &&
                previousStore.active !== false
            ) {

                previousStoreId =
                    previousStore.id;

            }

        }

    }

    /*
     購入予定詳細
     初期購入先を決定

     1. 前回購入した店舗
     2. 商品登録時の店舗
     3. どちらもなければ未選択
    */

    let initialStoreId =
        previousStoreId;


    if (
        !initialStoreId &&
        product &&
        product.storeId
    ) {

        const registeredStore =
            findStore(
                product.storeId
            );


        if (
            registeredStore &&
            registeredStore.active !== false
        ) {

            initialStoreId =
                registeredStore.id;

        }

    }


    select.value =
        initialStoreId || "";


    /*
     初期購入先に合わせて
     国・通貨・税率表示を更新
    */

    select.dispatchEvent(
        new Event(
            "change",
            {
                bubbles: true
            }
        )
    );

    /*
     前回購入店舗ボタン
    */
    if (
        previousArea &&
        previousContainer
    ) {

        previousContainer.innerHTML =
            "";


        if (previousStoreId) {

            const button =
                document.createElement(
                    "button"
                );

            button.type =
                "button";

            button.className =
                "btn-sub";

            button.textContent =
                "";

            button.appendChild(
                createStoreNameDisplay(
                    previousStoreId
                )
            );


            button.classList.add(
                "shopping-store-candidate"
            );


            button.addEventListener(
                "click",
                () => {

                    const text =
                        "🏬 " +
                        getStoreDisplayName(
                            previousStoreId
                        );


                    handleShoppingStoreCandidateClick(
                        previousStoreId,
                        text
                    );

                }
            );


            previousContainer.appendChild(
                button
            );


            previousArea.hidden =
                false;

        } else {

            previousArea.hidden =
                true;

        }

    }


    /*
     最近確認した店舗

     storeChecks の checkedAt が
     新しいものから取得する
    */
    const recentStoreIds =
        [];


    const sortedChecks =
        [...storeChecks]
            .sort(
                (a, b) =>
                    new Date(
                        b.checkedAt || 0
                    ) -
                    new Date(
                        a.checkedAt || 0
                    )
            );


    for (const check of sortedChecks) {

        if (
            !check.storeId
        ) {

            continue;
        }


        /*
         前回購入店は
         上に表示しているので除外
        */
        if (
            check.storeId ===
            previousStoreId
        ) {

            continue;
        }


        /*
         同じ店舗を重複表示しない
        */
        if (
            recentStoreIds.includes(
                check.storeId
            )
        ) {

            continue;
        }


        const store =
            findStore(
                check.storeId
            );


        if (
            !store ||
            store.active === false
        ) {

            continue;
        }


        recentStoreIds.push(
            check.storeId
        );


        /*
         最大3店舗
        */
        if (
            recentStoreIds.length >= 3
        ) {

            break;
        }

    }


    /*
     最近確認した店舗ボタン
    */
    if (
        recentArea &&
        recentContainer
    ) {

        recentContainer.innerHTML =
            "";


        if (
            recentStoreIds.length > 0
        ) {

            for (
                const storeId of
                recentStoreIds
            ) {

                const button =
                    document.createElement(
                        "button"
                    );

                button.type =
                    "button";

                button.className =
                    "btn-sub";

                button.textContent =
                    "";

                button.appendChild(
                    createStoreNameDisplay(
                        storeId
                    )
                );


                button.classList.add(
                    "shopping-store-candidate"
                );


                button.addEventListener(
                    "click",
                    () => {

                        const text =
                            "🏬 " +
                            getStoreDisplayName(
                                storeId
                            );


                        handleShoppingStoreCandidateClick(
                            storeId,
                            text
                        );

                    }
                );


                recentContainer.appendChild(
                    button
                );

            }


            recentArea.hidden =
                false;

        } else {

            recentArea.hidden =
                true;

        }

    }


    setupShoppingDetailStoreSearch();

}

/* ==========================================
   今回の購入
   購入先候補を表示
   ========================================== */

function displayPriceRecordStoreOptions() {

    const select =
        document.getElementById(
            "cmbPriceRecordStore"
        );


    if (!select) {

        return;

    }


    /*
     現在選択している購入先を保持
    */

    const currentStoreId =
        select.value;


    /*
     すべての購入先
    */
    select.innerHTML = `
        <option value="">
            ${t(
        "purchase.selectStore"
    )}
        </option>
    `;


    for (const store of stores) {

        if (
            store.active === false
        ) {

            continue;

        }


        const option =
            document.createElement(
                "option"
            );


        option.value =
            store.id;


        option.textContent =
            getStoreDisplayName(
                store.id
            );


        select.appendChild(
            option
        );

    }

    /*
     現在選択していた購入先が
     引き続き存在する場合は復元
    */

    if (
        currentStoreId &&
        Array.from(
            select.options
        ).some(
            option =>
                option.value ===
                currentStoreId
        )
    ) {

        select.value =
            currentStoreId;

    }

    /*
     前回購入した店舗
    */
    const previousArea =
        document.getElementById(
            "priceRecordPreviousStoreQuickArea"
        );


    const previousContainer =
        document.getElementById(
            "priceRecordPreviousStoreQuick"
        );


    if (
        previousArea &&
        previousContainer
    ) {

        previousContainer.innerHTML =
            "";


        const latestHistory =
            recordingProductId
                ? getLatestPriceHistory(
                    recordingProductId
                )
                : null;


        const previousStoreId =
            latestHistory &&
                latestHistory.storeId
                ? latestHistory.storeId
                : "";


        const previousStore =
            previousStoreId
                ? findStore(
                    previousStoreId
                )
                : null;


        if (
            previousStore &&
            previousStore.active !== false
        ) {

            const button =
                document.createElement(
                    "button"
                );


            button.type =
                "button";


            button.className =
                "btn-sub";


            button.textContent =
                "";


            button.appendChild(
                createStoreNameDisplay(
                    previousStoreId
                )
            );


            button.classList.add(
                "shopping-store-candidate"
            );


            button.addEventListener(
                "click",
                () => {

                    const text =
                        "🏬 " +
                        getStoreDisplayName(
                            previousStoreId
                        );


                    handlePriceRecordStoreCandidateClick(
                        previousStoreId,
                        text
                    );

                }
            );


            previousContainer.appendChild(
                button
            );


            previousArea.hidden =
                false;

        } else {

            previousArea.hidden =
                true;

        }

    }


    /*
     最近使った店舗
     購入履歴から新しい順に取得
    */
    const recentArea =
        document.getElementById(
            "priceRecordRecentStoresArea"
        );


    const recentContainer =
        document.getElementById(
            "priceRecordRecentStores"
        );


    if (
        recentArea &&
        recentContainer
    ) {

        recentContainer.innerHTML =
            "";


        const recentStoreIds =
            [];


        /*
         購入履歴全体から
         新しい履歴順に確認する
        */
        const sortedHistories =
            [...priceHistory]
                .sort(
                    (a, b) =>
                        new Date(
                            b.purchasedAt ||
                            b.createdAt ||
                            0
                        ) -
                        new Date(
                            a.purchasedAt ||
                            a.createdAt ||
                            0
                        )
                );


        for (
            const history of
            sortedHistories
        ) {

            if (
                !history.storeId
            ) {

                continue;

            }


            /*
             前回購入した店舗は
             上に表示しているため除外
            */
            const latestHistory =
                recordingProductId
                    ? getLatestPriceHistory(
                        recordingProductId
                    )
                    : null;


            const previousStoreId =
                latestHistory &&
                    latestHistory.storeId
                    ? latestHistory.storeId
                    : "";


            if (
                history.storeId ===
                previousStoreId
            ) {

                continue;

            }


            /*
             同じ店舗を重複表示しない
            */
            if (
                recentStoreIds.includes(
                    history.storeId
                )
            ) {

                continue;

            }


            const store =
                findStore(
                    history.storeId
                );


            if (
                !store ||
                store.active === false
            ) {

                continue;

            }


            recentStoreIds.push(
                history.storeId
            );


            /*
             最大3店舗
            */
            if (
                recentStoreIds.length >= 3
            ) {

                break;

            }

        }


        /*
         最近使った店舗ボタン
        */
        if (
            recentStoreIds.length > 0
        ) {

            for (
                const storeId of
                recentStoreIds
            ) {

                const button =
                    document.createElement(
                        "button"
                    );


                button.type =
                    "button";


                button.className =
                    "btn-sub";


                button.textContent =
                    "";


                button.appendChild(
                    createStoreNameDisplay(
                        storeId
                    )
                );


                button.classList.add(
                    "shopping-store-candidate"
                );


                button.addEventListener(
                    "click",
                    () => {

                        const text =
                            "🏬 " +
                            getStoreDisplayName(
                                storeId
                            );


                        handlePriceRecordStoreCandidateClick(
                            storeId,
                            text
                        );

                    }
                );


                recentContainer.appendChild(
                    button
                );

            }


            recentArea.hidden =
                false;

        } else {

            recentArea.hidden =
                true;

        }

    }


    /*
     今回の購入
     店舗検索を準備
    */
    setupPriceRecordStoreSearch();

}

/* ==========================================
   今回の購入
   購入先を選択
   ========================================== */

function selectPriceRecordStore(
    storeId
) {

    const select =
        document.getElementById(
            "cmbPriceRecordStore"
        );


    if (!select) {

        return;

    }


    /*
     選択候補に存在するか確認
    */
    const storeExists =
        Array.from(
            select.options
        ).some(
            option =>
                option.value ===
                storeId
        );


    if (!storeExists) {

        return;

    }


    /*
     購入先を設定
    */
    select.value =
        storeId;


    /*
     通常のselect変更と
     同じ処理を動かす
    */
    select.dispatchEvent(
        new Event(
            "change",
            {
                bubbles: true
            }
        )
    );


    /*
     全文確認モードを終了
    */
    closeShoppingStoreNameConfirm();

}

/* ==========================================
   今回の購入
   購入先候補をタップ
   ========================================== */

function handlePriceRecordStoreCandidateClick(
    storeId,
    text
) {

    /*
     長押し直後に発生するclickでは
     選択しない
    */
    if (
        shoppingStoreNameLongPressTriggered
    ) {

        shoppingStoreNameLongPressTriggered =
            false;

        return;

    }


    /*
     全文確認モード中
     → 選択せず表示だけ切り替える
    */
    if (
        shoppingStoreNameConfirmMode
    ) {

        openShoppingStoreNameConfirm(
            storeId,
            text
        );

        return;

    }


    /*
     通常時
     → 今回の購入先として選択
    */
    selectPriceRecordStore(
        storeId
    );

}

/* ==========================================
   今回の購入
   店舗検索 初期設定
   ========================================== */

function setupPriceRecordStoreSearch() {

    const toggleButton =
        document.getElementById(
            "btnTogglePriceRecordStoreSearch"
        );

    const panel =
        document.getElementById(
            "priceRecordStoreSearchPanel"
        );

    const countrySelect =
        document.getElementById(
            "cmbPriceRecordStoreSearchCountry"
        );

    const regionSelect =
        document.getElementById(
            "cmbPriceRecordStoreSearchRegion"
        );

    const nameInput =
        document.getElementById(
            "txtPriceRecordStoreSearchName"
        );


    if (
        !toggleButton ||
        !panel ||
        !countrySelect ||
        !regionSelect ||
        !nameInput
    ) {

        return;

    }

    /*
 今回の購入
 店舗検索の初期国を決定

 1. 前回購入した店舗の国
 2. 商品登録時の店舗の国
 3. 普段買い物する国・地域
*/

    const latestHistory =
        recordingProductId
            ? getLatestPriceHistory(
                recordingProductId
            )
            : null;

    const previousStore =
        latestHistory &&
            latestHistory.storeId
            ? findStore(
                latestHistory.storeId
            )
            : null;

    const product =
        recordingProductId
            ? products.find(
                product =>
                    product.id ===
                    recordingProductId
            )
            : null;

    const registeredStore =
        product &&
            product.storeId
            ? findStore(
                product.storeId
            )
            : null;

    const initialCountry =
        previousStore &&
            previousStore.country
            ? previousStore.country
            : registeredStore &&
                registeredStore.country
                ? registeredStore.country
                : getCocartlyBaseCountry();

    countrySelect.value =
        initialCountry;

    /*
     同じイベントを何度も
     登録しないようにする
    */
    if (
        toggleButton.dataset.storeSearchReady ===
        "true"
    ) {

        displayPriceRecordStoreSearchRegions();

        displayPriceRecordStoreSearchResults();

        return;

    }


    toggleButton.dataset.storeSearchReady =
        "true";


    /*
     検索パネル開閉
    */
    toggleButton.addEventListener(
        "click",
        () => {

            panel.hidden =
                !panel.hidden;


            if (!panel.hidden) {

                displayPriceRecordStoreSearchRegions();

                displayPriceRecordStoreSearchResults();

            }

        }
    );


    /*
     国・地域変更
    */
    countrySelect.addEventListener(
        "change",
        () => {

            displayPriceRecordStoreSearchRegions();

            displayPriceRecordStoreSearchResults();

        }
    );


    /*
     都道府県・地域変更
    */
    regionSelect.addEventListener(
        "change",
        () => {

            displayPriceRecordStoreSearchResults();

        }
    );


    /*
     店舗名検索
    */
    nameInput.addEventListener(
        "input",
        () => {

            displayPriceRecordStoreSearchResults();

        }
    );


    displayPriceRecordStoreSearchRegions();

    displayPriceRecordStoreSearchResults();

}


/* ==========================================
   今回の購入
   店舗検索 地域候補
   ========================================== */

function displayPriceRecordStoreSearchRegions() {

    const countrySelect =
        document.getElementById(
            "cmbPriceRecordStoreSearchCountry"
        );

    const regionSelect =
        document.getElementById(
            "cmbPriceRecordStoreSearchRegion"
        );


    if (
        !countrySelect ||
        !regionSelect
    ) {

        return;

    }


    const selectedCountry =
        countrySelect.value;


    regionSelect.innerHTML = `
        <option value="">
            ${t(
        "purchase.allRegions"
    )}
        </option>
    `;


    const regions =
        [];


    for (const store of stores) {

        if (
            store.active === false
        ) {

            continue;

        }


        const storeCountry =
            store.country ||
            "JP";


        if (
            selectedCountry &&
            storeCountry !==
            selectedCountry
        ) {

            continue;

        }


        const displayInfo =
            typeof getDefaultStoreDisplayInfo ===
                "function"

                ? getDefaultStoreDisplayInfo(
                    store
                )

                : {
                    name: store.name || "",
                    region: store.region || "",
                    city: store.city || ""
                };


        const region =
            (
                displayInfo.region ||
                ""
            ).trim();


        if (!region) {

            continue;

        }


        if (
            regions.includes(
                region
            )
        ) {

            continue;

        }


        regions.push(
            region
        );

    }


    regions.sort(
        (a, b) =>
            a.localeCompare(
                b,
                "ja"
            )
    );


    for (
        const region of
        regions
    ) {

        const option =
            document.createElement(
                "option"
            );


        option.value =
            region;


        option.textContent =
            region;


        regionSelect.appendChild(
            option
        );

    }

}


/* ==========================================
   今回の購入
   店舗検索 結果表示
   ========================================== */

function displayPriceRecordStoreSearchResults() {

    const countrySelect =
        document.getElementById(
            "cmbPriceRecordStoreSearchCountry"
        );

    const regionSelect =
        document.getElementById(
            "cmbPriceRecordStoreSearchRegion"
        );

    const nameInput =
        document.getElementById(
            "txtPriceRecordStoreSearchName"
        );

    const results =
        document.getElementById(
            "priceRecordStoreSearchResults"
        );


    if (
        !countrySelect ||
        !regionSelect ||
        !nameInput ||
        !results
    ) {

        return;

    }


    const selectedCountry =
        countrySelect.value;


    const selectedRegion =
        regionSelect.value;


    const keyword =
        nameInput.value
            .trim()
            .toLowerCase();


    results.innerHTML =
        "";


    /*
     条件に一致する店舗
    */
    const matchedStores =
        stores.filter(
            store => {

                if (
                    store.active === false
                ) {

                    return false;

                }


                const storeCountry =
                    store.country ||
                    "JP";


                if (
                    selectedCountry &&
                    storeCountry !==
                    selectedCountry
                ) {

                    return false;

                }


                const displayInfo =
                    typeof getDefaultStoreDisplayInfo ===
                        "function"

                        ? getDefaultStoreDisplayInfo(
                            store
                        )

                        : {
                            name: store.name || "",
                            region: store.region || "",
                            city: store.city || ""
                        };


                const storeRegion =
                    (
                        displayInfo.region ||
                        ""
                    ).trim();


                if (
                    selectedRegion &&
                    storeRegion !==
                    selectedRegion
                ) {

                    return false;

                }


                const searchText =
                    [
                        displayInfo.name || "",
                        displayInfo.region || "",
                        displayInfo.city || ""
                    ]
                        .join(" ")
                        .toLowerCase();


                if (
                    keyword &&
                    !searchText.includes(
                        keyword
                    )
                ) {

                    return false;

                }


                return true;

            }
        );


    /*
     最大10店舗
    */
    const visibleStores =
        matchedStores.slice(
            0,
            10
        );


    if (
        visibleStores.length === 0
    ) {

        const message =
            document.createElement(
                "p"
            );


        message.textContent =
            t(
                "shopping.store.noResults"
            );


        results.appendChild(
            message
        );


        return;

    }


    for (
        const store of
        visibleStores
    ) {

        const button =
            document.createElement(
                "button"
            );


        button.type =
            "button";


        button.className =
            "btn-sub";


        /*
         地域・市区町村
        */
        const displayInfo =
            typeof getDefaultStoreDisplayInfo ===
                "function"

                ? getDefaultStoreDisplayInfo(
                    store
                )

                : {
                    name: store.name || "",
                    region: store.region || "",
                    city: store.city || ""
                };


        const locationText =
            [
                displayInfo.region || "",
                displayInfo.city || ""
            ]
                .filter(
                    value =>
                        value !== ""
                )
                .join(" ");


        button.textContent =
            "";


        /*
         店舗名
         長い場合は既存の全文確認ルール
        */
        const storeNameSpan =
            createStoreNameDisplay(
                store.id
            );


        /*
         画面上には地域も表示する
         長押し全文表示は店舗名だけ
        */
        if (locationText) {

            storeNameSpan.textContent +=
                "　" +
                locationText;

        }


        button.appendChild(
            storeNameSpan
        );


        button.classList.add(
            "shopping-store-candidate"
        );


        button.addEventListener(
            "click",
            () => {

                const text =
                    "🏬 " +
                    getStoreDisplayName(
                        store.id
                    );


                const wasConfirmMode =
                    shoppingStoreNameConfirmMode;


                handlePriceRecordStoreCandidateClick(
                    store.id,
                    text
                );


                /*
                 通常選択した時だけ
                 検索パネルを閉じる
                */
                if (
                    !wasConfirmMode &&
                    !shoppingStoreNameLongPressTriggered
                ) {

                    const panel =
                        document.getElementById(
                            "priceRecordStoreSearchPanel"
                        );


                    if (panel) {

                        panel.hidden =
                            true;

                    }

                }

            }
        );


        results.appendChild(
            button
        );

    }


    /*
     10件を超えた場合
    */
    if (
        matchedStores.length > 10
    ) {

        const message =
            document.createElement(
                "small"
            );


        message.textContent =
            t(
                "shopping.store.tooMany"
            );


        results.appendChild(
            message
        );

    }

}

/* ==========================================
   店舗名確認モード
   ========================================== */

let shoppingStoreNameConfirmMode =
    false;

let shoppingStoreNameLongPressTimer =
    null;

let shoppingStoreNameLongPressTriggered =
    false;

let shoppingStoreNameConfirmStoreId =
    "";


/* ==========================================
   店舗名全文表示
   ========================================== */

function showStoreNameFullPopup(
    text
) {

    let popup =
        document.getElementById(
            "storeNameFullPopup"
        );


    if (!popup) {

        popup =
            document.createElement(
                "div"
            );

        popup.id =
            "storeNameFullPopup";

        popup.hidden =
            true;


        document.body.appendChild(
            popup
        );

    }


    popup.textContent =
        text;

    popup.hidden =
        false;

}


/* ==========================================
   店舗名確認モード開始・切り替え
   ========================================== */

function openShoppingStoreNameConfirm(
    storeId,
    text
) {

    shoppingStoreNameConfirmMode =
        true;

    shoppingStoreNameConfirmStoreId =
        storeId;


    /*
     店舗情報を取得
    */
    const store =
        findStore(
            storeId
        );


    /*
     店舗名
    */
    const storeName =
        "🏬 " +
        getStoreDisplayName(
            storeId
        );


    /*
     表示用店舗情報
    */
    const displayInfo =
        store &&
            typeof getDefaultStoreDisplayInfo ===
            "function"

            ? getDefaultStoreDisplayInfo(
                store
            )

            : store
                ? {
                    name: store.name || "",
                    region: store.region || "",
                    city: store.city || ""
                }

                : null;


    /*
     都道府県・州
     ＋
     区・市
    */
    const locationText =
        displayInfo
            ? [
                displayInfo.region || "",
                displayInfo.city || ""
            ]
                .filter(
                    value =>
                        value !== ""
                )
                .join(" ")
            : "";


    /*
     全文確認では

     店舗名
     ＋
     都道府県
     ＋
     区・市

     を表示
    */
    const fullText =
        locationText
            ? storeName +
            "　" +
            locationText
            : storeName;


    showStoreNameFullPopup(
        fullText
    );


    /*
     どの店舗を確認中なのか
     全候補へ反映
    */
    const candidates =
        document.querySelectorAll(
            ".shopping-store-candidate"
        );


    candidates.forEach(
        candidate => {

            const nameElement =
                candidate.querySelector(
                    ".store-name-ellipsis"
                );


            const candidateStoreId =
                nameElement
                    ? nameElement.dataset.storeId
                    : "";


            candidate.classList.toggle(
                "store-name-confirm-active",
                candidateStoreId ===
                storeId
            );

        }
    );

}


/* ==========================================
   店舗名確認モード終了
   ========================================== */

function closeShoppingStoreNameConfirm() {

    shoppingStoreNameConfirmMode =
        false;

    shoppingStoreNameConfirmStoreId =
        "";


    const popup =
        document.getElementById(
            "storeNameFullPopup"
        );


    if (popup) {

        popup.hidden =
            true;

    }


    /*
     確認中の色を
     すべて解除
    */
    const activeCandidates =
        document.querySelectorAll(
            ".store-name-confirm-active"
        );


    activeCandidates.forEach(
        candidate => {

            candidate.classList.remove(
                "store-name-confirm-active"
            );

        }
    );

}


/* ==========================================
   店舗候補をタップした時
   ========================================== */

function handleShoppingStoreCandidateClick(
    storeId,
    text
) {

    /*
     長押し直後に発生するclickは
     店舗選択させない
    */
    if (
        shoppingStoreNameLongPressTriggered
    ) {

        shoppingStoreNameLongPressTriggered =
            false;

        return;
    }


    /*
     確認モード中は
     選択せず全文だけ切り替える
    */
    if (
        shoppingStoreNameConfirmMode
    ) {

        openShoppingStoreNameConfirm(
            storeId,
            text
        );

        return;
    }


    /*
     通常時は店舗を選択
    */
    selectShoppingDetailStore(
        storeId
    );

}


/* ==========================================
   店舗名表示用
   長い場合だけ省略表示
   長押しで全文確認
   ========================================== */

function createStoreNameDisplay(
    storeId,
    extraText = ""
) {

    const span =
        document.createElement(
            "span"
        );


    const storeName =
        "🏬 " +
        getStoreDisplayName(
            storeId
        );


    const fullText =
        extraText
            ? storeName +
            "　" +
            extraText
            : storeName;


    span.className =
        "store-name-ellipsis";


    /*
     この表示が
     どの店舗なのか保持
    */
    span.dataset.storeId =
        storeId;


    span.textContent =
        fullText;


    /*
     長押し開始
    */
    span.addEventListener(
        "pointerdown",
        () => {

            shoppingStoreNameLongPressTriggered =
                false;


            /*
             名前が実際にはみ出している時だけ
             長押しを有効にする
            */
            if (
                span.scrollWidth <=
                span.clientWidth
            ) {

                return;
            }


            clearTimeout(
                shoppingStoreNameLongPressTimer
            );


            shoppingStoreNameLongPressTimer =
                setTimeout(
                    () => {

                        shoppingStoreNameLongPressTriggered =
                            true;


                        openShoppingStoreNameConfirm(
                            storeId,
                            fullText
                        );

                    },
                    550
                );

        }
    );


    /*
     指を離した時
     タイマーだけ止める

     全文表示は閉じない
    */
    span.addEventListener(
        "pointerup",
        () => {

            clearTimeout(
                shoppingStoreNameLongPressTimer
            );

        }
    );


    /*
     長押し途中で操作が中断された場合
    */
    span.addEventListener(
        "pointercancel",
        () => {

            clearTimeout(
                shoppingStoreNameLongPressTimer
            );

        }
    );


    /*
     iPhoneなどの
     長押しメニューを防ぐ
    */
    span.addEventListener(
        "contextmenu",
        event => {

            event.preventDefault();

        }
    );


    return span;

}

/* ==========================================
   店舗名確認モード
   店舗以外をタップしたら終了
   ========================================== */

document.addEventListener(
    "click",
    event => {

        if (
            !shoppingStoreNameConfirmMode
        ) {

            return;
        }


        /*
         店舗候補を押した場合は
         確認モードを続ける
        */
        if (
            event.target.closest(
                ".shopping-store-candidate"
            )
        ) {

            return;
        }


        /*
         全文表示そのものを押した場合も
         閉じない
        */
        if (
            event.target.closest(
                "#storeNameFullPopup"
            )
        ) {

            return;
        }


        closeShoppingStoreNameConfirm();

    }
);

/* ==========================================
   購入予定詳細
   販売店をクイック選択
   ========================================== */

function selectShoppingDetailStore(
    storeId
) {

    const select =
        document.getElementById(
            "cmbShoppingDetailStore"
        );


    if (!select) {

        return;
    }


    /*
     選択候補に存在するか確認
    */
    const storeExists =
        Array.from(
            select.options
        ).some(
            option =>
                option.value ===
                storeId
        );


    if (!storeExists) {

        return;
    }


    /*
     既存のselectへ
     店舗IDを設定
    */
    select.value =
        storeId;


    /*
     通常のselect変更時と
     同じ処理を動かす

     国・通貨・税率表示も更新される
    */
    select.dispatchEvent(
        new Event(
            "change",
            {
                bubbles: true
            }
        )
    );

}

/* ==========================================
   購入予定詳細
   その他の店舗を探す
   ========================================== */

function setupShoppingDetailStoreSearch() {

    const toggleButton =
        document.getElementById(
            "btnToggleShoppingStoreSearch"
        );

    const searchPanel =
        document.getElementById(
            "shoppingStoreSearchPanel"
        );

    const countrySelect =
        document.getElementById(
            "cmbShoppingStoreSearchCountry"
        );

    const regionSelect =
        document.getElementById(
            "cmbShoppingStoreSearchRegion"
        );

    const nameInput =
        document.getElementById(
            "txtShoppingStoreSearchName"
        );


    if (
        !toggleButton ||
        !searchPanel ||
        !countrySelect ||
        !regionSelect ||
        !nameInput
    ) {

        return;
    }

    /*
     購入予定詳細
     店舗検索の初期国を決定

     1. 前回購入した店舗の国
     2. 商品登録時の店舗の国
     3. 普段買い物する国・地域
    */

    const shoppingItem =
        shoppingItems.find(
            item =>
                item.id ===
                shoppingDetailItemId
        );

    const product =
        shoppingItem &&
            shoppingItem.productId
            ? products.find(
                product =>
                    product.id ===
                    shoppingItem.productId
            )
            : null;

    const latestHistory =
        product
            ? getLatestPriceHistory(
                product.id
            )
            : null;

    const previousStore =
        latestHistory &&
            latestHistory.storeId
            ? findStore(
                latestHistory.storeId
            )
            : null;

    const registeredStore =
        product &&
            product.storeId
            ? findStore(
                product.storeId
            )
            : null;

    const initialCountry =
        previousStore &&
            previousStore.country
            ? previousStore.country
            : registeredStore &&
                registeredStore.country
                ? registeredStore.country
                : getCocartlyBaseCountry();

    countrySelect.value =
        initialCountry;

    /*
     検索画面を開く・閉じる
    */
    toggleButton.onclick =
        () => {

            searchPanel.hidden =
                !searchPanel.hidden;


            if (
                !searchPanel.hidden
            ) {

                displayShoppingStoreSearchRegions();

                displayShoppingStoreSearchResults();

            }

        };


    /*
     国を変更
    */
    countrySelect.onchange =
        () => {

            displayShoppingStoreSearchRegions();

            displayShoppingStoreSearchResults();

        };


    /*
     地域を変更
    */
    regionSelect.onchange =
        () => {

            displayShoppingStoreSearchResults();

        };


    /*
     店舗名を入力
    */
    nameInput.oninput =
        () => {

            displayShoppingStoreSearchResults();

        };

}


/* ==========================================
   店舗検索
   地域候補を表示
   ========================================== */

function displayShoppingStoreSearchRegions() {

    const countrySelect =
        document.getElementById(
            "cmbShoppingStoreSearchCountry"
        );

    const regionSelect =
        document.getElementById(
            "cmbShoppingStoreSearchRegion"
        );


    if (
        !countrySelect ||
        !regionSelect
    ) {

        return;
    }


    const selectedCountry =
        countrySelect.value;


    regionSelect.innerHTML = `

        <option value="">
            ${t(
        "shopping.store.allRegions"
    )}
        </option>

    `;


    const regions =
        [];


    for (const store of stores) {

        if (
            store.active === false
        ) {

            continue;
        }


        const storeCountry =
            store.country ||
            "JP";


        /*
         国が指定されている場合だけ
         国で絞り込む
        */
        if (
            selectedCountry &&
            storeCountry !==
            selectedCountry
        ) {

            continue;
        }


        const displayInfo =
            typeof getDefaultStoreDisplayInfo ===
                "function"

                ? getDefaultStoreDisplayInfo(
                    store
                )

                : {
                    name: store.name || "",
                    region: store.region || "",
                    city: store.city || ""
                };


        const region =
            (
                displayInfo.region ||
                ""
            ).trim();


        if (!region) {

            continue;
        }


        if (
            regions.includes(
                region
            )
        ) {

            continue;
        }


        regions.push(
            region
        );

    }


    /*
     五十音・アルファベット順
    */
    regions.sort(
        (a, b) =>
            a.localeCompare(
                b,
                "ja"
            )
    );


    for (
        const region of
        regions
    ) {

        const option =
            document.createElement(
                "option"
            );

        option.value =
            region;

        option.textContent =
            region;


        regionSelect.appendChild(
            option
        );

    }

}


/* ==========================================
   店舗検索
   検索結果を表示
   ========================================== */

function displayShoppingStoreSearchResults() {

    const countrySelect =
        document.getElementById(
            "cmbShoppingStoreSearchCountry"
        );

    const regionSelect =
        document.getElementById(
            "cmbShoppingStoreSearchRegion"
        );

    const nameInput =
        document.getElementById(
            "txtShoppingStoreSearchName"
        );

    const results =
        document.getElementById(
            "shoppingStoreSearchResults"
        );


    if (
        !countrySelect ||
        !regionSelect ||
        !nameInput ||
        !results
    ) {

        return;
    }


    const selectedCountry =
        countrySelect.value;

    const selectedRegion =
        regionSelect.value;

    const keyword =
        nameInput.value
            .trim()
            .toLowerCase();


    results.innerHTML =
        "";


    const matchedStores =
        stores.filter(
            store => {

                if (
                    store.active === false
                ) {

                    return false;
                }


                const storeCountry =
                    store.country ||
                    "JP";


                if (
                    selectedCountry &&
                    storeCountry !==
                    selectedCountry
                ) {

                    return false;
                }


                const displayInfo =
                    typeof getDefaultStoreDisplayInfo ===
                        "function"

                        ? getDefaultStoreDisplayInfo(
                            store
                        )

                        : {
                            name: store.name || "",
                            region: store.region || "",
                            city: store.city || ""
                        };


                const storeRegion =
                    (
                        displayInfo.region ||
                        ""
                    ).trim();


                if (
                    selectedRegion &&
                    storeRegion !==
                    selectedRegion
                ) {

                    return false;
                }


                /*
                 店舗名だけではなく
                 地域・市区町村でも検索可能
                */
                const searchText =
                    [
                        displayInfo.name || "",
                        displayInfo.region || "",
                        displayInfo.city || ""
                    ]
                        .join(" ")
                        .toLowerCase();


                if (
                    keyword &&
                    !searchText.includes(
                        keyword
                    )
                ) {

                    return false;
                }


                return true;

            }
        );


    /*
     最大10店舗表示
    */
    const visibleStores =
        matchedStores.slice(
            0,
            10
        );


    if (
        visibleStores.length === 0
    ) {

        const message =
            document.createElement(
                "p"
            );

        message.textContent =
            t(
                "shopping.store.noResults"
            );

        results.appendChild(
            message
        );

        return;
    }


    for (
        const store of
        visibleStores
    ) {

        const button =
            document.createElement(
                "button"
            );

        button.type =
            "button";

        button.className =
            "btn-sub";


        const displayInfo =
            typeof getDefaultStoreDisplayInfo ===
                "function"

                ? getDefaultStoreDisplayInfo(
                    store
                )

                : {
                    name: store.name || "",
                    region: store.region || "",
                    city: store.city || ""
                };


        const locationText =
            [
                displayInfo.region || "",
                displayInfo.city || ""
            ]
                .filter(
                    value =>
                        value !== ""
                )
                .join(" ");


        button.textContent =
            "";


        const storeNameSpan =
            createStoreNameDisplay(
                store.id
            );


        if (locationText) {

            storeNameSpan.textContent +=
                "　" +
                locationText;

        }


        button.appendChild(
            storeNameSpan
        );


        button.classList.add(
            "shopping-store-candidate"
        );


        button.addEventListener(
            "click",
            () => {

                const text =
                    "🏬 " +
                    getStoreDisplayName(
                        store.id
                    );


                const wasConfirmMode =
                    shoppingStoreNameConfirmMode;


                handleShoppingStoreCandidateClick(
                    store.id,
                    text
                );


                /*
                 通常の店舗選択をした時だけ
                 検索パネルを閉じる
                */
                if (
                    !wasConfirmMode &&
                    !shoppingStoreNameLongPressTriggered
                ) {

                    const searchPanel =
                        document.getElementById(
                            "shoppingStoreSearchPanel"
                        );


                    if (searchPanel) {

                        searchPanel.hidden =
                            true;

                    }

                }

            }
        );


        results.appendChild(
            button
        );

    }


    /*
     10店舗を超えた場合
    */
    if (
        matchedStores.length > 10
    ) {

        const message =
            document.createElement(
                "small"
            );

        message.textContent =
            t(
                "shopping.store.tooMany"
            );

        results.appendChild(
            message
        );

    }

}

/* ==========================================
   購入予定 商品詳細
   表示言語更新
   ========================================== */

function updateShoppingDetailScreenLanguage() {

    const screen =
        document.getElementById(
            "shoppingDetailScreen"
        );

    if (!screen) {
        return;
    }


    const setText = (
        selector,
        key
    ) => {

        const element =
            screen.querySelector(
                selector
            );

        if (element) {

            element.textContent =
                t(key);

        }

    };


    const setLabel = (
        forId,
        key
    ) => {

        const label =
            screen.querySelector(
                `label[for="${forId}"]`
            );

        if (label) {

            label.textContent =
                t(key);

        }

    };


    /*
     ヘッダー
    */

    setText(
        "#btnBackShoppingDetail",
        "shopping.detail.back"
    );

    setText(
        "header h2",
        "shopping.detail.title"
    );


    /*
     商品基本情報
    */

    setText(
        "#shoppingDetailUnregistered",
        "shopping.detail.unregistered"
    );


    /*
     購入予定数量
     strongは残す
    */

    const quantity =
        document.getElementById(
            "shoppingDetailQuantity"
        );

    if (
        quantity &&
        quantity.parentElement
    ) {

        const parent =
            quantity.parentElement;

        const value =
            quantity.textContent;

        parent.innerHTML =
            t(
                "shopping.detail.quantity"
            ) +
            ' <strong id="shoppingDetailQuantity">' +
            value +
            "</strong>";

    }


    /*
     商品コード
    */

    setLabel(
        "txtShoppingDetailJanCode",
        "shopping.detail.code"
    );

    const janInput =
        document.getElementById(
            "txtShoppingDetailJanCode"
        );

    if (janInput) {

        janInput.placeholder =
            t(
                "shopping.detail.codePlaceholder"
            );

    }

    setText(
        "#btnSaveShoppingDetailJan",
        "shopping.detail.saveCode"
    );


    /*
     前回・最安・平均
    */

    const summary =
        document.getElementById(
            "shoppingDetailPriceSummary"
        );

    if (summary) {

        const blocks =
            summary.querySelectorAll(
                ":scope > div"
            );

        const summaryKeys = [
            "shopping.detail.latest",
            "shopping.detail.lowest",
            "shopping.detail.average"
        ];

        blocks.forEach(
            (block, index) => {

                const smalls =
                    block.querySelectorAll(
                        "small"
                    );

                if (
                    smalls[0] &&
                    summaryKeys[index]
                ) {

                    smalls[0].textContent =
                        t(
                            summaryKeys[index]
                        );

                }

                if (smalls[1]) {

                    smalls[1].textContent =
                        t(
                            "shopping.detail.taxIncludedShort"
                        );

                }

            }
        );

    }

    setText(
        "#shoppingDetailNoHistory",
        "shopping.detail.noHistory"
    );


    /*
     今日の店頭情報
    */

    const storeArea =
        document.getElementById(
            "shoppingDetailStoreArea"
        );

    if (storeArea) {

        const title =
            storeArea.querySelector(
                "h3"
            );

        if (title) {

            title.textContent =
                t(
                    "shopping.store.title"
                );

        }

    }


    /*
     販売店
    */

    const firstStoreLabel =
        storeArea
            ? storeArea.querySelector(
                "label:not([for])"
            )
            : null;

    if (firstStoreLabel) {

        firstStoreLabel.textContent =
            t(
                "shopping.store.store"
            );

    }


    const previousArea =
        document.getElementById(
            "shoppingDetailPreviousStoreQuickArea"
        );

    if (previousArea) {

        const small =
            previousArea.querySelector(
                "small"
            );

        if (small) {

            small.textContent =
                t(
                    "shopping.store.previous"
                );

        }

    }


    const recentArea =
        document.getElementById(
            "shoppingDetailRecentStoresArea"
        );

    if (recentArea) {

        const small =
            recentArea.querySelector(
                "small"
            );

        if (small) {

            small.textContent =
                t(
                    "shopping.store.recent"
                );

        }

    }


    setText(
        "#btnToggleShoppingStoreSearch",
        "shopping.store.search"
    );


    setLabel(
        "cmbShoppingStoreSearchCountry",
        "shopping.store.country"
    );


    /*
     店舗検索
     国・地域の選択肢
    */
    const shoppingCountry =
        document.getElementById(
            "cmbShoppingStoreSearchCountry"
        );

    if (shoppingCountry) {

        const countryOptionMap = {

            "": "shopping.store.allCountries",
            JP: "country.JP",
            US: "country.US",
            CA: "country.CA",
            AU: "country.AU",
            KR: "country.KR",
            CN: "country.CN",
            TW: "country.TW",
            OTHER: "country.OTHER"

        };

        const countryFlagMap = {

            JP: "🇯🇵 ",
            US: "🇺🇸 ",
            CA: "🇨🇦 ",
            AU: "🇦🇺 ",
            KR: "🇰🇷 ",
            CN: "🇨🇳 ",
            TW: "🇹🇼 ",
            OTHER: ""

        };


        for (
            const option of
            shoppingCountry.options
        ) {

            const key =
                countryOptionMap[
                option.value
                ];

            if (!key) {

                continue;

            }

            option.textContent =
                (
                    countryFlagMap[
                    option.value
                    ] || ""
                ) +
                t(key);

        }

    }


    setLabel(
        "cmbShoppingStoreSearchRegion",
        "shopping.store.region"
    );

    setLabel(
        "txtShoppingStoreSearchName",
        "shopping.store.name"
    );


    const searchName =
        document.getElementById(
            "txtShoppingStoreSearchName"
        );

    if (searchName) {

        searchName.placeholder =
            t(
                "shopping.store.searchPlaceholder"
            );

    }


    setLabel(
        "cmbShoppingDetailStore",
        "shopping.store.allStores"
    );

    setLabel(
        "cmbShoppingAvailability",
        "shopping.store.availability"
    );


    /*
     販売状況
    */

    const availability =
        document.getElementById(
            "cmbShoppingAvailability"
        );

    if (availability) {

        const optionMap = {

            available:
                "shopping.store.available",

            soldout:
                "shopping.store.soldout",

            notavailable:
                "shopping.store.notavailable",

            unknown:
                "shopping.store.unknown"

        };

        for (
            const option of
            availability.options
        ) {

            if (
                optionMap[
                option.value
                ]
            ) {

                option.textContent =
                    t(
                        optionMap[
                        option.value
                        ]
                    );

            }

        }

    }


    /*
     価格
    */

    setLabel(
        "txtShoppingStorePriceTaxExcluded",
        "shopping.store.taxExcluded"
    );

    setText(
        "#btnShoppingTaxCalc",
        "shopping.store.priceCalc"
    );

    setLabel(
        "cmbShoppingPriceType",
        "shopping.store.priceType"
    );


    /*
     価格の種類
    */
    const shoppingPriceType =
        document.getElementById(
            "cmbShoppingPriceType"
        );

    if (shoppingPriceType) {

        const optionMap = {

            taxExcluded:
                "shopping.store.priceTypeExcluded",

            taxIncluded:
                "shopping.store.priceTypeIncluded"

        };

        for (
            const option of
            shoppingPriceType.options
        ) {

            if (
                optionMap[
                option.value
                ]
            ) {

                option.textContent =
                    t(
                        optionMap[
                        option.value
                        ]
                    );

            }

        }

    }


    setLabel(
        "cmbShoppingTaxRate",
        "shopping.store.taxRate"
    );

    setLabel(
        "txtShoppingCustomTaxRate",
        "shopping.store.customTaxRate"
    );

    setLabel(
        "cmbShoppingRoundingMode",
        "shopping.store.rounding"
    );


    /*
     端数処理
    */
    const shoppingRounding =
        document.getElementById(
            "cmbShoppingRoundingMode"
        );

    if (shoppingRounding) {

        const optionMap = {

            round:
                "shopping.store.round",

            floor:
                "shopping.store.floor",

            ceil:
                "shopping.store.ceil"

        };

        for (
            const option of
            shoppingRounding.options
        ) {

            if (
                optionMap[
                option.value
                ]
            ) {

                option.textContent =
                    t(
                        optionMap[
                        option.value
                        ]
                    );

            }

        }

    }


    setLabel(
        "cmbShoppingDiscountType",
        "shopping.store.discount"
    );


    /*
     値引き
    */
    const shoppingDiscount =
        document.getElementById(
            "cmbShoppingDiscountType"
        );

    if (shoppingDiscount) {

        const optionMap = {

            none:
                "shopping.store.discountNone",

            percent:
                "shopping.store.discountPercent"

        };

        for (
            const option of
            shoppingDiscount.options
        ) {

            if (
                optionMap[
                option.value
                ]
            ) {

                option.textContent =
                    t(
                        optionMap[
                        option.value
                        ]
                    );

            }

        }

    }


    setLabel(
        "txtShoppingDiscountValue",
        "shopping.store.discountValue"
    );

    setText(
        "#btnUseShoppingTaxPrice",
        "shopping.store.usePrice"
    );

    setText(
        "#btnCloseShoppingTaxCalc",
        "shopping.store.close"
    );

    setLabel(
        "txtShoppingStorePriceTaxIncluded",
        "shopping.store.taxIncluded"
    );


    const includedInput =
        document.getElementById(
            "txtShoppingStorePriceTaxIncluded"
        );

    if (includedInput) {

        includedInput.placeholder =
            t(
                "shopping.store.taxIncludedKnown"
            );

    }


    setText(
        "#btnSaveShoppingStoreCheck",
        "shopping.store.save"
    );


    /*
     購入状態
    */

    setText(
        "#btnShoppingMarkPurchased",
        "shopping.detail.purchased"
    );

    setText(
        "#btnShoppingMarkHold",
        "shopping.detail.hold"
    );

    setText(
        "#btnShoppingReturnPending",
        "shopping.detail.returnPending"
    );

    setText(
        "#btnDeleteShoppingItem",
        "shopping.detail.delete"
    );


    const deleteGuide =
        screen.querySelector(
            ".shopping-delete-guide"
        );

    if (deleteGuide) {

        deleteGuide.textContent =
            t(
                "shopping.detail.deleteGuide"
            );

    }

}

/* ==========================================
   購入予定 商品詳細を開く
   ========================================== */

function openShoppingItemDetail(
    shoppingItemId
) {

    const item =
        shoppingItems.find(

            item =>
                item.id ===
                shoppingItemId

        );


    if (!item) {

        return;

    }


    shoppingDetailItemId =
        shoppingItemId;

    /*
     商品詳細画面の表示言語更新
    */

    updateShoppingDetailScreenLanguage();

    /*
     ==========================================
     今日の店頭情報
     価格計算を初期状態へ戻す
     ==========================================
    */

    const shoppingTaxCalc =
        document.getElementById(
            "shoppingTaxCalc"
        );

    const shoppingPriceType =
        document.getElementById(
            "cmbShoppingPriceType"
        );

    const shoppingTaxRate =
        document.getElementById(
            "cmbShoppingTaxRate"
        );

    const shoppingCustomTaxRate =
        document.getElementById(
            "txtShoppingCustomTaxRate"
        );

    const shoppingRoundingMode =
        document.getElementById(
            "cmbShoppingRoundingMode"
        );

    const shoppingDiscountType =
        document.getElementById(
            "cmbShoppingDiscountType"
        );

    const shoppingDiscountValue =
        document.getElementById(
            "txtShoppingDiscountValue"
        );

    const shoppingTaxResult =
        document.getElementById(
            "shoppingTaxResult"
        );


    /*
     価格計算パネルは閉じた状態
    */

    if (shoppingTaxCalc) {

        shoppingTaxCalc.hidden =
            true;

    }


    /*
     表示価格
     税抜へ戻す
    */

    if (shoppingPriceType) {

        shoppingPriceType.value =
            "taxExcluded";

    }


    /*
     日本の標準税率
     10％へ戻す
    */

    if (shoppingTaxRate) {

        shoppingTaxRate.value =
            "10";

    }


    /*
     海外用税率を空にする
    */

    if (shoppingCustomTaxRate) {

        shoppingCustomTaxRate.value =
            "";

    }


    /*
     端数処理
     四捨五入へ戻す
    */

    if (shoppingRoundingMode) {

        shoppingRoundingMode.value =
            "round";

    }


    /*
     値引きなしへ戻す
    */

    if (shoppingDiscountType) {

        shoppingDiscountType.value =
            "none";

    }


    /*
     値引き額・割引率
     0へ戻す
    */

    if (shoppingDiscountValue) {

        shoppingDiscountValue.value =
            "0";

    }


    /*
     前回の計算結果を消す
    */

    if (shoppingTaxResult) {

        shoppingTaxResult.textContent =
            "";

    }


    const product =
        products.find(

            product =>
                product.id ===
                item.productId

        );


    const productName =

        product
            ? product.name
            : (
                item.productName ||
                item.name ||
                t(
                    "shopping.productNameUnset"
                )
            );

    const isUnregistered =
        !product &&
        item.needsRegistration ===
        true;


    /*
     商品名
    */

    const nameElement =
        document.getElementById(
            "shoppingDetailProductName"
        );

    if (nameElement) {

        nameElement.textContent =
            productName;

    }

    const unregisteredElement =
        document.getElementById(
            "shoppingDetailUnregistered"
        );


    if (unregisteredElement) {

        unregisteredElement.hidden =
            !isUnregistered;

    }

    const janArea =
        document.getElementById(
            "shoppingDetailJanArea"
        );

    const janInput =
        document.getElementById(
            "txtShoppingDetailJanCode"
        );


    if (janArea) {

        janArea.hidden =
            !isUnregistered;

    }


    if (
        janInput &&
        isUnregistered
    ) {

        janInput.value =
            item.janCode ||
            "";

    }

    /*
     数量
    */

    const quantityElement =
        document.getElementById(
            "shoppingDetailQuantity"
        );

    if (quantityElement) {

        quantityElement.textContent =
            item.quantity || 1;

    }


    /*
     前回購入店
    */

    let previousStoreText =
        t(
            "shopping.detail.previousStoreNone"
        );


    if (product) {

        const latestHistory =
            getLatestPriceHistory(
                product.id
            );


        if (latestHistory) {

            previousStoreText =
                t(
                    "shopping.detail.previousStore"
                ).replace(
                    "{store}",
                    getStoreDisplayName(
                        latestHistory.storeId
                    )
                );

        }

    }


    const previousStoreElement =
        document.getElementById(
            "shoppingDetailPreviousStore"
        );


    if (previousStoreElement) {

        previousStoreElement.textContent =
            previousStoreText;

    }

    /*
     購入価格情報
    */

    const latestPriceElement =
        document.getElementById(
            "shoppingDetailLatestPrice"
        );

    const lowestPriceElement =
        document.getElementById(
            "shoppingDetailLowestPrice"
        );

    const averagePriceElement =
        document.getElementById(
            "shoppingDetailAveragePrice"
        );

    const noHistoryElement =
        document.getElementById(
            "shoppingDetailNoHistory"
        );

    const productHistories =
        product
            ? getProductPriceHistory(
                product.id
            )
            : [];


    if (noHistoryElement) {

        noHistoryElement.hidden =
            productHistories.length > 0;

    }

    if (product) {

        /*
         最新の購入履歴を基準に
         表示通貨を決定
    
         currency がない古い履歴は
         JPYとして扱う
        */

        const latestHistory =
            getLatestPriceHistory(
                product.id
            );

        const productCurrency =
            latestHistory
                ? latestHistory.currency ||
                "JPY"
                : getCurrencyByCountry(
                    getCocartlyBaseCountry()
                );

        const currencySymbols = {
            JPY: "¥",
            USD: "$",
            CAD: "C$",
            AUD: "A$",
            KRW: "₩",
            CNY: "¥",
            TWD: "NT$"
        };

        const productCurrencySymbol =
            currencySymbols[
            productCurrency
            ] || "";


        const latestPrice =
            getLatestPrice(
                product.id,
                productCurrency
            );

        const lowestPrice =
            getLowestPrice(
                product.id,
                productCurrency
            );

        const averagePrice =
            getAveragePrice(
                product.id,
                productCurrency
            );


        if (latestPriceElement) {

            latestPriceElement.textContent =
                latestPrice !== null &&
                    latestPrice !== undefined
                    ? productCurrencySymbol +
                    latestPrice
                    : "-";

        }


        if (lowestPriceElement) {

            lowestPriceElement.textContent =
                lowestPrice !== null &&
                    lowestPrice !== undefined
                    ? productCurrencySymbol +
                    lowestPrice
                    : "-";

        }


        if (averagePriceElement) {

            averagePriceElement.textContent =
                averagePrice !== null &&
                    averagePrice !== undefined
                    ? productCurrencySymbol +
                    averagePrice
                    : "-";

        }

    } else {

        if (latestPriceElement) {

            latestPriceElement.textContent =
                "-";

        }

        if (lowestPriceElement) {

            lowestPriceElement.textContent =
                "-";

        }

        if (averagePriceElement) {

            averagePriceElement.textContent =
                "-";

        }

    }

    /*
     状態に応じてボタンを切り替える
    */

    updateShoppingDetailButtons(
        item
    );


    displayShoppingDetailStoreOptions();

    displayShoppingStoreChecks(
        item
    );


    /*
     詳細画面表示
    */

    hideAllScreens();


    const detailScreen =
        document.getElementById(
            "shoppingDetailScreen"
        );


    if (detailScreen) {

        detailScreen.hidden =
            false;

    }


    window.scrollTo({

        top: 0,

        behavior: "smooth"

    });

}

/* ==========================================
   購入予定 未登録商品の商品コード保存
   ========================================== */

function saveShoppingDetailJanCode() {

    if (!shoppingDetailItemId) {

        return;

    }


    const item =
        shoppingItems.find(

            item =>
                item.id ===
                shoppingDetailItemId

        );


    if (!item) {

        return;

    }


    const input =
        document.getElementById(
            "txtShoppingDetailJanCode"
        );


    if (!input) {

        return;

    }


    item.janCode =
        input.value.trim();


    item.updatedAt =
        new Date().toISOString();


    saveShoppingItems();


    const message =
        document.getElementById(
            "shoppingDetailMessage"
        );


    if (message) {

        message.textContent =
            item.janCode !== ""
                ? t(
                    "shopping.detail.codeSaved"
                )
                : t(
                    "shopping.detail.codeSavedEmpty"
                );

    }

}

/* ==========================================
   購入予定 詳細ボタン表示
   ========================================== */

function updateShoppingDetailButtons(
    item
) {

    const purchasedButton =
        document.getElementById(
            "btnShoppingMarkPurchased"
        );

    const holdButton =
        document.getElementById(
            "btnShoppingMarkHold"
        );

    const pendingButton =
        document.getElementById(
            "btnShoppingReturnPending"
        );


    if (purchasedButton) {

        purchasedButton.hidden =
            item.status ===
            "purchased";

    }


    if (holdButton) {

        holdButton.hidden =
            item.status ===
            "hold";

    }


    if (pendingButton) {

        pendingButton.hidden =
            item.status ===
            "pending";

    }

}

/* ==========================================
   購入予定から完全削除
   ========================================== */

function permanentlyDeleteShoppingItem(
    shoppingItemId
) {

    const itemIndex =
        shoppingItems.findIndex(

            item =>
                item.id ===
                shoppingItemId

        );


    if (itemIndex === -1) {

        return false;

    }


    const confirmed =
        confirm(

            t(
                "shopping.detail.deleteConfirm"
            )

        );


    if (!confirmed) {

        return false;

    }


    /*
     shoppingItemsの対象1件だけ削除

     products
     priceHistory
     は一切変更しない
    */

    shoppingItems.splice(
        itemIndex,
        1
    );


    saveShoppingItems();


    shoppingDetailItemId =
        null;


    return true;

}

/* ==========================================
   今日の店頭情報を保存
   ========================================== */

function saveShoppingStoreCheck() {

    if (!shoppingDetailItemId) {

        return;

    }


    const shoppingItem =
        shoppingItems.find(

            item =>
                item.id ===
                shoppingDetailItemId

        );


    if (!shoppingItem) {

        return;

    }


    const storeSelect =
        document.getElementById(
            "cmbShoppingDetailStore"
        );

    const availabilitySelect =
        document.getElementById(
            "cmbShoppingAvailability"
        );

    const taxExcludedInput =
        document.getElementById(
            "txtShoppingStorePriceTaxExcluded"
        );

    const taxIncludedInput =
        document.getElementById(
            "txtShoppingStorePriceTaxIncluded"
        );


    const taxRateSelect =
        document.getElementById(
            "cmbShoppingTaxRate"
        );

    const customTaxRateInput =
        document.getElementById(
            "txtShoppingCustomTaxRate"
        );

    const roundingModeSelect =
        document.getElementById(
            "cmbShoppingRoundingMode"
        );

    const discountTypeSelect =
        document.getElementById(
            "cmbShoppingDiscountType"
        );

    const discountValueInput =
        document.getElementById(
            "txtShoppingDiscountValue"
        );


    const message =
        document.getElementById(
            "shoppingStoreCheckMessage"
        );


    const storeId =
        storeSelect
            ? storeSelect.value
            : "";

    const availability =
        availabilitySelect
            ? availabilitySelect.value
            : "unknown";

    const taxExcludedPrice =
        taxExcludedInput
            ? taxExcludedInput.value.trim()
            : "";

    const taxIncludedPrice =
        taxIncludedInput
            ? taxIncludedInput.value.trim()
            : "";


    /*
     選択した販売店の国を取得
    */

    const selectedStore =
        storeId
            ? findStore(storeId)
            : null;

    const shoppingCountry =
        selectedStore
            ? selectedStore.country || "JP"
            : "JP";


    /*
     日本は選択税率、
     海外は入力税率を使用
    */

    const taxRate =
        shoppingCountry === "JP"
            ? (
                taxRateSelect
                    ? taxRateSelect.value
                    : ""
            )
            : (
                customTaxRateInput
                    ? customTaxRateInput.value.trim()
                    : ""
            );


    /*
     端数処理を取得
    */

    const roundingMode =
        roundingModeSelect
            ? roundingModeSelect.value
            : "round";

    /*
     値引き情報を取得
    */
    const discountType =
        discountTypeSelect
            ? discountTypeSelect.value
            : "none";

    const discountValue =
        discountValueInput
            ? Number(
                discountValueInput.value || 0
            )
            : 0;


    if (storeId === "") {

        if (message) {

            message.textContent =
                t(
                    "shopping.store.select"
                );

        }

        return;

    }


    /*
     商品はproductIdで参照
     未登録商品だけshoppingItemIdを保持
    */

    const storeCheck = {

        id:
            createStoreCheckId(),

        productId:
            shoppingItem.productId ||
            null,

        shoppingItemId:
            shoppingItem.productId
                ? null
                : shoppingItem.id,

        storeId:
            storeId,

        availability:
            availability,

        taxExcludedPrice:
            taxExcludedPrice === ""
                ? null
                : Number(
                    taxExcludedPrice
                ),

        taxIncludedPrice:
            taxIncludedPrice === ""
                ? null
                : Number(
                    taxIncludedPrice
                ),

        taxRate:
            taxRate === ""
                ? null
                : Number(
                    taxRate
                ),

        roundingMode:
            roundingMode || "round",

        discountType:
            discountType || "none",

        discountValue:
            discountType === "none"
                ? 0
                : discountValue,

        checkedAt:
            new Date().toISOString()

    };


    storeChecks.push(
        storeCheck
    );


    saveStoreChecks();


    if (message) {

        message.textContent =
            t(
                "shopping.store.saved"
            );

    }


    if (taxExcludedInput) {

        taxExcludedInput.value =
            "";

    }


    if (taxIncludedInput) {

        taxIncludedInput.value =
            "";

    }


    displayShoppingStoreChecks(
        shoppingItem
    );

}

/* ==========================================
   購入履歴に使用する店頭情報取得
   ========================================== */

function getShoppingPurchaseStoreCheck(
    shoppingItem
) {

    if (!shoppingItem) {

        return null;

    }


    const checks =
        storeChecks

            .filter(

                check => {

                    /*
                     登録済み商品
                    */

                    if (
                        shoppingItem.productId
                    ) {

                        return (
                            check.productId ===
                            shoppingItem.productId
                        );

                    }


                    /*
                     未登録商品
                    */

                    return (
                        check.shoppingItemId ===
                        shoppingItem.id
                    );

                }

            )

            /*
             販売あり＋税込価格あり
             だけを購入候補にする
            */

            .filter(

                check =>

                    check.availability ===
                    "available" &&

                    check.taxIncludedPrice !==
                    null &&

                    check.taxIncludedPrice !==
                    undefined &&

                    String(
                        check.taxIncludedPrice
                    ).trim() !== "" &&

                    Number.isFinite(
                        Number(
                            check.taxIncludedPrice
                        )
                    )

            )

            /*
             新しい店頭情報を先頭へ
            */

            .sort(

                (a, b) =>

                    new Date(
                        b.checkedAt
                    ) -

                    new Date(
                        a.checkedAt
                    )

            );


    return checks.length > 0
        ? checks[0]
        : null;

}

/* ==========================================
   未登録の購入予定商品を
   正式商品へ登録
   ========================================== */

function registerShoppingItemProduct(
    shoppingItem,
    storeCheck
) {

    if (!shoppingItem) {

        return null;

    }


    /*
     すでに登録済み商品なら
     その商品を返す
    */

    if (shoppingItem.productId) {

        return products.find(

            product =>
                product.id ===
                shoppingItem.productId

        ) || null;

    }


    /*
     商品コードがある場合、
     すでに同じ商品コードの商品が
     登録されていないか確認
    */

    if (shoppingItem.janCode) {

        const existingProduct =
            products.find(

                product =>
                    product.active !== false &&
                    product.janCode ===
                    shoppingItem.janCode

            );


        if (existingProduct) {

            return existingProduct;

        }

    }


    /*
     新しい正式商品を作成
    */

    const product =
        createProduct({

            name:
                shoppingItem.productName ||
                t(
                    "shopping.productNameUnset"
                ),

            makerId:
                "",

            categoryId:
                "",

            janCode:
                shoppingItem.janCode ||
                "",

            volume:
                "",

            unit:
                "",

            storeId:
                storeCheck
                    ? storeCheck.storeId
                    : "",

            shared:
                false

        });


    /*
     商品登録画面でも
     今回価格が確認できるよう保持
    */

    product.price =
        storeCheck &&
            storeCheck.taxIncludedPrice !== null
            ? Number(
                storeCheck.taxIncludedPrice
            )
            : null;


    product.favorite =
        false;


    product.updatedAt =
        new Date().toISOString();


    addProduct(
        product
    );


    return product;

}

/* ==========================================
   店頭確認履歴 表示
   ========================================== */

function displayShoppingStoreChecks(
    shoppingItem
) {

    const list =
        document.getElementById(
            "shoppingStoreCheckList"
        );

    if (!list) {

        return;

    }


    list.innerHTML =
        "";

    const getCheckCurrency =
        check => {

            const store =
                check.storeId
                    ? findStore(
                        check.storeId
                    )
                    : null;

            const country =
                store
                    ? store.country || "JP"
                    : "JP";

            return getCurrencyByCountry(
                country
            );

        };

    const checks =
        storeChecks

            .filter(

                check => {

                    if (
                        shoppingItem.productId
                    ) {

                        return (
                            check.productId ===
                            shoppingItem.productId
                        );

                    }


                    return (
                        check.shoppingItemId ===
                        shoppingItem.id
                    );

                }

            )

            .sort(

                (a, b) => {

                    /*
                     販売あり＋価格ありを優先
                    */

                    const aAvailable =
                        a.availability ===
                        "available" &&
                        (
                            a.taxIncludedPrice !== null ||
                            a.taxExcludedPrice !== null
                        );


                    const bAvailable =
                        b.availability ===
                        "available" &&
                        (
                            b.taxIncludedPrice !== null ||
                            b.taxExcludedPrice !== null
                        );


                    if (
                        aAvailable &&
                        !bAvailable
                    ) {

                        return -1;

                    }


                    if (
                        !aAvailable &&
                        bAvailable
                    ) {

                        return 1;

                    }


                    /*
                     両方とも比較できる場合は
                     税込価格を優先して安い順
                    */

                    if (
                        aAvailable &&
                        bAvailable
                    ) {

                        const aPrice =
                            a.taxIncludedPrice !== null
                                ? Number(
                                    a.taxIncludedPrice
                                )
                                : Number(
                                    a.taxExcludedPrice
                                );


                        const bPrice =
                            b.taxIncludedPrice !== null
                                ? Number(
                                    b.taxIncludedPrice
                                )
                                : Number(
                                    b.taxExcludedPrice
                                );


                        const aCurrency =
                            getCheckCurrency(a);

                        const bCurrency =
                            getCheckCurrency(b);


                        if (
                            aCurrency === bCurrency &&
                            aPrice !== bPrice
                        ) {

                            return (
                                aPrice -
                                bPrice
                            );

                        }

                    }


                    /*
                     同条件なら新しい確認を先頭
                    */

                    return (

                        new Date(
                            b.checkedAt
                        ) -

                        new Date(
                            a.checkedAt
                        )

                    );

                }

            );

    if (checks.length === 0) {

        list.innerHTML = `

            <p class="shopping-detail-guide">

                ${t(
            "shopping.store.noChecks"
        )}

            </p>

        `;

        return;

    }

    /*
     比較可能な最安価格を取得
    */

    const comparableChecks =
        checks.filter(

            check =>

                check.availability ===
                "available" &&

                (
                    check.taxIncludedPrice !== null ||
                    check.taxExcludedPrice !== null
                )

        );


    let cheapestCheckId =
        null;


    /*
     比較対象がすべて同じ通貨の場合だけ
     最安として扱う
    */

    const comparableCurrencies =
        new Set(

            comparableChecks.map(

                check => {

                    const store =
                        check.storeId
                            ? findStore(
                                check.storeId
                            )
                            : null;

                    const country =
                        store
                            ? store.country ||
                            "JP"
                            : "JP";

                    return getCurrencyByCountry(
                        country
                    );

                }

            )

        );


    if (
        comparableChecks.length > 0 &&
        comparableCurrencies.size === 1
    ) {

        cheapestCheckId =
            comparableChecks[0].id;

    }

    for (const check of checks) {

        /*
         店頭確認した販売店から
         通貨を取得
        */

        const checkStore =
            check.storeId
                ? findStore(
                    check.storeId
                )
                : null;

        const checkCountry =
            checkStore
                ? checkStore.country || "JP"
                : "JP";

        const checkCurrency =
            getCurrencyByCountry(
                checkCountry
            );

        const currencySymbols = {
            JPY: "¥",
            USD: "$",
            CAD: "C$",
            AUD: "A$",
            KRW: "₩",
            CNY: "¥",
            TWD: "NT$"
        };

        const checkCurrencySymbol =
            currencySymbols[
            checkCurrency
            ] || "¥";


        let availabilityText =
            t(
                "shopping.store.unknown"
            );


        if (
            check.availability ===
            "available"
        ) {

            availabilityText =
                t(
                    "shopping.store.available"
                );

        } else if (
            check.availability ===
            "soldout"
        ) {

            availabilityText =
                t(
                    "shopping.store.soldout"
                );

        } else if (
            check.availability ===
            "notavailable"
        ) {

            availabilityText =
                t(
                    "shopping.store.notavailable"
                );

        }


        const checkedDate =
            new Date(
                check.checkedAt
            );


        const dateText =

            checkedDate.getFullYear() +

            "/" +

            String(
                checkedDate.getMonth() + 1
            ).padStart(2, "0") +

            "/" +

            String(
                checkedDate.getDate()
            ).padStart(2, "0");


        const card =
            document.createElement(
                "div"
            );


        card.className =
            "shopping-store-check-card";


        card.innerHTML = `

            ${check.id ===
                cheapestCheckId
                ? `
                        <div class="shopping-store-cheapest">
                            ${t(
                    "shopping.store.cheapest"
                )}
                        </div>
                    `
                : ""
            }


            <strong>

                ${getStoreDisplayName(
                check.storeId
            )}

            </strong>


            <p>
                ${availabilityText}
            </p>


            ${check.taxExcludedPrice !== null
                ? `
                      <p>
                          ${t(
                    "shopping.store.taxExcludedShort"
                )}
                          ${checkCurrencySymbol}${check.taxExcludedPrice}
                      </p>
                   `
                : ""
            }


            ${check.taxIncludedPrice !== null
                ? `
                      <p>
                          ${t(
                    "shopping.detail.taxIncludedShort"
                )}
                          ${checkCurrencySymbol}${check.taxIncludedPrice}
                      </p>
                   `
                : ""
            }


            <small>
                ${t(
                "shopping.store.checked"
            ).replace(
                "{date}",
                dateText
            )}
            </small>

        `;


        list.appendChild(
            card
        );

    }

}

/* ==========================================
   購入完了した予定を削除
   ========================================== */

function completePendingShoppingItem() {

    if (!pendingShoppingItemId) {

        return;

    }

    removeShoppingItem(
        pendingShoppingItemId
    );

    pendingShoppingItemId =
        null;

}

/* ==========================================
   価格計算
   税込 / 税抜
   ％OFF / 円引き
   ========================================== */

function calculateFinalPrice(
    basePrice,
    priceType,
    taxRate,
    discountType,
    discountValue,
    currency = "JPY",
    roundingMode = "round"
) {

    const originalPrice =
        Number(basePrice);

    const rate =
        Number(taxRate);

    const discount =
        Number(discountValue || 0);


    if (
        !Number.isFinite(originalPrice) ||
        originalPrice < 0
    ) {

        return null;

    }


    let discountedPrice =
        originalPrice;


    /*
     値引き
    */

    if (
        discountType ===
        "percent"
    ) {

        if (
            discount < 0 ||
            discount > 100
        ) {

            return null;

        }

        discountedPrice =
            originalPrice *
            (1 - discount / 100);

    } else if (
        discountType ===
        "yen"
    ) {

        discountedPrice =
            originalPrice -
            discount;

    }


    /*
     マイナス価格防止
    */

    if (discountedPrice < 0) {

        discountedPrice = 0;

    }


    /*
     税抜表示の場合だけ
     消費税を加える
    */

    let finalPrice =
        discountedPrice;

    if (
        priceType ===
        "taxExcluded"
    ) {

        finalPrice =
            discountedPrice *
            (1 + rate / 100);

    }


    /*
     通貨に応じて
     端数処理する
    */

    const roundingFunction =
        roundingMode === "floor"
            ? Math.floor
            : roundingMode === "ceil"
                ? Math.ceil
                : Math.round;


    if (
        currency === "JPY" ||
        currency === "KRW"
    ) {

        /*
         日本円・韓国ウォン
         1円 / 1ウォン単位
        */

        finalPrice =
            roundingFunction(
                finalPrice
            );

    } else {

        /*
         USD / CAD / AUD /
         CNY / TWD など
         小数第2位単位
        */

        finalPrice =
            roundingFunction(
                finalPrice * 100
            ) / 100;

    }


    return {

        finalPrice:
            finalPrice,

        /*
         値引き後・税加算前の金額
        */

        taxExcludedPrice:
            priceType ===
                "taxExcluded"
                ? discountedPrice
                : null,

        originalPrice:
            originalPrice,

        priceType:
            priceType,

        taxRate:
            priceType ===
                "taxExcluded"
                ? rate
                : null,

        roundingMode:
            roundingMode,

        discountType:
            discountType,

        discountValue:
            discountType ===
                "none"
                ? 0
                : discount

    };

}

/* ==========================================
   ボタン取得
   ========================================== */

function setupButtons() {

    const btnCompleteSetup = document.getElementById("btnCompleteSetup");
    const cmbSetupLanguage = document.getElementById("cmbSetupLanguage");
    const btnCode = document.getElementById("btnCode");
    const btnProduct = document.getElementById("btnProduct");
    const cmbProductFilterCategory = document.getElementById("cmbProductFilterCategory");
    const btnShopping = document.getElementById("btnShopping");

    const btnShoppingHelp = document.getElementById("btnShoppingHelp");
    const btnCloseShoppingHelp = document.getElementById("btnCloseShoppingHelp");
    const btnCloseShoppingHelpBottom = document.getElementById("btnCloseShoppingHelpBottom");

    const btnShoppingPending = document.getElementById("btnShoppingPending");
    const btnShoppingPurchased = document.getElementById("btnShoppingPurchased");
    const btnShoppingHold = document.getElementById("btnShoppingHold");
    const btnShoppingAddView = document.getElementById("btnShoppingAddView");
    const btnFinishShopping = document.getElementById("btnFinishShopping");

    const btnBackShoppingDetail = document.getElementById("btnBackShoppingDetail");
    const btnShoppingMarkPurchased = document.getElementById("btnShoppingMarkPurchased");
    const btnShoppingMarkHold = document.getElementById("btnShoppingMarkHold");
    const btnShoppingReturnPending = document.getElementById("btnShoppingReturnPending");
    const btnDeleteShoppingItem = document.getElementById("btnDeleteShoppingItem");
    const btnSaveShoppingStoreCheck = document.getElementById("btnSaveShoppingStoreCheck");
    const btnSaveShoppingDetailJan = document.getElementById("btnSaveShoppingDetailJan");

    const btnBackShopping = document.getElementById("btnBackShopping");
    const btnAddShopping = document.getElementById("btnAddShopping");
    /*
     購入予定 ＋追加
     商品コード読取ボタン
    */
    const btnScanShoppingJan = document.getElementById("btnScanShoppingJan");
    const btnHistory = document.getElementById("btnHistory");

    const cmbHistoryProductCategory = document.getElementById("cmbHistoryProductCategory");
    const btnHistoryHelp = document.getElementById("btnHistoryHelp");
    const btnCloseHistoryHelp = document.getElementById("btnCloseHistoryHelp");
    const btnCloseHistoryHelpBottom = document.getElementById("btnCloseHistoryHelpBottom");

    const btnHistoryByDate = document.getElementById("btnHistoryByDate");
    const btnHistoryByProduct = document.getElementById("btnHistoryByProduct");
    const btnDeletedPriceHistory = document.getElementById("btnDeletedPriceHistory");
    const btnCheck = document.getElementById("btnCheck");

    const btnNotice = document.getElementById("btnNotice");
    const btnBackNotice = document.getElementById("btnBackNotice");

    const btnBackCheck = document.getElementById("btnBackCheck");
    const btnOutingHelp = document.getElementById("btnOutingHelp");

    const btnCloseOutingHelp = document.getElementById("btnCloseOutingHelp");
    const btnCloseOutingHelpBottom = document.getElementById("btnCloseOutingHelpBottom");

    const btnCreateOuting = document.getElementById("btnCreateOuting");
    const cmbOutingType = document.getElementById("cmbOutingType");

    const btnOutingUpcoming = document.getElementById("btnOutingUpcoming");
    const btnOutingRoutine = document.getElementById("btnOutingRoutine");
    const btnOutingPast = document.getElementById("btnOutingPast");

    const btnBackOutingCreate = document.getElementById("btnBackOutingCreate");
    const btnAddOutingItem = document.getElementById("btnAddOutingItem");
    const btnSaveOuting = document.getElementById("btnSaveOuting");
    const btnBackOutingCheck = document.getElementById("btnBackOutingCheck");
    const btnBackOutingPastDetail = document.getElementById("btnBackOutingPastDetail");

    const btnCompleteOuting = document.getElementById("btnCompleteOuting");

    const btnOutingPending = document.getElementById("btnOutingPending");
    const btnOutingReady = document.getElementById("btnOutingReady");
    const btnOutingHold = document.getElementById("btnOutingHold");
    const btnOutingAdd = document.getElementById("btnOutingAdd");

    const btnSettings = document.getElementById("btnSettings");

    const cmbSettingsLanguage = document.getElementById("cmbSettingsLanguage");
    const cmbSettingsBaseCountry = document.getElementById("cmbSettingsBaseCountry");

    const btnHomeFromProduct = document.getElementById("btnHomeFromProduct");
    const btnSaveProduct = document.getElementById("btnSaveProduct");

    const btnCancelProductEdit = document.getElementById("btnCancelProductEdit");

    const btnProductTaxCalc = document.getElementById("btnProductTaxCalc");
    const btnShoppingTaxCalc = document.getElementById("btnShoppingTaxCalc");
    const btnUseShoppingTaxPrice = document.getElementById("btnUseShoppingTaxPrice");
    const btnCloseShoppingTaxCalc = document.getElementById("btnCloseShoppingTaxCalc");

    const btnUseProductTaxPrice = document.getElementById("btnUseProductTaxPrice");
    const btnCloseProductTaxCalc = document.getElementById("btnCloseProductTaxCalc");

    const btnPurchaseTaxCalc = document.getElementById("btnPurchaseTaxCalc");
    const btnUsePurchaseTaxPrice = document.getElementById("btnUsePurchaseTaxPrice");
    const btnClosePurchaseTaxCalc = document.getElementById("btnClosePurchaseTaxCalc");

    /*
     今回の購入
     商品コード読取ボタン
    */

    const btnScanPriceRecordJan = document.getElementById("btnScanPriceRecordJan");

    const btnVolumeExample = document.getElementById("btnVolumeExample");
    const btnCloseVolumeExample = document.getElementById("btnCloseVolumeExample");

    const btnProductHelp = document.getElementById("btnProductHelp");
    const btnCloseProductHelp = document.getElementById("btnCloseProductHelp");
    const btnCloseProductHelpBottom = document.getElementById("btnCloseProductHelpBottom");

    const btnCloseRegisteredJan = document.getElementById("btnCloseRegisteredJan");
    const btnRegisteredJanPrice = document.getElementById("btnRegisteredJanPrice");
    const btnRegisteredJanSkip = document.getElementById("btnRegisteredJanSkip");

    const btnScrollTop = document.getElementById("btnScrollTop");

    const btnManageCategories = document.getElementById("btnManageCategories");
    const btnManageStores = document.getElementById("btnManageStores");
    const btnManageStoresFromPriceRecord = document.getElementById("btnManageStoresFromPriceRecord");

    const btnDeletedProducts = document.getElementById("btnDeletedProducts");
    const btnBackProduct = document.getElementById("btnBackProduct");
    const btnBackSettings = document.getElementById("btnBackSettings");
    const btnBackHistory = document.getElementById("btnBackHistory");

    const btnCategoryMaster = document.getElementById("btnCategoryMaster");
    const btnStoreMaster = document.getElementById("btnStoreMaster");

    /*
     購入先管理
     国・地域
    */
    const cmbStoreMasterCountry =
        document.getElementById(
            "cmbStoreMasterCountry"
        );

    /*
     設定
     データ / アプリ
    */
    const btnBackupData = document.getElementById("btnBackupData");
    const btnRestoreData = document.getElementById("btnRestoreData");
    const btnAboutCocartly = document.getElementById("btnAboutCocartly");

    const btnBackAboutCocartly = document.getElementById("btnBackAboutCocartly");

    const btnPrivacyPolicy = document.getElementById("btnPrivacyPolicy");
    const btnBackPrivacyPolicy = document.getElementById("btnBackPrivacyPolicy");

    const btnBackCategory = document.getElementById("btnBackCategory");
    const btnBackStore = document.getElementById("btnBackStore");
    const btnSaveCategory = document.getElementById("btnSaveCategory");
    const btnSaveStore = document.getElementById("btnSaveStore");
    const btnBackCode = document.getElementById("btnBackCode");
    const btnFlash = document.getElementById("btnFlash");
    const btnJanInput = document.getElementById("btnJanInput");

    const btnCancelScan = document.getElementById("btnCancelScan");

    /*
     初回設定
     はじめるボタン
    */

    if (btnCompleteSetup) {

        btnCompleteSetup.addEventListener(
            "click",
            completeInitialSetup
        );

    }

    /*
     初回設定
     表示言語変更
    */

    if (cmbSetupLanguage) {

        cmbSetupLanguage.addEventListener(
            "change",
            () => {

                setCocartlyLanguage(
                    cmbSetupLanguage.value
                );

                updateSetupScreenLanguage();

            }
        );

    }

    btnCode.addEventListener("click", openCode);
    btnProduct.addEventListener("click", openProduct);

    if (cmbProductFilterCategory) {
        cmbProductFilterCategory.addEventListener(
            "change",
            () => {
                productFilterCategoryId =
                    cmbProductFilterCategory.value;
                displayProducts();
            }
        );
    }

    btnShopping.addEventListener("click", openShopping);

    /* ==========================================
       購入予定
       使い方
       ========================================== */

    if (btnShoppingHelp) {

        btnShoppingHelp.addEventListener(
            "click",
            () => {

                const help =
                    document.getElementById(
                        "shoppingHelp"
                    );

                if (!help) {
                    return;
                }

                help.hidden =
                    !help.hidden;

            }
        );

    }


    if (btnCloseShoppingHelp) {

        btnCloseShoppingHelp.addEventListener(
            "click",
            () => {

                const help =
                    document.getElementById(
                        "shoppingHelp"
                    );

                if (help) {
                    help.hidden = true;
                }

            }
        );

    }


    if (btnCloseShoppingHelpBottom) {

        btnCloseShoppingHelpBottom.addEventListener(
            "click",
            () => {

                const help =
                    document.getElementById(
                        "shoppingHelp"
                    );

                if (help) {
                    help.hidden = true;
                }

            }
        );

    }

    if (btnBackShopping) {
        btnBackShopping.addEventListener(
            "click",
            openHome
        );
    }

    if (btnAddShopping) {
        btnAddShopping.addEventListener(
            "click",
            addShoppingItem
        );
    }

    /*
     購入予定 ＋追加
     商品コードをカメラで読み取る
    */

    if (btnScanShoppingJan) {

        btnScanShoppingJan.addEventListener(

            "click",

            () => {

                /*
                 購入予定＋追加から
                 商品コード読取を開いたことを記録
                */

                barcodeReadMode =
                    "shoppingAddJan";


                /*
                 戻るボタンを
                 購入予定用表示へ変更
                */

                const btnBackCode =
                    document.getElementById(
                        "btnBackCode"
                    );

                const btnCancelScan =
                    document.getElementById(
                        "btnCancelScan"
                    );

                if (btnBackCode) {

                    btnBackCode.textContent =
                        t("code.backShopping");

                }

                if (btnCancelScan) {

                    btnCancelScan.textContent =
                        t("code.backShopping");

                }


                /*
                 購入予定画面を閉じる
                */

                const shoppingScreen =
                    document.getElementById(
                        "shoppingScreen"
                    );

                if (shoppingScreen) {

                    shoppingScreen.hidden =
                        true;

                }


                /*
                 コード読取画面を表示
                */

                const codeScreen =
                    document.getElementById(
                        "codeScreen"
                    );

                if (codeScreen) {

                    codeScreen.hidden =
                        false;

                }


                /*
                 カメラ開始
                */

                readCode();

            }

        );

    }

    if (btnFinishShopping) {

        btnFinishShopping.addEventListener(
            "click",
            finishShopping
        );

    }

    /*
     購入予定 表示切り替え
    */

    if (btnShoppingPending) {
        btnShoppingPending.addEventListener(
            "click",
            () => {
                changeShoppingView(
                    "pending"
                );
            }
        );
    }

    if (btnShoppingPurchased) {
        btnShoppingPurchased.addEventListener(
            "click",
            () => {
                changeShoppingView(
                    "purchased"
                );
            }
        );
    }


    if (btnShoppingHold) {
        btnShoppingHold.addEventListener(
            "click",
            () => {
                changeShoppingView(
                    "hold"
                );
            }
        );
    }


    if (btnShoppingAddView) {
        btnShoppingAddView.addEventListener(
            "click",
            () => {
                changeShoppingView(
                    "add"
                );
            }
        );
    }

    /*
     購入予定 商品詳細
    */

    if (btnBackShoppingDetail) {

        btnBackShoppingDetail.addEventListener(

            "click",

            openShopping

        );

    }


    if (btnShoppingMarkPurchased) {

        btnShoppingMarkPurchased.addEventListener(

            "click",

            () => {

                if (!shoppingDetailItemId) {

                    return;

                }


                changeShoppingItemStatus(

                    shoppingDetailItemId,

                    "purchased"

                );


                openShopping();

            }

        );

    }


    if (btnShoppingMarkHold) {

        btnShoppingMarkHold.addEventListener(

            "click",

            () => {

                if (!shoppingDetailItemId) {

                    return;

                }


                changeShoppingItemStatus(

                    shoppingDetailItemId,

                    "hold"

                );


                alert(
                    t(
                        "shopping.detail.holdMessage"
                    )
                );


                openShopping();

            }

        );

    }


    if (btnShoppingReturnPending) {

        btnShoppingReturnPending.addEventListener(

            "click",

            () => {

                if (!shoppingDetailItemId) {

                    return;

                }


                changeShoppingItemStatus(

                    shoppingDetailItemId,

                    "pending"

                );


                openShopping();

            }

        );

    }


    if (btnDeleteShoppingItem) {

        btnDeleteShoppingItem.addEventListener(

            "click",

            () => {

                if (!shoppingDetailItemId) {

                    return;

                }


                const deleted =
                    permanentlyDeleteShoppingItem(
                        shoppingDetailItemId
                    );


                if (!deleted) {

                    return;

                }


                openShopping();

            }

        );

    }

    if (btnSaveShoppingStoreCheck) {

        btnSaveShoppingStoreCheck.addEventListener(

            "click",

            saveShoppingStoreCheck

        );

    }

    if (btnSaveShoppingDetailJan) {

        btnSaveShoppingDetailJan.addEventListener(

            "click",

            saveShoppingDetailJanCode

        );

    }

    btnHistory.addEventListener("click", openHistory);

    if (cmbHistoryProductCategory) {
        cmbHistoryProductCategory.addEventListener(
            "change",
            () => {
                historyProductCategoryId =
                    cmbHistoryProductCategory.value;
                displayPriceHistoryByProduct();
            }
        );
    }

    if (btnHistoryHelp) {
        btnHistoryHelp.addEventListener(
            "click",
            () => {
                const help =
                    document.getElementById(
                        "historyHelp"
                    );
                if (!help) {
                    return;
                }
                help.hidden =
                    !help.hidden;
            }
        );
    }

    if (btnCloseHistoryHelp) {
        btnCloseHistoryHelp.addEventListener(
            "click",
            () => {
                const help =
                    document.getElementById(
                        "historyHelp"
                    );
                if (help) {
                    help.hidden = true;
                }
            }
        );
    }

    if (btnCloseHistoryHelpBottom) {
        btnCloseHistoryHelpBottom.addEventListener(
            "click",
            () => {
                const help =
                    document.getElementById(
                        "historyHelp"
                    );
                if (help) {
                    help.hidden = true;
                }
            }
        );
    }

    if (btnHistoryByDate) {
        btnHistoryByDate.addEventListener(
            "click",
            () =>
                setHistoryViewMode(
                    "date"
                )
        );
    }

    if (btnHistoryByProduct) {
        btnHistoryByProduct.addEventListener(
            "click",
            () =>
                setHistoryViewMode(
                    "product"
                )
        );
    }

    if (btnDeletedPriceHistory) {
        btnDeletedPriceHistory.addEventListener(
            "click",
            displayDeletedPriceHistory
        );
    }

    btnCheck.addEventListener("click", openCheck);

    if (btnBackCheck) {
        btnBackCheck.addEventListener(
            "click",
            openHome
        );
    }

    if (btnCreateOuting) {
        btnCreateOuting.addEventListener(
            "click",
            openCreateOuting
        );
    }

    /*
     お出かけタイプ変更
    */

    if (cmbOutingType) {
        cmbOutingType.addEventListener(
            "change",
            updateOutingDateArea
        );
    }

    /*
     お出かけ一覧
     表示切替
    */

    if (btnOutingUpcoming) {
        btnOutingUpcoming.addEventListener(
            "click",
            () => {
                changeOutingListView(
                    "upcoming"
                );
            }
        );
    }

    if (btnOutingRoutine) {
        btnOutingRoutine.addEventListener(
            "click",
            () => {
                changeOutingListView(
                    "routine"
                );
            }
        );
    }

    if (btnOutingPast) {
        btnOutingPast.addEventListener(
            "click",
            () => {
                changeOutingListView(
                    "past"
                );
            }
        );
    }

    /*
     持ち物を追加
    */

    if (btnAddOutingItem) {
        btnAddOutingItem.addEventListener(
            "click",
            addOutingDraftItem
        );
    }

    /*
     お出かけを保存
    */

    if (btnSaveOuting) {
        btnSaveOuting.addEventListener(
            "click",
            saveCurrentOuting
        );
    }

    if (btnBackOutingCreate) {
        btnBackOutingCreate.addEventListener(
            "click",
            openCheck
        );
    }

    if (btnBackOutingCheck) {
        btnBackOutingCheck.addEventListener(
            "click",
            openCheck
        );
    }

    if (btnBackOutingPastDetail) {
        btnBackOutingPastDetail.addEventListener(
            "click",
            () => {
                outingListViewMode =
                    "past";
                openCheck();
            }
        );
    }

    /*
     お出かけ終了
    */

    if (btnCompleteOuting) {
        btnCompleteOuting.addEventListener(
            "click",
            completeCurrentOuting
        );
    }

    /*
     お出かけチェック
     状態切替
    */

    if (btnOutingPending) {
        btnOutingPending.addEventListener(
            "click",
            () => {
                changeOutingView(
                    "pending"
                );
            }
        );
    }

    if (btnOutingReady) {
        btnOutingReady.addEventListener(
            "click",
            () => {
                changeOutingView(
                    "ready"
                );
            }
        );
    }

    if (btnOutingHold) {
        btnOutingHold.addEventListener(
            "click",
            () => {
                changeOutingView(
                    "hold"
                );
            }
        );
    }

    if (btnOutingAdd) {
        btnOutingAdd.addEventListener(
            "click",
            () => {
                changeOutingView(
                    "add"
                );
            }
        );
    }

    /*
     お知らせ
    */

    if (btnNotice) {

        btnNotice.addEventListener(
            "click",
            () => {

                hideAllScreens();

                const noticeScreen =
                    document.getElementById(
                        "noticeScreen"
                    );

                if (noticeScreen) {

                    const backButton =
                        document.getElementById(
                            "btnBackNotice"
                        );

                    if (backButton) {
                        backButton.textContent =
                            t(
                                "notice.backHome"
                            );
                    }

                    const title =
                        noticeScreen.querySelector(
                            ".screen-header h2"
                        );

                    if (title) {
                        title.textContent =
                            t(
                                "notice.title"
                            );
                    }

                    const noticeTitle =
                        noticeScreen.querySelector(
                            ".notice-card h3"
                        );

                    if (noticeTitle) {
                        noticeTitle.textContent =
                            t(
                                "notice.development.title"
                            );
                    }

                    const noticeText =
                        noticeScreen.querySelector(
                            ".notice-card p"
                        );

                    if (noticeText) {
                        noticeText.textContent =
                            t(
                                "notice.development.text"
                            );
                    }

                    noticeScreen.hidden =
                        false;

                }

            }
        );

    }


    /*
     お知らせからホームへ戻る
    */

    if (btnBackNotice) {

        btnBackNotice.addEventListener(
            "click",
            openHome
        );

    }


    btnSettings.addEventListener("click", openSettings);

    /*
     設定
     表示言語変更
    */

    if (cmbSettingsLanguage) {

        cmbSettingsLanguage.addEventListener(
            "change",
            () => {

                /*
                 言語変更前の
                 商品登録ジャンルを保持
                */

                const productCategorySelect =
                    document.getElementById(
                        "cmbCategory"
                    );

                const currentProductCategoryId =
                    productCategorySelect
                        ? productCategorySelect.value
                        : "";


                /*
                 言語変更前の
                 購入予定の商品選択を保持
                */

                const shoppingProductSelect =
                    document.getElementById(
                        "cmbShoppingProduct"
                    );

                const currentShoppingProductId =
                    shoppingProductSelect
                        ? shoppingProductSelect.value
                        : "";


                /*
                 選択した表示言語を保存
                */

                setCocartlyLanguage(
                    cmbSettingsLanguage.value
                );


                /*
                 各画面の固定表示を更新
                */

                updateHomeScreenLanguage();

                updateSettingsScreenLanguage();

                updateProductScreenLanguage();

                updateShoppingScreenLanguage();

                updateHistoryScreenLanguage();

                updateOutingScreenLanguage();

                updateCodeScreenLanguage();

                updateInformationScreenLanguage();


                /*
                 商品登録
                 ジャンル選択肢を更新
                */

                displayCategorySelect();


                /*
                 言語変更前に選択していた
                 商品ジャンルを復元
                */

                if (
                    productCategorySelect &&
                    currentProductCategoryId &&
                    Array.from(
                        productCategorySelect.options
                    ).some(
                        option =>
                            option.value ===
                            currentProductCategoryId
                    )
                ) {

                    productCategorySelect.value =
                        currentProductCategoryId;

                }


                /*
                 登録済み商品
                 ジャンル絞り込みを更新
    
                 productFilterCategoryId は
                 displayProductFilterCategories()
                 内で保持される
                */

                displayProductFilterCategories();


                /*
                 登録済み商品一覧を
                 新しい表示言語で再描画
                */

                displayProducts();


                /*
                 購入予定
                 ジャンルタブを再描画
    
                 shoppingProductFilter は
                 変更しないため、
                 現在の絞り込み状態を維持する
                */

                displayShoppingCategoryTabs();


                /*
                 購入予定の商品候補を
                 新しい表示言語で再描画
                */

                displayShoppingProductOptions();


                /*
                 言語変更前に選択していた
                 購入予定の商品を復元
                */

                if (
                    shoppingProductSelect &&
                    currentShoppingProductId &&
                    Array.from(
                        shoppingProductSelect.options
                    ).some(
                        option =>
                            option.value ===
                            currentShoppingProductId
                    )
                ) {

                    shoppingProductSelect.value =
                        currentShoppingProductId;

                }


                /*
                 今回の購入
                 購入先候補を再描画
    
                 displayPriceRecordStoreOptions()
                 側で現在の購入先を保持する
                */

                displayPriceRecordStoreOptions();

            }
        );

    }


    /*
     設定
     普段買い物する国・地域変更
    */

    if (cmbSettingsBaseCountry) {

        cmbSettingsBaseCountry.addEventListener(
            "change",
            () => {

                setCocartlyBaseCountry(
                    cmbSettingsBaseCountry.value
                );

            }
        );

    }

    if (btnHomeFromProduct) {
        btnHomeFromProduct.addEventListener("click", openHome);
    }

    btnSaveProduct.addEventListener(
        "click",
        saveProduct
    );

    if (btnCancelProductEdit) {

        btnCancelProductEdit.addEventListener(
            "click",
            cancelProductEdit
        );

    }

    if (btnProductTaxCalc) {
        btnProductTaxCalc.addEventListener(
            "click",
            () => {
                const panel =
                    document.getElementById(
                        "productTaxCalc"
                    );
                if (panel) {
                    panel.hidden =
                        !panel.hidden;
                }
            }
        );
    }

    /*
     商品登録
     国・地域による税率入力切り替え
    */

    const productStoreCountry =
        document.getElementById(
            "cmbStoreCountry"
        );

    const productTaxRate =
        document.getElementById(
            "cmbProductTaxRate"
        );

    const productCustomTaxArea =
        document.getElementById(
            "productCustomTaxArea"
        );

    const productCustomTaxRate =
        document.getElementById(
            "txtProductCustomTaxRate"
        );

    const productTaxGuide =
        document.getElementById(
            "productTaxGuide"
        );

    /*
     値引き値の入力例を
     値引き方法・国に合わせる
    */

    function updateProductDiscountPlaceholder() {

        const country =
            productStoreCountry
                ? productStoreCountry.value ||
                getCocartlyBaseCountry()
                : getCocartlyBaseCountry();


        const discountType =
            document.getElementById(
                "cmbProductDiscountType"
            );

        const discountValueInput =
            document.getElementById(
                "txtProductDiscountValue"
            );


        if (
            !discountType ||
            !discountValueInput
        ) {
            return;
        }


        /*
         ％OFFの場合
        */

        if (
            discountType.value ===
            "percent"
        ) {

            discountValueInput.placeholder =
                t(
                    "product.tax.examplePrefix"
                ) +
                "20";

            return;
        }


        /*
         金額引きの場合
        */

        if (
            discountType.value ===
            "yen"
        ) {

            const amountExamples = {

                JP: "20",
                US: "2",
                CA: "2",
                AU: "2",
                KR: "2000",
                CN: "20",
                TW: "50",
                OTHER: "2"

            };


            const amountExample =
                amountExamples[
                country
                ] || "20";


            discountValueInput.placeholder =
                t(
                    "product.tax.examplePrefix"
                ) +
                amountExample;

            return;
        }


        /*
         値引きなし
        */

        discountValueInput.placeholder =
            t(
                "product.tax.examplePrefix"
            ) +
            "20";

    }

    function updateProductTaxInput() {

        const country =
            productStoreCountry
                ? productStoreCountry.value ||
                getCocartlyBaseCountry()
                : getCocartlyBaseCountry();


        /*
         国・地域に応じた
         通貨表示
        */

        const currencyLabels = {

            JP: "円",
            US: "$",
            CA: "C$",
            AU: "A$",
            KR: "₩",
            CN: "¥",
            TW: "NT$"

        };


        const currencyLabel =
            currencyLabels[
            country
            ] || "円";

        /*
         表示価格の入力例を
         国・地域に合わせる
        */

        const displayPriceExamples = {

            JP: "198",
            US: "1.98",
            CA: "1.98",
            AU: "1.98",
            KR: "1980",
            CN: "19.8",
            TW: "198",
            OTHER: "1.98"

        };


        const displayPriceExample =
            displayPriceExamples[
            country
            ] || "198";


        /*
         商品登録
         価格（税込）の入力例
        */

        const productPriceInput =
            document.getElementById(
                "txtPrice"
            );


        if (productPriceInput) {

            productPriceInput.placeholder =
                t(
                    "product.tax.examplePrefix"
                ) +
                displayPriceExample;

        }


        /*
         価格計算
         表示価格の入力例
        */

        const productTaxExcludedInput =
            document.getElementById(
                "txtProductTaxExcluded"
            );


        if (productTaxExcludedInput) {

            productTaxExcludedInput.placeholder =
                t(
                    "product.tax.examplePrefix"
                ) +
                displayPriceExample;

        }

        /*
         値引きの通貨表示を
         国・地域に合わせる
        */

        const productDiscountType =
            document.getElementById(
                "cmbProductDiscountType"
            );

        if (productDiscountType) {

            const amountDiscountOption =
                productDiscountType.querySelector(
                    'option[value="yen"]'
                );

            if (amountDiscountOption) {

                amountDiscountOption.textContent =
                    t(
                        "product.tax.discountAmountLabel"
                    ).replace(
                        "{currency}",
                        currencyLabel
                    );

            }

        }

        updateProductDiscountPlaceholder();

        const productTaxCurrencyLabel =
            document.getElementById(
                "productTaxCurrencyLabel"
            );


        if (productTaxCurrencyLabel) {

            productTaxCurrencyLabel.textContent =
                currencyLabel;

        }

        /*
         海外税率の入力例を
         国・地域に合わせる

         ※ あくまで入力例であり、
           実際の税率を自動設定するものではない
        */

        const customTaxRateExamples = {

            US: "8.25",
            CA: "13",
            AU: "10",
            KR: "10",
            CN: "13",
            TW: "5",
            OTHER: "10"

        };


        if (
            productCustomTaxRate &&
            country !== "JP"
        ) {

            const customTaxRateExample =
                customTaxRateExamples[
                country
                ] || "10";

            productCustomTaxRate.placeholder =
                t(
                    "product.tax.examplePrefix"
                ) +
                customTaxRateExample;

        }

        /*
         日本と海外で
         税率入力方法を切り替える
        */

        if (country === "JP") {

            if (productTaxRate) {
                productTaxRate.hidden = false;
            }

            if (productCustomTaxArea) {
                productCustomTaxArea.hidden = true;
            }

            if (productCustomTaxRate) {
                productCustomTaxRate.value = "";
            }

            if (productTaxGuide) {
                productTaxGuide.textContent =
                    t(
                        "product.tax.jpGuide"
                    );
            }

        } else {

            if (productTaxRate) {
                productTaxRate.hidden = true;
            }

            if (productCustomTaxArea) {
                productCustomTaxArea.hidden = false;
            }

            if (productTaxGuide) {
                productTaxGuide.textContent =
                    t(
                        "product.tax.overseasGuide"
                    );
            }

        }

    }


    if (productStoreCountry) {

        productStoreCountry.addEventListener(
            "change",
            updateProductTaxInput
        );

    }

    /*
     値引き方法を変更したとき
     入力例を更新
    */

    const productDiscountTypeSelect =
        document.getElementById(
            "cmbProductDiscountType"
        );

    if (productDiscountTypeSelect) {

        productDiscountTypeSelect.addEventListener(
            "change",
            updateProductDiscountPlaceholder
        );

    }

    /*
     初期表示
    */

    updateProductTaxInput();

    if (btnUseProductTaxPrice) {

        btnUseProductTaxPrice.addEventListener(

            "click",

            () => {

                const basePrice =
                    document.getElementById(
                        "txtProductTaxExcluded"
                    );

                const priceType =
                    document.getElementById(
                        "cmbProductPriceType"
                    );

                const taxRate =
                    document.getElementById(
                        "cmbProductTaxRate"
                    );

                const customTaxRate =
                    document.getElementById(
                        "txtProductCustomTaxRate"
                    );

                const discountType =
                    document.getElementById(
                        "cmbProductDiscountType"
                    );

                const discountValue =
                    document.getElementById(
                        "txtProductDiscountValue"
                    );

                const result =
                    document.getElementById(
                        "productTaxResult"
                    );

                const priceInput =
                    document.getElementById(
                        "txtPrice"
                    );


                /*
                 商品登録画面で選択されている
                 国・地域から通貨を取得
                */

                const productCountrySelect =
                    document.getElementById(
                        "cmbStoreCountry"
                    );

                const productCountry =
                    productCountrySelect
                        ? productCountrySelect.value ||
                        getCocartlyBaseCountry()
                        : getCocartlyBaseCountry();

                const productCurrency =
                    getCurrencyByCountry(
                        productCountry
                    );


                /*
                 日本は選択された税率、
                 海外は入力された税率を使用
                */

                const selectedTaxRate =
                    productCountry === "JP"
                        ? taxRate.value
                        : customTaxRate.value;


                /*
                 端数処理を取得
                */

                const roundingMode =
                    document.getElementById(
                        "cmbProductRoundingMode"
                    ).value;


                const calculation =
                    calculateFinalPrice(

                        basePrice.value,

                        priceType.value,

                        selectedTaxRate,

                        discountType.value,

                        discountValue.value,

                        productCurrency,

                        roundingMode

                    );


                if (!calculation) {

                    result.textContent =
                        t(
                            "product.tax.inputError"
                        );

                    return;

                }


                /*
                 通貨に応じて
                 自然な金額表示にする
                */

                const formattedPrice =
                    formatCurrencyPrice(
                        calculation.finalPrice,
                        productCurrency
                    );


                result.textContent =
                    t(
                        "product.tax.finalPrice"
                    ).replace(
                        "{price}",
                        formattedPrice
                    );


                priceInput.value =
                    calculation.finalPrice;


                /*
                 履歴保存用に保持
                */

                productPriceCalculation =
                    calculation;

            }

        );

    }


    if (btnCloseProductTaxCalc) {
        btnCloseProductTaxCalc.addEventListener(
            "click",
            () => {
                const panel =
                    document.getElementById(
                        "productTaxCalc"
                    );
                if (panel) {
                    panel.hidden = true;
                }
            }
        );
    }

    /*
     購入予定 商品詳細
     価格計算を開く
    */

    /*
     購入予定 商品詳細
     販売店の国・地域による
     通貨・税率入力切り替え
    */

    const shoppingDetailStore =
        document.getElementById(
            "cmbShoppingDetailStore"
        );

    const shoppingTaxRate =
        document.getElementById(
            "cmbShoppingTaxRate"
        );

    const shoppingCustomTaxArea =
        document.getElementById(
            "shoppingCustomTaxArea"
        );

    const shoppingCustomTaxRate =
        document.getElementById(
            "txtShoppingCustomTaxRate"
        );

    const shoppingTaxGuide =
        document.getElementById(
            "shoppingTaxGuide"
        );


    function updateShoppingTaxInput() {

        const store =
            shoppingDetailStore &&
                shoppingDetailStore.value
                ? findStore(
                    shoppingDetailStore.value
                )
                : null;


        const country =
            store
                ? store.country ||
                getCocartlyBaseCountry()
                : getCocartlyBaseCountry();


        const currencyLabels = {

            JP: "円",
            US: "$",
            CA: "C$",
            AU: "A$",
            KR: "₩",
            CN: "¥",
            TW: "NT$"

        };


        const currencyLabel =
            currencyLabels[
            country
            ] || "円";


        /*
         Price Calculator
         価格入力例を
         購入先の国・通貨に合わせる
        */

        const priceExamples = {

            JP: "398",
            US: "3.99",
            CA: "4.99",
            AU: "5.99",
            KR: "3980",
            CN: "19.90",
            TW: "99",
            OTHER: "9.99"

        };


        const priceExample =
            priceExamples[
            country
            ] ||
            priceExamples.OTHER;


        const taxExcludedInput =
            document.getElementById(
                "txtShoppingStorePriceTaxExcluded"
            );


        if (taxExcludedInput) {

            taxExcludedInput.placeholder =
                t(
                    "shopping.store.examplePrefix"
                ) +
                priceExample;

        }


        if (shoppingCustomTaxRate) {

            shoppingCustomTaxRate.placeholder =
                t(
                    "shopping.store.examplePrefix"
                ) +
                "8.25";

        }


        /*
         値引きの通貨表示を
         購入先の国・通貨に合わせる
        */

        const shoppingDiscountAmountOption =
            document.querySelector(
                '#cmbShoppingDiscountType option[value="yen"]'
            );


        if (shoppingDiscountAmountOption) {

            shoppingDiscountAmountOption.textContent =
                t(
                    "shopping.store.discountAmount"
                ).replace(
                    "{currency}",
                    currencyLabel
                );

        }


        const taxExcludedCurrencyLabel =
            document.getElementById(
                "shoppingTaxExcludedCurrencyLabel"
            );

        const taxIncludedCurrencyLabel =
            document.getElementById(
                "shoppingTaxIncludedCurrencyLabel"
            );


        if (taxExcludedCurrencyLabel) {

            taxExcludedCurrencyLabel.textContent =
                currencyLabel;

        }


        if (taxIncludedCurrencyLabel) {

            taxIncludedCurrencyLabel.textContent =
                currencyLabel;

        }


        /*
         日本と海外で
         税率入力方法を切り替える
        */

        if (country === "JP") {

            if (shoppingTaxRate) {
                shoppingTaxRate.hidden = false;
            }

            if (shoppingCustomTaxArea) {
                shoppingCustomTaxArea.hidden = true;
            }

            if (shoppingCustomTaxRate) {
                shoppingCustomTaxRate.value = "";
            }

            if (shoppingTaxGuide) {
                shoppingTaxGuide.textContent =
                    t("purchase.taxGuide.jp");
            }

        } else {

            if (shoppingTaxRate) {
                shoppingTaxRate.hidden = true;
            }

            if (shoppingCustomTaxArea) {
                shoppingCustomTaxArea.hidden = false;
            }

            if (shoppingTaxGuide) {
                shoppingTaxGuide.textContent =
                    t("purchase.taxGuide.other");
            }

        }

    }


    if (shoppingDetailStore) {

        shoppingDetailStore.addEventListener(
            "change",
            updateShoppingTaxInput
        );

    }


    updateShoppingTaxInput();

    if (btnShoppingTaxCalc) {

        btnShoppingTaxCalc.addEventListener(
            "click",
            () => {

                const panel =
                    document.getElementById(
                        "shoppingTaxCalc"
                    );

                if (panel) {

                    panel.hidden =
                        !panel.hidden;

                }

            }
        );

    }


    /*
     購入予定 商品詳細
     計算した価格を使用
    */

    if (btnUseShoppingTaxPrice) {

        btnUseShoppingTaxPrice.addEventListener(
            "click",
            () => {

                const basePrice =
                    document.getElementById(
                        "txtShoppingStorePriceTaxExcluded"
                    );

                const priceType =
                    document.getElementById(
                        "cmbShoppingPriceType"
                    );

                const taxRate =
                    document.getElementById(
                        "cmbShoppingTaxRate"
                    );

                const customTaxRate =
                    document.getElementById(
                        "txtShoppingCustomTaxRate"
                    );

                const discountType =
                    document.getElementById(
                        "cmbShoppingDiscountType"
                    );

                const discountValue =
                    document.getElementById(
                        "txtShoppingDiscountValue"
                    );

                const result =
                    document.getElementById(
                        "shoppingTaxResult"
                    );

                const taxExcludedInput =
                    document.getElementById(
                        "txtShoppingStorePriceTaxExcluded"
                    );

                const taxIncludedInput =
                    document.getElementById(
                        "txtShoppingStorePriceTaxIncluded"
                    );


                /*
                 選択された販売店から
                 国・地域と通貨を取得
                */

                const selectedStore =
                    shoppingDetailStore &&
                        shoppingDetailStore.value
                        ? findStore(
                            shoppingDetailStore.value
                        )
                        : null;


                const shoppingCountry =
                    selectedStore
                        ? selectedStore.country ||
                        getCocartlyBaseCountry()
                        : getCocartlyBaseCountry();


                const shoppingCurrency =
                    getCurrencyByCountry(
                        shoppingCountry
                    );


                /*
                 日本は選択された税率、
                 海外は入力された税率を使用
                */

                const selectedTaxRate =
                    shoppingCountry === "JP"
                        ? taxRate.value
                        : customTaxRate.value;


                /*
                 端数処理を取得
                */

                const roundingMode =
                    document.getElementById(
                        "cmbShoppingRoundingMode"
                    ).value;


                const calculation =
                    calculateFinalPrice(
                        basePrice.value,
                        priceType.value,
                        selectedTaxRate,
                        discountType.value,
                        discountValue.value,
                        shoppingCurrency,
                        roundingMode
                    );


                if (!calculation) {

                    result.textContent =
                        t(
                            "shopping.store.inputError"
                        );

                    return;

                }


                /*
                 通貨に応じた
                 自然な金額表示
                */

                const formattedPrice =
                    formatCurrencyPrice(
                        calculation.finalPrice,
                        shoppingCurrency
                    );


                result.textContent =
                    t(
                        "shopping.store.finalPrice"
                    ).replace(
                        "{price}",
                        formattedPrice
                    );


                taxIncludedInput.value =
                    calculation.finalPrice;


                if (
                    priceType.value ===
                    "taxExcluded"
                ) {

                    taxExcludedInput.value =
                        basePrice.value;

                }

            }
        );

    }


    /*
     購入予定 商品詳細
     価格計算を閉じる
    */

    if (btnCloseShoppingTaxCalc) {

        btnCloseShoppingTaxCalc.addEventListener(
            "click",
            () => {

                const panel =
                    document.getElementById(
                        "shoppingTaxCalc"
                    );

                if (panel) {

                    panel.hidden =
                        true;

                }

            }
        );

    }

    if (btnPurchaseTaxCalc) {
        btnPurchaseTaxCalc.addEventListener(
            "click",
            () => {
                const panel =
                    document.getElementById(
                        "purchaseTaxCalc"
                    );
                if (panel) {
                    panel.hidden =
                        !panel.hidden;
                }
            }
        );
    }


    if (btnUsePurchaseTaxPrice) {

        btnUsePurchaseTaxPrice.addEventListener(

            "click",

            () => {

                const basePrice =
                    document.getElementById(
                        "txtPurchaseTaxExcluded"
                    );

                const taxRate =
                    document.getElementById(
                        "cmbPurchaseTaxRate"
                    );

                const customTaxRate =
                    document.getElementById(
                        "txtPurchaseCustomTaxRate"
                    );

                const discountType =
                    document.getElementById(
                        "cmbPurchaseDiscountType"
                    );

                const discountValue =
                    document.getElementById(
                        "txtPurchaseDiscountValue"
                    );


                /*
                 今回選択している購入先から
                 国・地域を取得

                 購入先が未選択の場合は
                 商品登録時の購入先を使用
                */

                const product =
                    products.find(
                        product =>
                            product.id ===
                            recordingProductId
                    );

                const purchaseStoreSelect =
                    document.getElementById(
                        "cmbPriceRecordStore"
                    );

                const selectedPurchaseStore =
                    purchaseStoreSelect &&
                        purchaseStoreSelect.value
                        ? findStore(
                            purchaseStoreSelect.value
                        )
                        : null;

                const registeredStore =
                    product &&
                        product.storeId
                        ? findStore(
                            product.storeId
                        )
                        : null;

                const productCountry =
                    selectedPurchaseStore
                        ? selectedPurchaseStore.country ||
                        "JP"
                        : registeredStore
                            ? registeredStore.country ||
                            "JP"
                            : getCocartlyBaseCountry();


                /*
                 日本は選択された税率、
                 海外は入力された税率を使用
                */

                const selectedTaxRate =
                    productCountry === "JP"
                        ? taxRate.value
                        : customTaxRate.value;


                /*
                 国・地域から
                 計算に使用する通貨を取得
                */

                const currency =
                    getCurrencyByCountry(
                        productCountry
                    );


                const result =
                    document.getElementById(
                        "purchaseTaxResult"
                    );

                const priceInput =
                    document.getElementById(
                        "txtPriceRecordPrice"
                    );


                /*
                 端数処理を取得
                */

                const roundingMode =
                    document.getElementById(
                        "cmbPurchaseRoundingMode"
                    ).value;


                const calculation =
                    calculateFinalPrice(

                        basePrice.value,

                        "taxExcluded",

                        selectedTaxRate,

                        discountType
                            ? discountType.value
                            : "none",

                        discountValue
                            ? discountValue.value
                            : 0,

                        currency,

                        roundingMode
                    );


                if (!calculation) {

                    result.textContent =
                        t(
                            "product.tax.inputError"
                        );

                    return;

                }


                const formattedPrice =
                    formatCurrencyPrice(
                        calculation.finalPrice,
                        currency
                    );


                result.textContent =
                    t(
                        "purchase.taxIncludedPrice"
                    ).replace(
                        "{price}",
                        formattedPrice
                    );


                priceInput.value =
                    calculation.finalPrice;


                /*
                 履歴保存用に保持
                */

                purchasePriceCalculation =
                    calculation;

            }

        );

    }


    if (btnClosePurchaseTaxCalc) {
        btnClosePurchaseTaxCalc.addEventListener(
            "click",
            () => {
                const panel =
                    document.getElementById(
                        "purchaseTaxCalc"
                    );
                if (panel) {
                    panel.hidden = true;
                }
            }
        );
    }

    /*
     今回の購入
     商品コードをカメラで読み取る
    */

    if (btnScanPriceRecordJan) {

        btnScanPriceRecordJan.addEventListener(

            "click",

            () => {

                barcodeReadMode =
                    "priceRecordJan";


                /*
                 戻るボタンを
                 今回の購入用表示へ変更
                */

                const btnBackCode =
                    document.getElementById(
                        "btnBackCode"
                    );

                const btnCancelScan =
                    document.getElementById(
                        "btnCancelScan"
                    );

                if (btnBackCode) {

                    btnBackCode.textContent =
                        t(
                            "purchase.backPurchaseScan"
                        );

                }

                if (btnCancelScan) {

                    btnCancelScan.textContent =
                        t(
                            "purchase.backPurchaseScan"
                        );

                }


                /*
                 今回の購入画面を閉じる
                */

                const priceRecordScreen =
                    document.getElementById(
                        "priceRecordScreen"
                    );

                if (priceRecordScreen) {

                    priceRecordScreen.hidden =
                        true;

                }


                /*
                 コード読取画面を表示
                */

                const codeScreen =
                    document.getElementById(
                        "codeScreen"
                    );

                if (codeScreen) {

                    codeScreen.hidden =
                        false;

                }


                readCode();

            }

        );

    }

    /* ==========================================
       商品登録 使い方
       ========================================== */

    if (btnProductHelp) {
        btnProductHelp.addEventListener(
            "click",
            () => {
                const help =
                    document.getElementById(
                        "productHelp"
                    );
                if (!help) {
                    return;
                }
                help.hidden =
                    !help.hidden;
            }
        );
    }


    if (btnCloseProductHelp) {
        btnCloseProductHelp.addEventListener(
            "click",
            () => {
                const help =
                    document.getElementById(
                        "productHelp"
                    );
                if (help) {
                    help.hidden = true;
                }
            }
        );
    }


    if (btnCloseProductHelpBottom) {
        btnCloseProductHelpBottom.addEventListener(
            "click",
            () => {
                const help =
                    document.getElementById(
                        "productHelp"
                    );
                if (help) {
                    help.hidden = true;
                }
            }
        );
    }

    /*
     お出かけチェック
     使い方
    */

    if (btnOutingHelp) {

        btnOutingHelp.addEventListener(
            "click",
            () => {

                const help =
                    document.getElementById(
                        "outingHelp"
                    );


                if (!help) {

                    return;

                }


                help.hidden =
                    !help.hidden;

            }
        );

    }

    /*
     お出かけチェック
     ×で使い方を閉じる
    */

    if (btnCloseOutingHelp) {

        btnCloseOutingHelp.addEventListener(
            "click",
            () => {

                const help =
                    document.getElementById(
                        "outingHelp"
                    );

                if (help) {

                    help.hidden = true;

                }

            }
        );

    }


    /*
     お出かけチェック
     下の「閉じる」で使い方を閉じる
    */

    if (btnCloseOutingHelpBottom) {

        btnCloseOutingHelpBottom.addEventListener(
            "click",
            () => {

                const help =
                    document.getElementById(
                        "outingHelp"
                    );

                if (help) {

                    help.hidden = true;

                }

            }
        );

    }

    if (btnVolumeExample) {
        btnVolumeExample.addEventListener(
            "click",
            () => {
                const example =
                    document.getElementById(
                        "volumeExample"
                    );
                if (!example) {
                    return;
                }
                example.hidden =
                    !example.hidden;
            }
        );
    }

    if (btnCloseVolumeExample) {
        btnCloseVolumeExample.addEventListener(
            "click",
            () => {
                const example =
                    document.getElementById(
                        "volumeExample"
                    );
                if (example) {
                    example.hidden = true;
                }
            }
        );
    }

    /* ==========================================
       登録済み商品コード確認
       ========================================== */

    if (btnCloseRegisteredJan) {
        btnCloseRegisteredJan.addEventListener(
            "click",
            closeRegisteredJanInfo
        );
    }

    if (btnRegisteredJanPrice) {
        btnRegisteredJanPrice.addEventListener(
            "click",
            recordRegisteredJanPrice
        );
    }

    if (btnRegisteredJanSkip) {
        btnRegisteredJanSkip.addEventListener(
            "click",
            skipRegisteredJanPurchase
        );
    }

    /* 先頭へ戻る */

    if (btnScrollTop) {
        btnScrollTop.addEventListener(
            "click",
            () => {
                window.scrollTo({
                    top: 0,
                    behavior: "smooth"
                });
            }
        );
    }

    btnManageCategories.addEventListener(
        "click",
        openCategoryMasterFromProduct
    );

    btnManageStores.addEventListener(
        "click",
        openStoreMasterFromProduct
    );

    if (btnManageStoresFromPriceRecord) {
        btnManageStoresFromPriceRecord.addEventListener(
            "click",
            () => {
                openStoreMasterFromPriceRecord();
            }
        );
    }


    btnDeletedProducts.addEventListener("click", openDeletedProducts);
    btnBackProduct.addEventListener("click", backProduct);
    btnBackSettings.addEventListener("click", openHome);
    btnBackHistory.addEventListener("click", openHome);

    btnCategoryMaster.addEventListener(
        "click",
        openCategoryMaster
    );

    btnStoreMaster.addEventListener(
        "click",
        openStoreMaster
    );


    /*
     購入先管理
     国・地域変更
    */

    if (cmbStoreMasterCountry) {

        cmbStoreMasterCountry.addEventListener(
            "change",
            () => {

                updateStoreLocationPlaceholders();

                displayStores();

            }
        );

    }


    /*
     設定
     データをバックアップ
    */

    if (btnBackupData) {

        btnBackupData.addEventListener(
            "click",
            backupCocartlyData
        );

    }

    /*
     設定
     バックアップから復元
    */

    if (btnRestoreData) {

        btnRestoreData.addEventListener(
            "click",
            restoreCocartlyData
        );

    }

    /*
     設定
     Cocartlyについて
    */

    if (btnAboutCocartly) {

        btnAboutCocartly.addEventListener(
            "click",
            openAboutCocartly
        );

    }


    if (btnBackAboutCocartly) {

        btnBackAboutCocartly.addEventListener(
            "click",
            openSettings
        );

    }

    /*
     設定
     プライバシーポリシー
    */

    if (btnPrivacyPolicy) {

        btnPrivacyPolicy.addEventListener(
            "click",
            openPrivacyPolicy
        );

    }


    if (btnBackPrivacyPolicy) {

        btnBackPrivacyPolicy.addEventListener(
            "click",
            openSettings
        );

    }

    btnBackCategory.addEventListener("click", backFromMasterScreen);
    btnBackStore.addEventListener("click", backFromMasterScreen);

    btnSaveCategory.addEventListener("click", saveCategory);
    btnSaveStore.addEventListener("click", saveStore);
    btnBackCode.addEventListener("click", backHomeFromCode);
    btnCancelScan.addEventListener("click", backHomeFromCode);
    btnFlash.addEventListener("click", toggleFlash);
    btnJanInput.addEventListener("click", openJanInput);
}

/* ==========================================
   先頭へ戻るボタン表示制御
   ========================================== */

function updateScrollTopButton() {

    const button =
        document.getElementById(
            "btnScrollTop"
        );

    if (!button) {

        return;

    }


    /*
     「先頭へ戻る」を使用する画面
    */

    const targetScreens = [

        "productScreen",
        "shoppingScreen",
        "historyScreen",
        "checkScreen"

    ];


    /*
     対象画面のどれかが
     現在表示されているか確認
    */

    const isTargetScreenOpen =
        targetScreens.some(
            (screenId) => {

                const screen =
                    document.getElementById(
                        screenId
                    );

                return (
                    screen &&
                    !screen.hidden
                );

            }
        );


    /*
     対象画面以外では表示しない
    */

    if (!isTargetScreenOpen) {

        button.hidden = true;

        return;

    }


    /*
     対象画面で
     300px以上スクロールしたら表示
    */

    button.hidden =
        window.scrollY < 300;

}


/* ==========================================
   スクロール監視
   ========================================== */

window.addEventListener(

    "scroll",

    updateScrollTopButton,

    {
        passive: true
    }

);

/* ==========================================
   ボタン処理
   ========================================== */

function openBarcode() {

    /*
     ホームからの
     コード読取
    */
    barcodeReadMode =
        "normal";

    /*
     コード読取画面の
     戻るボタンをホーム用に戻す
    */
    const btnBackCode =
        document.getElementById(
            "btnBackCode"
        );

    const btnCancelScan =
        document.getElementById(
            "btnCancelScan"
        );

    if (btnBackCode) {

        btnBackCode.textContent =
            t("code.backHome");

    }

    if (btnCancelScan) {

        btnCancelScan.textContent =
            t("code.backHome");

    }

    changeScreen("バーコード読取");

}

function openProduct() {

    changeScreen("商品登録");

    /*
     商品登録画面の表示言語を更新
    */

    updateProductScreenLanguage();

    /*
     普段買い物する国・地域を
     商品登録画面へ反映
    */

    const cmbStoreCountry =
        document.getElementById(
            "cmbStoreCountry"
        );

    if (cmbStoreCountry) {

        cmbStoreCountry.value =
            getCocartlyBaseCountry();

    }

    /*
     国・地域に合わせて
     税率・通貨表示を更新
    */

    if (cmbStoreCountry) {

        cmbStoreCountry.dispatchEvent(
            new Event("change")
        );

    }

    /*
     選択した国・地域の
     購入先一覧を表示
    */

    populateStoreCombo();

    /*
     populateStoreCombo()で
     購入先一覧を作り直した後に
     表示言語をもう一度反映
    */

    updateProductScreenLanguage();

    /*
     登録済み商品の
     ジャンル絞り込み候補を更新
    */

    displayProductFilterCategories();

    /*
     登録済み商品を表示
    */

    displayProducts();

}

/*
 購入先簡易追加欄を初期化
*/

const storeQuickAdd =
    document.getElementById("storeQuickAdd");

const cmbQuickStoreType =
    document.getElementById("cmbQuickStoreType");

const txtQuickStore =
    document.getElementById("txtQuickStore");

if (storeQuickAdd) {

    storeQuickAdd.hidden = true;

}

if (cmbQuickStoreType) {

    cmbQuickStoreType.selectedIndex = 0;

}

if (txtQuickStore) {

    txtQuickStore.value = "";

}

/* ==========================================
   購入予定画面
   表示言語を更新
   ========================================== */

function updateShoppingScreenLanguage() {

    const shoppingScreen =
        document.getElementById(
            "shoppingScreen"
        );

    if (!shoppingScreen) {

        return;

    }


    /*
     ホームへ
    */

    const backButton =
        document.getElementById(
            "btnBackShopping"
        );

    if (backButton) {

        backButton.textContent =
            t(
                "shopping.backHome"
            );

    }


    /*
     画面タイトル
    */

    const title =
        shoppingScreen.querySelector(
            "header h2"
        );

    if (title) {

        title.textContent =
            t(
                "shopping.title"
            );

    }


    /*
     使い方ボタン
    */

    const helpButton =
        document.getElementById(
            "btnShoppingHelp"
        );

    if (helpButton) {

        helpButton.textContent =
            t(
                "shopping.help"
            );

    }


    /*
     使い方パネル
    */

    const helpPanel =
        document.getElementById(
            "shoppingHelp"
        );

    if (helpPanel) {

        const helpHeader =
            helpPanel.querySelector(
                ".screen-help-panel-header strong"
            );

        if (helpHeader) {

            helpHeader.textContent =
                t(
                    "shopping.help.title"
                );

        }


        const helpSections =
            helpPanel.querySelectorAll(
                ".help-section"
            );


        /*
         この機能でできること
        */

        if (helpSections[0]) {

            const heading =
                helpSections[0]
                    .querySelector(
                        "h3"
                    );

            if (heading) {

                heading.textContent =
                    t(
                        "shopping.help.featureTitle"
                    );

            }


            const paragraphs =
                helpSections[0]
                    .querySelectorAll(
                        "p"
                    );

            const featureKeys = [

                "shopping.help.feature1",
                "shopping.help.feature2",
                "shopping.help.feature3",
                "shopping.help.feature4"

            ];


            paragraphs.forEach(
                (paragraph, index) => {

                    if (
                        featureKeys[index]
                    ) {

                        paragraph.textContent =
                            t(
                                featureKeys[index]
                            );

                    }

                }
            );

        }


        /*
         こんな時に便利です
        */

        if (helpSections[1]) {

            const heading =
                helpSections[1]
                    .querySelector(
                        "h3"
                    );

            if (heading) {

                heading.textContent =
                    t(
                        "shopping.help.usefulTitle"
                    );

            }


            const paragraphs =
                helpSections[1]
                    .querySelectorAll(
                        "p"
                    );

            const usefulKeys = [

                "shopping.help.useful1",
                "shopping.help.useful2",
                "shopping.help.useful3",
                "shopping.help.useful4"

            ];


            paragraphs.forEach(
                (paragraph, index) => {

                    if (
                        usefulKeys[index]
                    ) {

                        paragraph.textContent =
                            t(
                                usefulKeys[index]
                            );

                    }

                }
            );

        }


        /*
         使い方
        */

        if (helpSections[2]) {

            const heading =
                helpSections[2]
                    .querySelector(
                        "h3"
                    );

            if (heading) {

                heading.textContent =
                    t(
                        "shopping.help.howToTitle"
                    );

            }


            const steps =
                helpSections[2]
                    .querySelectorAll(
                        "li"
                    );

            const stepKeys = [

                "shopping.help.step1",
                "shopping.help.step2",
                "shopping.help.step3",
                "shopping.help.step4",
                "shopping.help.step5",
                "shopping.help.step6",
                "shopping.help.step7",
                "shopping.help.step8",
                "shopping.help.step9"

            ];


            steps.forEach(
                (step, index) => {

                    if (
                        stepKeys[index]
                    ) {

                        step.textContent =
                            t(
                                stepKeys[index]
                            );

                    }

                }
            );

        }

    }


    /*
     使い方 下側の閉じる
    */

    const closeHelpBottom =
        document.getElementById(
            "btnCloseShoppingHelpBottom"
        );

    if (closeHelpBottom) {

        closeHelpBottom.textContent =
            t(
                "shopping.help.close"
            );

    }


    /*
     購入前
     購入済
     保留
     
     件数のspanは残す
    */

    const viewButtons = [

        {
            id: "btnShoppingPending",
            key: "shopping.pending"
        },

        {
            id: "btnShoppingPurchased",
            key: "shopping.purchased"
        },

        {
            id: "btnShoppingHold",
            key: "shopping.hold"
        }

    ];


    for (
        const item of
        viewButtons
    ) {

        const button =
            document.getElementById(
                item.id
            );

        if (!button) {

            continue;

        }

        const count =
            button.querySelector(
                "span"
            );

        if (count) {

            const countValue =
                count.textContent;

            button.innerHTML =
                t(
                    item.key
                ) +
                ' <span id="' +
                count.id +
                '">' +
                countValue +
                "</span>";

        }

    }


    /*
     ＋追加
    */

    const addViewButton =
        document.getElementById(
            "btnShoppingAddView"
        );

    if (addViewButton) {

        addViewButton.textContent =
            t(
                "shopping.addTab"
            );

    }


    /*
     残り ○ 商品
    */

    const remainingSummary =
        document.getElementById(
            "shoppingRemainingSummary"
        );

    const remainingCount =
        document.getElementById(
            "shoppingRemainingCount"
        );

    if (
        remainingSummary &&
        remainingCount
    ) {

        const currentCount =
            remainingCount.textContent;

        const remainingSummaryKey =
            Number(currentCount) === 1
                ? "shopping.remainingSummaryOne"
                : "shopping.remainingSummary";

        remainingSummary.innerHTML =

            t(
                remainingSummaryKey
            ).replace(
                "{count}",
                '<strong id="shoppingRemainingCount">' +
                currentCount +
                "</strong>"
            );

    }


    /*
     購入予定に追加
    */

    const addTitle =
        shoppingScreen.querySelector(
            "#shoppingAddArea h3"
        );

    if (addTitle) {

        addTitle.textContent =
            t(
                "shopping.add.title"
            );

    }


    /*
     固定ジャンルボタン
     displayShoppingCategoryTabs()でも
     作り直されるため初期HTML用
    */

    const favoriteButton =
        document.getElementById(
            "btnShoppingFavorite"
        );

    if (favoriteButton) {

        favoriteButton.textContent =
            t(
                "shopping.favorite"
            );

    }


    const allButton =
        document.getElementById(
            "btnShoppingCategoryAll"
        );

    if (allButton) {

        allButton.textContent =
            t(
                "shopping.all"
            );

    }


    /*
     登録済み商品
    */

    const registeredProductLabel =
        shoppingScreen.querySelector(
            'label[for="cmbShoppingProduct"]'
        );

    if (registeredProductLabel) {

        registeredProductLabel.textContent =
            t(
                "shopping.registeredProduct"
            );

    }


    /*
     または
    */

    const addOr =
        shoppingScreen.querySelector(
            ".shopping-add-or"
        );

    if (addOr) {

        addOr.textContent =
            t(
                "shopping.or"
            );

    }


    /*
     商品名
    */

    const productNameLabel =
        shoppingScreen.querySelector(
            'label[for="txtShoppingNewProductName"]'
        );

    if (productNameLabel) {

        productNameLabel.textContent =
            t(
                "shopping.productName"
            );

    }


    const productNameInput =
        document.getElementById(
            "txtShoppingNewProductName"
        );

    if (productNameInput) {

        productNameInput.placeholder =
            t(
                "shopping.productNamePlaceholder"
            );

    }


    /*
     商品コード
    */

    const codeLabel =
        shoppingScreen.querySelector(
            'label[for="txtShoppingJanCode"]'
        );

    if (codeLabel) {

        codeLabel.textContent =
            t(
                "shopping.code"
            );

    }


    const codeInput =
        document.getElementById(
            "txtShoppingJanCode"
        );

    if (codeInput) {

        codeInput.placeholder =
            t(
                "shopping.codePlaceholder"
            );

    }


    const scanButton =
        document.getElementById(
            "btnScanShoppingJan"
        );

    if (scanButton) {

        scanButton.textContent =
            t(
                "shopping.scanCode"
            );

    }


    const codeGuide =
        shoppingScreen.querySelector(
            ".purchase-jan-guide"
        );

    if (codeGuide) {

        codeGuide.textContent =
            t(
                "shopping.codeGuide"
            );

    }


    /*
     予定数量
    */

    const quantityLabel =
        shoppingScreen.querySelector(
            'label[for="txtShoppingQuantity"]'
        );

    if (quantityLabel) {

        quantityLabel.textContent =
            t(
                "shopping.quantity"
            );

    }


    /*
     購入予定に追加
    */

    const addButton =
        document.getElementById(
            "btnAddShopping"
        );

    if (addButton) {

        addButton.textContent =
            t(
                "shopping.add"
            );

    }


    /*
     買い物終了
    */

    const finishButton =
        document.getElementById(
            "btnFinishShopping"
        );

    if (finishButton) {

        finishButton.textContent =
            t(
                "shopping.finish"
            );

    }

}

function openShopping() {

    hideAllScreens();

    const shoppingScreen =
        document.getElementById(
            "shoppingScreen"
        );

    if (!shoppingScreen) {

        return;

    }

    shoppingScreen.hidden =
        false;

    /*
     購入予定画面の表示言語を更新
    */

    updateShoppingScreenLanguage();

    /*
     購入予定画面を開いた時は
     「購入前」を表示
    */

    shoppingViewMode =
        "pending";


    shoppingProductFilter =
        "all";


    displayShoppingCategoryTabs();

    displayShoppingProductOptions();


    updateShoppingViewButtons();

    updateShoppingCounts();

    displayShoppingItemsByCurrentView();


    window.scrollTo({

        top: 0,

        behavior: "smooth"

    });

}

function openHistory() {

    changeScreen(
        "履歴・価格比較"
    );

    updateHistoryScreenLanguage();

    displayPriceHistory();

}

function openCheck() {

    hideAllScreens();


    /*
     お出かけチェック
     固定表示を更新
    */

    updateOutingScreenLanguage();

    /*
     保存済みお出かけを表示
    */

    displayOutings();


    const checkScreen =
        document.getElementById(
            "checkScreen"
        );


    if (checkScreen) {

        checkScreen.hidden =
            false;

    }


    window.scrollTo({

        top: 0,

        behavior: "smooth"

    });

}

/* ==========================================
   お出かけチェック
   日本語表示更新
   ========================================== */

function updateOutingScreenLanguage() {

    const checkScreen =
        document.getElementById(
            "checkScreen"
        );

    if (checkScreen) {

        const backButton =
            document.getElementById(
                "btnBackCheck"
            );

        if (backButton) {
            backButton.textContent =
                t("outing.backHome");
        }


        const title =
            checkScreen.querySelector(
                ".screen-title-row h2"
            );

        if (title) {
            title.textContent =
                t("outing.title");
        }


        const helpButton =
            document.getElementById(
                "btnOutingHelp"
            );

        if (helpButton) {
            helpButton.textContent =
                t("outing.help");
        }


        const introTitle =
            checkScreen.querySelector(
                ".check-intro-card h3"
            );

        if (introTitle) {
            introTitle.textContent =
                t("outing.intro.title");
        }


        const introGuide =
            checkScreen.querySelector(
                ".check-intro-card p"
            );

        if (introGuide) {
            introGuide.textContent =
                t("outing.intro.guide");
        }


        const helpPanel =
            document.getElementById(
                "outingHelp"
            );

        if (helpPanel) {

            const helpTitle =
                helpPanel.querySelector(
                    ".screen-help-panel-header strong"
                );

            if (helpTitle) {
                helpTitle.textContent =
                    t("outing.help.title");
            }


            const sections =
                helpPanel.querySelectorAll(
                    ".help-section"
                );

            /*
             この機能でできること
            */
            if (sections[0]) {

                const heading =
                    sections[0].querySelector(
                        "h3"
                    );

                const paragraphs =
                    sections[0].querySelectorAll(
                        "p"
                    );

                if (heading) {
                    heading.textContent =
                        t(
                            "outing.help.whatCanDo"
                        );
                }

                const keys = [
                    "outing.help.description1",
                    "outing.help.description2",
                    "outing.help.description3",
                    "outing.help.description4"
                ];

                paragraphs.forEach(
                    (paragraph, index) => {

                        if (keys[index]) {
                            paragraph.textContent =
                                t(keys[index]);
                        }

                    }
                );
            }


            /*
             こんな時に便利です
            */
            if (sections[1]) {

                const heading =
                    sections[1].querySelector(
                        "h3"
                    );

                const paragraphs =
                    sections[1].querySelectorAll(
                        "p"
                    );

                if (heading) {
                    heading.textContent =
                        t(
                            "outing.help.usefulTitle"
                        );
                }

                const keys = [
                    "outing.help.useful1",
                    "outing.help.useful2",
                    "outing.help.useful3",
                    "outing.help.useful4"
                ];

                paragraphs.forEach(
                    (paragraph, index) => {

                        if (keys[index]) {
                            paragraph.textContent =
                                t(keys[index]);
                        }

                    }
                );
            }


            /*
             使い方
            */
            if (sections[2]) {

                const heading =
                    sections[2].querySelector(
                        "h3"
                    );

                const items =
                    sections[2].querySelectorAll(
                        "li"
                    );

                if (heading) {
                    heading.textContent =
                        t(
                            "outing.help.howTo"
                        );
                }

                const keys = [
                    "outing.help.step1",
                    "outing.help.step2",
                    "outing.help.step3",
                    "outing.help.step4",
                    "outing.help.step5",
                    "outing.help.step6",
                    "outing.help.step7"
                ];

                items.forEach(
                    (item, index) => {

                        if (keys[index]) {
                            item.textContent =
                                t(keys[index]);
                        }

                    }
                );
            }


            /*
             お出かけの管理
            */
            if (sections[3]) {

                const heading =
                    sections[3].querySelector(
                        "h3"
                    );

                const paragraphs =
                    sections[3].querySelectorAll(
                        "p"
                    );

                if (heading) {
                    heading.textContent =
                        t(
                            "outing.help.management"
                        );
                }

                const keys = [
                    "outing.help.managementUpcoming",
                    "outing.help.managementRoutine",
                    "outing.help.managementPast"
                ];

                paragraphs.forEach(
                    (paragraph, index) => {

                        if (keys[index]) {
                            paragraph.textContent =
                                t(keys[index]);
                        }

                    }
                );
            }
        }


        const createButton =
            document.getElementById(
                "btnCreateOuting"
            );

        if (createButton) {
            createButton.textContent =
                t("outing.create");
        }


        const upcomingButton =
            document.getElementById(
                "btnOutingUpcoming"
            );

        if (upcomingButton) {

            const count =
                document.getElementById(
                    "outingUpcomingCount"
                );

            upcomingButton.childNodes[0]
                .textContent =
                t("outing.tab.upcoming") +
                " ";

            if (count) {
                count.textContent =
                    count.textContent;
            }
        }


        const routineButton =
            document.getElementById(
                "btnOutingRoutine"
            );

        if (routineButton) {

            routineButton.childNodes[0]
                .textContent =
                t("outing.tab.routine") +
                " ";
        }


        const pastButton =
            document.getElementById(
                "btnOutingPast"
            );

        if (pastButton) {

            pastButton.childNodes[0]
                .textContent =
                t("outing.tab.past") +
                " ";
        }
    }


    /*
     お出かけ作成画面
    */

    const createScreen =
        document.getElementById(
            "outingCreateScreen"
        );

    if (createScreen) {

        const backButton =
            document.getElementById(
                "btnBackOutingCreate"
            );

        if (backButton) {
            backButton.textContent =
                t("outing.create.back");
        }


        const title =
            createScreen.querySelector(
                ".screen-header h2"
            );

        if (title) {
            title.textContent =
                t("outing.create.title");
        }


        const card =
            createScreen.querySelector(
                ".outing-create-card"
            );

        if (card) {

            const title =
                card.querySelector("h3");

            if (title) {
                title.textContent =
                    t(
                        "outing.create.question"
                    );
            }

            const guide =
                card.querySelector("h3 + p");

            if (guide) {
                guide.textContent =
                    t(
                        "outing.create.guide"
                    );
            }
        }


        const nameLabel =
            createScreen.querySelector(
                'label[for="txtOutingName"]'
            );

        if (nameLabel) {
            nameLabel.textContent =
                t("outing.create.name");
        }


        const typeLabel =
            createScreen.querySelector(
                'label[for="cmbOutingType"]'
            );

        if (typeLabel) {
            typeLabel.textContent =
                t("outing.create.type");
        }


        const typeSelect =
            document.getElementById(
                "cmbOutingType"
            );

        if (typeSelect) {

            const scheduledOption =
                typeSelect.querySelector(
                    'option[value="scheduled"]'
                );

            const routineOption =
                typeSelect.querySelector(
                    'option[value="routine"]'
                );

            if (scheduledOption) {
                scheduledOption.textContent =
                    t(
                        "outing.create.typeScheduled"
                    );
            }

            if (routineOption) {
                routineOption.textContent =
                    t(
                        "outing.create.typeRoutine"
                    );
            }

        }


        const typeGuide =
            createScreen.querySelector(
                ".outing-type-guide"
            );

        if (typeGuide) {
            typeGuide.textContent =
                t(
                    "outing.create.typeGuide"
                );
        }


        const dateLabel =
            createScreen.querySelector(
                'label[for="txtOutingDate"]'
            );

        if (dateLabel) {
            dateLabel.textContent =
                t("outing.create.date");
        }


        const itemLabel =
            createScreen.querySelector(
                'label[for="txtOutingItemName"]'
            );

        if (itemLabel) {
            itemLabel.textContent =
                t("outing.create.item");
        }

        const nameInput =
            document.getElementById(
                "txtOutingName"
            );

        if (nameInput) {
            nameInput.placeholder =
                t(
                    "outing.create.namePlaceholder"
                );
        }


        const itemInput =
            document.getElementById(
                "txtOutingItemName"
            );

        if (itemInput) {
            itemInput.placeholder =
                t(
                    "outing.create.itemPlaceholder"
                );
        }


        const addButton =
            document.getElementById(
                "btnAddOutingItem"
            );

        if (addButton) {
            addButton.textContent =
                t(
                    "outing.create.addItem"
                );
        }


        const saveButton =
            document.getElementById(
                "btnSaveOuting"
            );

        if (saveButton) {
            saveButton.textContent =
                t("outing.create.save");
        }
    }


    /*
     お出かけ実行画面
    */

    const outingCheckScreen =
        document.getElementById(
            "outingCheckScreen"
        );

    if (outingCheckScreen) {

        const backButton =
            document.getElementById(
                "btnBackOutingCheck"
            );

        if (backButton) {
            backButton.textContent =
                t("outing.check.back");
        }


        const title =
            document.getElementById(
                "outingCheckTitle"
            );

        if (title) {
            title.textContent =
                t("outing.check.title");
        }


        const pending =
            document.getElementById(
                "btnOutingPending"
            );

        if (pending) {
            pending.childNodes[0]
                .textContent =
                t("outing.check.pending") +
                " ";
        }


        const ready =
            document.getElementById(
                "btnOutingReady"
            );

        if (ready) {
            ready.childNodes[0]
                .textContent =
                t("outing.check.ready") +
                " ";
        }


        const hold =
            document.getElementById(
                "btnOutingHold"
            );

        if (hold) {
            hold.childNodes[0]
                .textContent =
                t("outing.check.hold") +
                " ";
        }


        const add =
            document.getElementById(
                "btnOutingAdd"
            );

        if (add) {
            add.textContent =
                t("outing.check.add");
        }


        const complete =
            document.getElementById(
                "btnCompleteOuting"
            );

        if (complete) {
            complete.textContent =
                t(
                    "outing.check.complete"
                );
        }


        const completeGuide =
            outingCheckScreen.querySelector(
                ".outing-complete-guide"
            );

        if (completeGuide) {
            completeGuide.textContent =
                t(
                    "outing.check.completeGuide"
                );
        }
    }


    /*
     過去のお出かけ詳細
    */

    const pastScreen =
        document.getElementById(
            "outingPastDetailScreen"
        );

    if (pastScreen) {

        const backButton =
            document.getElementById(
                "btnBackOutingPastDetail"
            );

        if (backButton) {
            backButton.textContent =
                t("outing.past.back");
        }


        const title =
            document.getElementById(
                "outingPastDetailTitle"
            );

        if (title) {
            title.textContent =
                t("outing.past.title");
        }


        const itemsTitle =
            pastScreen.querySelector(
                ".outing-past-detail-card h3"
            );

        if (itemsTitle) {
            itemsTitle.textContent =
                t("outing.past.items");
        }
    }
}

function openCreateOuting() {

    /*
     新規作成なので
     作成途中データをリセット
    */

    outingDraftItems =
        [];


    const nameInput =
        document.getElementById(
            "txtOutingName"
        );

    const itemInput =
        document.getElementById(
            "txtOutingItemName"
        );

    const typeSelect =
        document.getElementById(
            "cmbOutingType"
        );

    const dateInput =
        document.getElementById(
            "txtOutingDate"
        );

    const message =
        document.getElementById(
            "outingCreateMessage"
        );


    if (nameInput) {

        nameInput.value =
            "";

    }


    if (itemInput) {

        itemInput.value =
            "";

    }

    if (typeSelect) {

        typeSelect.value =
            "scheduled";

    }


    if (dateInput) {

        dateInput.value =
            "";

    }


    updateOutingDateArea();

    if (message) {

        message.textContent =
            "";

    }


    displayOutingDraftItems();


    /*
     お出かけ作成画面の
     表示言語を更新
    */

    updateOutingScreenLanguage();


    hideAllScreens();


    const outingCreateScreen =
        document.getElementById(
            "outingCreateScreen"
        );


    if (outingCreateScreen) {

        outingCreateScreen.hidden =
            false;

    }


    window.scrollTo({

        top: 0,

        behavior: "smooth"

    });

}

/* ==========================================
   お出かけ作成
   日付欄切替
   ========================================== */

function updateOutingDateArea() {

    const typeSelect =
        document.getElementById(
            "cmbOutingType"
        );

    const dateArea =
        document.getElementById(
            "outingDateArea"
        );


    if (
        !typeSelect ||
        !dateArea
    ) {

        return;

    }


    /*
     1回のお出かけだけ
     日付を表示
    */

    dateArea.hidden =
        typeSelect.value ===
        "routine";

}

/* ==========================================
   お出かけ作成
   持ち物追加
   ========================================== */

function addOutingDraftItem() {

    const itemInput =
        document.getElementById(
            "txtOutingItemName"
        );

    const message =
        document.getElementById(
            "outingCreateMessage"
        );


    if (!itemInput) {

        return;

    }


    const itemName =
        itemInput.value.trim();


    /*
     未入力
    */

    if (itemName === "") {

        if (message) {

            message.textContent =
                t(
                    "outing.message.itemRequired"
                );
        }

        itemInput.focus();
        return;
    }


    /*
     同じ持ち物の重複防止
    */

    const duplicate =
        outingDraftItems.some(

            item =>
                item.name ===
                itemName

        );


    if (duplicate) {

        if (message) {

            message.textContent =
                t(
                    "outing.message.itemDuplicate"
                );
        }

        itemInput.focus();
        return;
    }


    /*
     作成中リストへ追加
    */

    outingDraftItems.push({

        id:
            "outing-item-" +
            Date.now() +
            "-" +
            Math.random()
                .toString(36)
                .slice(2, 7),

        name:
            itemName

    });


    /*
     入力欄を空にする
    */

    itemInput.value =
        "";


    if (message) {

        message.textContent =
            t(
                "outing.message.itemAdded"
            );
    }


    displayOutingDraftItems();


    itemInput.focus();

}

/* ==========================================
   お出かけ
   保存データ読込
   ========================================== */

function loadOutings() {

    const saved =
        localStorage.getItem(
            "outings"
        );


    if (!saved) {

        outings =
            [];

        return;

    }


    try {

        const parsed =
            JSON.parse(
                saved
            );


        outings =
            Array.isArray(parsed)
                ? parsed
                : [];


        /*
         古いお出かけデータへの対応
        
         statusがない場合は
         「予定」として扱う
        */

        for (const outing of outings) {

            if (!outing.status) {

                outing.status =
                    "upcoming";

            }


            /*
             typeがない旧データも
             1回のお出かけとして扱う
            */

            if (!outing.type) {

                outing.type =
                    "scheduled";

            }


            /*
             itemsがない場合の安全対策
            */

            if (
                !Array.isArray(
                    outing.items
                )
            ) {

                outing.items =
                    [];

            }


            /*
 古い持ち物にも
 statusを追加
*/

            for (
                const item of
                outing.items
            ) {

                if (!item.status) {

                    item.status =
                        "pending";

                }


                /*
                 古い持ち物にも
                 家にある物 / 購入する物
                 の区分を追加
            
                 home
                 → 家にある物
            
                 purchase
                 → 購入する物
                */

                if (!item.itemType) {

                    item.itemType =

                        item.productId ||
                            item.shoppingItemId ||
                            item.readySource === "shopping"

                            ? "purchase"
                            : "home";

                }

            }

        }


        /*
         変換した旧データを
         新しい形式で保存
        */

        saveOutings();

    } catch (error) {

        console.error(
            "お出かけデータの読込に失敗しました。",
            error
        );

        outings =
            [];

    }

}

/* ==========================================
   お出かけ
   保存
   ========================================== */

function saveOutings() {

    localStorage.setItem(

        "outings",

        JSON.stringify(
            outings
        )

    );

}

/* ==========================================
   お出かけ作成
   持ち物一覧表示
   ========================================== */

function displayOutingDraftItems() {

    const list =
        document.getElementById(
            "outingItemList"
        );


    if (!list) {

        return;

    }


    list.innerHTML =
        "";


    /*
     まだ持ち物がない
    */

    if (
        outingDraftItems.length === 0
    ) {

        list.innerHTML = `

    <p class="check-empty-message">

        ${t("outing.create.noItems")}

    </p>

`;

        return;

    }


    /*
     持ち物を表示
    */

    for (
        const item of
        outingDraftItems
    ) {

        const row =
            document.createElement(
                "div"
            );

        row.className =
            "outing-draft-item";


        row.innerHTML = `

            <span class="outing-draft-item-name">

                ${item.name}

            </span>


            <button
    type="button"
    class="outing-draft-delete-button"
    onclick="deleteOutingDraftItem('${item.id}')">

    ${t("outing.create.deleteItem")}

</button>

        `;


        list.appendChild(
            row
        );

    }

}

/* ==========================================
   お出かけ作成
   持ち物削除
   ========================================== */

function deleteOutingDraftItem(
    itemId
) {

    outingDraftItems =
        outingDraftItems.filter(

            item =>
                item.id !==
                itemId

        );


    displayOutingDraftItems();

}

/* ==========================================
   お出かけ
   新規保存
   ========================================== */

function saveCurrentOuting() {

    const nameInput =
        document.getElementById(
            "txtOutingName"
        );

    const typeSelect =
        document.getElementById(
            "cmbOutingType"
        );

    const dateInput =
        document.getElementById(
            "txtOutingDate"
        );

    const message =
        document.getElementById(
            "outingCreateMessage"
        );


    if (!nameInput) {

        return;

    }


    const outingName =
        nameInput.value.trim();

    const outingType =
        typeSelect
            ? typeSelect.value
            : "scheduled";


    const outingDate =
        dateInput
            ? dateInput.value
            : "";

    /*
     お出かけ名チェック
    */

    if (outingName === "") {

        if (message) {

            message.textContent =
                t(
                    "outing.message.nameRequired"
                );

        }

        nameInput.focus();

        return;

    }

    /*
     1回のお出かけは
     日付必須
    */

    if (
        outingType ===
        "scheduled" &&
        outingDate ===
        ""
    ) {

        if (message) {

            message.textContent =
                t(
                    "outing.message.dateRequired"
                );

        }

        if (dateInput) {

            dateInput.focus();

        }

        return;

    }

    /*
     持ち物チェック
    */

    if (
        outingDraftItems.length === 0
    ) {

        if (message) {

            message.textContent =
                t(
                    "outing.message.itemsRequired"
                );

        }

        return;

    }


    /*
     同名チェック
    */

    const duplicate =
        outings.some(

            outing =>
                outing.name ===
                outingName

        );


    if (duplicate) {

        if (message) {

            message.textContent =
                t(
                    "outing.message.nameDuplicate"
                );

        }

        nameInput.focus();

        return;

    }


    const now =
        new Date().toISOString();


    outings.push({

        id:
            "outing-" +
            Date.now() +
            "-" +
            Math.random()
                .toString(36)
                .slice(2, 7),

        name:
            outingName,


        /*
         scheduled = 1回のお出かけ
         routine   = 繰り返し
        */

        type:
            outingType,


        /*
         1回のお出かけだけ
         日付を保存
        */

        date:
            outingType ===
                "scheduled"
                ? outingDate
                : null,


        /*
         1回のお出かけは予定から開始
        
         繰り返しはroutineとして管理
        */

        status:
            outingType ===
                "scheduled"
                ? "upcoming"
                : "routine",


        items:
            outingDraftItems.map(

                item => ({

                    id:
                        item.id,

                    name:
                        item.name,


                    /*
                     新しいお出かけでは
                     必ず準備前から開始
                    */

                    status:
                        "pending",


                    /*
                     家にある物 / 購入する物
                    */

                    itemType:
                        item.itemType ||
                        "home",


                    /*
                     関連付け済みの商品は維持
                    */

                    productId:
                        item.productId ||
                        null,


                    /*
                     数量
                    */

                    quantity:
                        item.quantity ||
                        1,


                    createdAt:
                        now,

                    updatedAt:
                        now

                })

            ),

        createdAt:
            now,

        updatedAt:
            now

    });


    saveOutings();


    /*
     作成途中データをリセット
    */

    outingDraftItems =
        [];


    if (message) {

        message.textContent =
            "";

    }


    openCheck();

}

/* ==========================================
   お出かけ一覧
   表示切替
   ========================================== */

function changeOutingListView(
    viewMode
) {

    outingListViewMode =
        viewMode;


    updateOutingListTabs();

    displayOutings();

}

/* ==========================================
   お出かけ一覧
   タブ表示更新
   ========================================== */

function updateOutingListTabs() {

    const buttons = {

        upcoming:
            document.getElementById(
                "btnOutingUpcoming"
            ),

        routine:
            document.getElementById(
                "btnOutingRoutine"
            ),

        past:
            document.getElementById(
                "btnOutingPast"
            )

    };


    for (
        const [mode, button] of
        Object.entries(buttons)
    ) {

        if (!button) {

            continue;

        }


        button.classList.toggle(

            "active",

            outingListViewMode ===
            mode

        );

    }

}

/* ==========================================
   お出かけ一覧
   件数更新
   ========================================== */

function updateOutingListCounts() {

    const upcomingCount =
        outings.filter(

            outing =>
                outing.status ===
                "upcoming"

        ).length;


    const routineCount =
        outings.filter(

            outing =>
                outing.status ===
                "routine"

        ).length;


    const pastCount =
        outings.filter(

            outing =>
                outing.status ===
                "completed"

        ).length;


    const upcomingElement =
        document.getElementById(
            "outingUpcomingCount"
        );

    const routineElement =
        document.getElementById(
            "outingRoutineCount"
        );

    const pastElement =
        document.getElementById(
            "outingPastCount"
        );


    if (upcomingElement) {

        upcomingElement.textContent =
            upcomingCount;

    }


    if (routineElement) {

        routineElement.textContent =
            routineCount;

    }


    if (pastElement) {

        pastElement.textContent =
            pastCount;

    }

}

/* ==========================================
   お出かけ
   一覧表示
   ========================================== */

function displayOutings() {

    const list =
        document.getElementById(
            "outingList"
        );


    if (!list) {

        return;

    }


    list.innerHTML =
        "";


    /*
     予定・いつも使う・過去
     件数と選択状態を更新
    */

    updateOutingListCounts();

    updateOutingListTabs();


    /*
     現在のタブに対応する
     お出かけ状態を決める
    */

    let targetStatus =
        outingListViewMode;


    if (
        outingListViewMode ===
        "past"
    ) {

        targetStatus =
            "completed";

    }


    const filteredOutings =
        outings.filter(

            outing =>
                outing.status ===
                targetStatus

        );


    if (
        filteredOutings.length === 0
    ) {

        let emptyMessage =
            t(
                "outing.empty.default"
            );

        if (
            outingListViewMode ===
            "upcoming"
        ) {

            emptyMessage =
                t(
                    "outing.empty.upcoming"
                );

        } else if (
            outingListViewMode ===
            "routine"
        ) {

            emptyMessage =
                t(
                    "outing.empty.routine"
                );

        } else if (
            outingListViewMode ===
            "past"
        ) {

            emptyMessage =
                t(
                    "outing.empty.past"
                );
        }

        list.innerHTML = `

        <p class="check-empty-message">

            ${emptyMessage}

        </p>

    `;

        return;
    }


    /*
     過去タブだけ
     年月別表示
    */

    if (
        outingListViewMode ===
        "past"
    ) {

        displayPastOutingsByMonth(
            filteredOutings,
            list
        );

        return;

    }


    for (
        const outing of
        filteredOutings
    ) {

        const card =
            document.createElement(
                "div"
            );


        card.className =
            "outing-card";


        /*
         予定のお出かけだけ
         日付を表示
        */

        let outingDateHtml =
            "";


        if (
            outing.status ===
            "upcoming" &&
            outing.date
        ) {

            const outingDate =
                new Date(
                    outing.date +
                    "T00:00:00"
                );


            if (
                !Number.isNaN(
                    outingDate.getTime()
                )
            ) {

                const dateText =

                    String(
                        outingDate.getMonth() + 1
                    ).padStart(
                        2,
                        "0"
                    ) +

                    "/" +

                    String(
                        outingDate.getDate()
                    ).padStart(
                        2,
                        "0"
                    );


                outingDateHtml = `

            <span class="outing-card-date">

                ${dateText}

            </span>

        `;

            }

        }


        card.innerHTML = `

            <button
                type="button"
                class="outing-card-button"
                onclick="openOutingCheck('${outing.id}')">

                ${outingDateHtml}

                <span class="outing-card-name">

                    ${outing.name}

                </span>

                <span class="outing-card-count">

                    ${t(
            outing.items.length === 1
                ? "outing.past.itemCountOne"
                : "outing.past.itemCount"
        ).replace(
            "{count}",
            outing.items.length
        )}

</span>

            </button>

        `;


        list.appendChild(
            card
        );

    }

}

/* ==========================================
   過去のお出かけ
   年月別表示
   ========================================== */

function displayPastOutingsByMonth(
    pastOutings,
    list
) {

    const monthlyOutings = {};


    /*
     終了日時ごとに年月を作る
    */

    for (
        const outing of
        pastOutings
    ) {

        const baseDate =
            outing.completedAt ||
            outing.date ||
            outing.updatedAt ||
            outing.createdAt;


        const date =
            new Date(
                baseDate
            );


        if (
            Number.isNaN(
                date.getTime()
            )
        ) {

            continue;

        }


        const monthKey =

            date.getFullYear() +

            "-" +

            String(
                date.getMonth() + 1
            ).padStart(
                2,
                "0"
            );


        if (
            !monthlyOutings[
            monthKey
            ]
        ) {

            monthlyOutings[
                monthKey
            ] = [];

        }


        monthlyOutings[
            monthKey
        ].push(
            outing
        );

    }


    /*
     新しい年月から表示
    */

    const monthKeys =
        Object.keys(
            monthlyOutings
        )
            .sort()
            .reverse();


    monthKeys.forEach(

        (
            monthKey,
            monthIndex
        ) => {

            const [
                year,
                month
            ] =
                monthKey.split(
                    "-"
                );


            const outingsForMonth =
                monthlyOutings[
                monthKey
                ];


            /*
             最新月だけ開く
            */

            const isOpen =
                monthIndex === 0;


            const section =
                document.createElement(
                    "section"
                );


            section.className =
                "outing-past-month";


            section.innerHTML = `

                <button
                    type="button"
                    class="outing-past-month-header"
                    aria-expanded="${isOpen}"
                    onclick="toggleOutingPastMonth(this)">

                    <span class="outing-past-month-arrow">

                        ${isOpen ? "▼" : "▶"}

                    </span>

                    <span>

                        ${t(
                "outing.past.yearMonth"
            )
                    .replace(
                        "{year}",
                        Number(year)
                    )
                    .replace(
                        "{month}",
                        Number(month)
                    )}

                    </span>

                    <span>

                         ${t(
                        outingsForMonth.length === 1
                            ? "outing.past.countOne"
                            : "outing.past.count"
                    ).replace(
                        "{count}",
                        outingsForMonth.length
                    )}

                    </span>

                </button>


                <div
                    class="outing-past-month-content"
                    ${isOpen ? "" : "hidden"}>

                </div>

            `;


            const content =
                section.querySelector(
                    ".outing-past-month-content"
                );


            for (
                const outing of
                outingsForMonth
            ) {

                const date =
                    new Date(

                        outing.completedAt ||
                        outing.date ||
                        outing.updatedAt ||
                        outing.createdAt

                    );


                const dateText =

                    String(
                        date.getMonth() + 1
                    ).padStart(
                        2,
                        "0"
                    ) +

                    "/" +

                    String(
                        date.getDate()
                    ).padStart(
                        2,
                        "0"
                    );


                const card =
                    document.createElement(
                        "button"
                    );


                card.type =
                    "button";


                card.className =
                    "outing-past-card";


                card.innerHTML = `

                    <span class="outing-past-date">

                        ${dateText}

                    </span>

                    <strong>

                        ${outing.name}

                    </strong>

                    <span>

                        ${t(
                    outing.items.length === 1
                        ? "outing.past.itemCountOne"
                        : "outing.past.itemCount"
                ).replace(
                    "{count}",
                    outing.items.length
                )}

                    </span>

                `;


                card.addEventListener(

                    "click",

                    () => {

                        openPastOutingDetail(
                            outing.id
                        );

                    }

                );


                content.appendChild(
                    card
                );

            }


            list.appendChild(
                section
            );

        }

    );

}

/* ==========================================
   過去のお出かけ
   年月開閉
   ========================================== */

function toggleOutingPastMonth(
    button
) {

    const section =
        button.closest(
            ".outing-past-month"
        );


    if (!section) {

        return;

    }


    const content =
        section.querySelector(
            ".outing-past-month-content"
        );

    const arrow =
        button.querySelector(
            ".outing-past-month-arrow"
        );


    if (
        !content ||
        !arrow
    ) {

        return;

    }


    const willOpen =
        content.hidden;


    content.hidden =
        !willOpen;


    button.setAttribute(

        "aria-expanded",

        String(
            willOpen
        )

    );


    arrow.textContent =
        willOpen
            ? "▼"
            : "▶";

}

/* ==========================================
   過去のお出かけ
   詳細表示
   ========================================== */

function openPastOutingDetail(
    outingId
) {

    const outing =
        outings.find(

            outing =>
                outing.id ===
                outingId

        );


    if (!outing) {

        return;

    }


    hideAllScreens();


    const screen =
        document.getElementById(
            "outingPastDetailScreen"
        );

    const title =
        document.getElementById(
            "outingPastDetailTitle"
        );

    const dateElement =
        document.getElementById(
            "outingPastDetailDate"
        );

    const itemsElement =
        document.getElementById(
            "outingPastDetailItems"
        );


    if (title) {

        title.textContent =
            "🎒 " +
            outing.name;

    }


    /*
     日付表示
    */

    if (dateElement) {

        const baseDate =
            outing.date ||
            outing.completedAt;


        if (baseDate) {

            const date =
                new Date(
                    baseDate
                );


            dateElement.textContent =

                date.getFullYear() +

                "/" +

                String(
                    date.getMonth() + 1
                ).padStart(
                    2,
                    "0"
                ) +

                "/" +

                String(
                    date.getDate()
                ).padStart(
                    2,
                    "0"
                );

        } else {

            dateElement.textContent =
                t(
                    "outing.past.noDate"
                );

        }

    }


    /*
     持ち物表示
    */

    if (itemsElement) {

        itemsElement.innerHTML =
            "";


        if (
            outing.items.length === 0
        ) {

            itemsElement.innerHTML = `

                <p class="check-empty-message">

                    ${t("outing.past.noItems")}

                </p>

            `;

        } else {

            for (
                const item of
                outing.items
            ) {

                const row =
                    document.createElement(
                        "div"
                    );


                row.className =
                    "outing-past-detail-item";


                let statusText =
                    t(
                        "outing.check.pending"
                    );

                if (
                    item.status ===
                    "ready"
                ) {

                    statusText =
                        t(
                            "outing.check.ready"
                        );

                } else if (
                    item.status ===
                    "hold"
                ) {

                    statusText =
                        t(
                            "outing.check.hold"
                        );

                }


                row.innerHTML = `

                    <span class="outing-past-detail-item-name">

                        ${item.name}

                    </span>

                    <span
                        class="outing-past-status
                        outing-past-status-${item.status || "pending"}">

                    ${statusText}

                     </span>

                `;


                itemsElement.appendChild(
                    row
                );

            }

        }

    }

    /*
    「もう一度使う」ボタン

     以前表示したボタンがあれば
     一度削除
    */

    const oldReuseArea =
        document.getElementById(
            "outingPastReuseArea"
        );


    if (oldReuseArea) {

        oldReuseArea.remove();

    }


    /*
     再利用・削除エリアを作る
    */

    if (screen) {

        const reuseArea =
            document.createElement(
                "div"
            );


        reuseArea.id =
            "outingPastReuseArea";


        reuseArea.innerHTML = `

<button
    type="button"
    class="outing-past-reuse-button"
    onclick="reusePastOuting('${outing.id}')">
    ${t("outing.past.reuse")}
</button>

<button
    type="button"
    class="outing-past-delete-button"
    onclick="deletePastOuting('${outing.id}')">
    ${t("outing.past.delete")}
</button>

`;


        screen.appendChild(
            reuseArea
        );

    }

    if (screen) {

        screen.hidden =
            false;

    }


    window.scrollTo({

        top: 0,

        behavior:
            "smooth"

    });

}

/* ==========================================
   過去のお出かけ
   もう一度使う
   ========================================== */

function reusePastOuting(
    outingId
) {

    const sourceOuting =
        outings.find(

            outing =>
                outing.id ===
                outingId

        );


    if (!sourceOuting) {

        return;

    }


    /*
     新しいお出かけ作成画面を開く

     ここで入力欄や
     outingDraftItemsが一度リセットされる
    */

    openCreateOuting();


    /*
     お出かけ名

     同名チェックに引っかからないよう
     「（再利用）」を付ける
    */

    const nameInput =
        document.getElementById(
            "txtOutingName"
        );


    if (nameInput) {

        nameInput.value =
            sourceOuting.name +
            t(
                "outing.past.reuseSuffix"
            );
    }


    /*
     1回のお出かけとして開始
    */

    const typeSelect =
        document.getElementById(
            "cmbOutingType"
        );


    if (typeSelect) {

        typeSelect.value =
            "scheduled";

    }


    /*
     日付は前回の日付を使わず
     新しく選んでもらう
    */

    const dateInput =
        document.getElementById(
            "txtOutingDate"
        );


    if (dateInput) {

        dateInput.value =
            "";

    }


    updateOutingDateArea();


    /*
     過去の持ち物を
     新しい下書きへコピー
    */

    outingDraftItems =
        sourceOuting.items.map(

            item => ({

                /*
                 新しい持ち物ID
                */

                id:
                    "outing-item-" +
                    Date.now() +
                    "-" +
                    Math.random()
                        .toString(36)
                        .slice(2, 7),


                /*
                 持ち物名
                */

                name:
                    item.name,


                /*
                 家にある物 / 購入する物
                 を維持
                */

                itemType:
                    item.itemType ||
                    "home",


                /*
                 正式商品が設定されていれば
                 その関連は維持
                */

                productId:
                    item.productId ||
                    null,


                /*
                 数量も維持
                */

                quantity:
                    item.quantity ||
                    1

            })

        );


    /*
     下書き一覧を再表示
    */

    displayOutingDraftItems();


    /*
     案内
    */

    const message =
        document.getElementById(
            "outingCreateMessage"
        );


    if (message) {

        message.textContent =
            t(
                "outing.message.copied"
            );
    }


    window.scrollTo({

        top: 0,

        behavior:
            "smooth"

    });

}

/* ==========================================
   過去のお出かけ
   削除
   ========================================== */

function deletePastOuting(
    outingId
) {

    const outing =
        outings.find(

            outing =>
                outing.id ===
                outingId

        );


    if (!outing) {

        return;

    }


    /*
     過去のお出かけだけを
     削除対象にする
    */

    if (
        outing.status !==
        "completed"
    ) {

        return;

    }


    /*
     削除確認
    */

    const confirmed =
        confirm(

            t(
                "outing.confirm.deletePast"
            ).replace(
                "{name}",
                outing.name
            )
        );


    if (!confirmed) {

        return;

    }


    /*
     このお出かけだけ削除
    */

    outings =
        outings.filter(

            item =>
                item.id !==
                outingId

        );


    /*
     保存
    */

    saveOutings();


    /*
     過去タブへ戻る
    */

    outingListViewMode =
        "past";


    openCheck();

}

/* ==========================================
   お出かけ
   チェック画面を開く
   ========================================== */

function openOutingCheck(
    outingId
) {

    const outing =
        outings.find(

            outing =>
                outing.id ===
                outingId

        );


    if (!outing) {

        return;

    }


    /*
     現在開いている
     お出かけを記憶
    */

    currentOutingId =
        outingId;


    /*
     最初は準備前
    */

    outingViewMode =
        "pending";


    /*
     古いデータにも
     statusを付ける
    */

    for (
        const item of
        outing.items
    ) {

        if (!item.status) {

            item.status =
                "pending";

        }

    }


    hideAllScreens();


    const screen =
        document.getElementById(
            "outingCheckScreen"
        );

    const title =
        document.getElementById(
            "outingCheckTitle"
        );

    const dateElement =
        document.getElementById(
            "outingCheckDate"
        );

    if (title) {

        title.textContent =
            "🎒 " +
            outing.name;

    }

    /*
     予定のお出かけの日付表示
    */

    if (dateElement) {

        if (
            outing.status ===
            "upcoming" &&
            outing.date
        ) {

            const outingDate =
                new Date(
                    outing.date +
                    "T00:00:00"
                );


            if (
                !Number.isNaN(
                    outingDate.getTime()
                )
            ) {

                dateElement.textContent =

                    outingDate.getFullYear() +

                    "/" +

                    String(
                        outingDate.getMonth() + 1
                    ).padStart(
                        2,
                        "0"
                    ) +

                    "/" +

                    String(
                        outingDate.getDate()
                    ).padStart(
                        2,
                        "0"
                    );

                dateElement.hidden =
                    false;

            }

        } else {

            dateElement.textContent =
                "";

            dateElement.hidden =
                true;

        }

    }

    /*
     お出かけ終了エリア

     予定のお出かけだけ表示する
     いつも使う・過去では非表示
    */

    const completeArea =
        document.getElementById(
            "outingCompleteArea"
        );


    if (completeArea) {

        completeArea.hidden =

            outing.status !==
            "upcoming" ||

            outingViewMode ===
            "add";

    }

    if (screen) {

        screen.hidden =
            false;

    }


    updateOutingCounts();

    updateOutingViewButtons();

    displayOutingItemsByCurrentView();


    window.scrollTo({

        top: 0,

        behavior:
            "smooth"

    });

}

/* ==========================================
   お出かけ
   表示切替
   ========================================== */

function changeOutingView(
    viewMode
) {

    outingViewMode =
        viewMode;

    /*
     上部の「＋追加」を直接押した場合は
     通常の追加画面
    */

    if (
        viewMode ===
        "add" &&
        !outingLinkTargetItemId
    ) {

        outingProductSelectMode =
            false;

    }

    updateOutingViewButtons();


    /*
     ＋追加画面では
     お出かけ終了を非表示
    */

    const outing =
        outings.find(

            outing =>
                outing.id ===
                currentOutingId

        );


    const completeArea =
        document.getElementById(
            "outingCompleteArea"
        );


    if (
        outing &&
        completeArea
    ) {

        completeArea.hidden =

            outing.status !==
            "upcoming" ||

            outingViewMode ===
            "add";

    }


    displayOutingItemsByCurrentView();

}

/* ==========================================
   お出かけ
   表示ボタン更新
   ========================================== */

function updateOutingViewButtons() {

    const buttons = {

        pending:
            document.getElementById(
                "btnOutingPending"
            ),

        ready:
            document.getElementById(
                "btnOutingReady"
            ),

        hold:
            document.getElementById(
                "btnOutingHold"
            ),

        add:
            document.getElementById(
                "btnOutingAdd"
            )

    };


    for (
        const [mode, button] of
        Object.entries(buttons)
    ) {

        if (!button) {

            continue;

        }


        button.classList.toggle(

            "active",

            outingViewMode ===
            mode

        );

    }

}

/* ==========================================
   お出かけ
   件数更新
   ========================================== */

function updateOutingCounts() {

    const outing =
        outings.find(

            outing =>
                outing.id ===
                currentOutingId

        );


    if (!outing) {

        return;

    }


    const pendingCount =
        outing.items.filter(

            item =>
                item.status ===
                "pending"

        ).length;


    const readyCount =
        outing.items.filter(

            item =>
                item.status ===
                "ready"

        ).length;


    const holdCount =
        outing.items.filter(

            item =>
                item.status ===
                "hold"

        ).length;


    const pendingElement =
        document.getElementById(
            "outingPendingCount"
        );

    const readyElement =
        document.getElementById(
            "outingReadyCount"
        );

    const holdElement =
        document.getElementById(
            "outingHoldCount"
        );


    if (pendingElement) {

        pendingElement.textContent =
            pendingCount;

    }


    if (readyElement) {

        readyElement.textContent =
            readyCount;

    }


    if (holdElement) {

        holdElement.textContent =
            holdCount;

    }

}

/* ==========================================
   お出かけ
   現在の状態の商品表示
   ========================================== */

function displayOutingItemsByCurrentView() {

    const list =
        document.getElementById(
            "outingCheckItemList"
        );


    if (!list) {

        return;

    }


    list.innerHTML =
        "";


    const outing =
        outings.find(

            outing =>
                outing.id ===
                currentOutingId

        );


    if (!outing) {

        return;

    }


    /*
     ＋追加
    */

    if (
        outingViewMode ===
        "add"
    ) {

        /*
         既存の購入する物から
        「商品を選ぶ」を押した場合
        */

        if (
            outingProductSelectMode &&
            outingLinkTargetItemId
        ) {

            const targetItem =
                outing.items.find(

                    item =>
                        item.id ===
                        outingLinkTargetItemId

                );


            if (!targetItem) {

                outingProductSelectMode =
                    false;

                outingLinkTargetItemId =
                    null;

                outingViewMode =
                    "pending";

                displayOutingItemsByCurrentView();

                return;

            }


            /*
             商品選択専用画面
            */

            list.innerHTML = `

        <div class="outing-product-select-screen">


            <button
                type="button"
                id="btnCancelOutingProductSelect"
                class="outing-product-select-back">

                ${t("outing.productSelect.back")}

            </button>


            <div class="outing-add-section">


                <h3 class="outing-add-section-title">

                    ${t("outing.productSelect.title")}

                </h3>


                <p class="outing-product-select-target">

                    ${t("outing.productSelect.guide")
                    .replace(
                        "{name}",
                        targetItem.name
                    )}

                </p>


                <div class="outing-product-select-method">

                    <h4 class="outing-product-method-title">

                        ${t("outing.add.registered")}

                    </h4>


                    <label for="cmbOutingProductCategory">

                        ${t("outing.add.category")}

                    </label>


                    <select id="cmbOutingProductCategory">

                        <option value="">
                            ${t("outing.add.allCategories")}
                        </option>

                    </select>


                    <label for="cmbOutingAddProduct">

                        ${t("outing.add.product")}

                    </label>


                    <select id="cmbOutingAddProduct">

                        <option value="">
                            ${t("outing.add.selectProduct")}
                        </option>

                    </select>

                </div>


                <div class="outing-product-method-separator">
                </div>


                <div class="outing-product-select-method">

                    <h4 class="outing-product-method-title">

                        ${t("outing.add.code")}

                    </h4>


                    <p class="outing-product-method-guide">

                        ${t("outing.add.codeGuide")}

                    </p>


                    <button
                        type="button"
                        id="btnScanOutingJan"
                        class="btn-main">

                        ${t("outing.add.scan")}

                    </button>

                </div>


                <label for="txtOutingProductQuantity">

                    ${t("outing.add.quantity")}

                </label>


                <input
                    type="number"
                    id="txtOutingProductQuantity"
                    min="1"
                    value="${targetItem.quantity || 1}">


                <button
                    type="button"
                    id="btnAddProductToOuting"
                    class="btn-main">

                    ${t("outing.productSelect.selectButton")}

                </button>


                <p
                    id="outingAddProductMessage"
                    class="shopping-detail-guide">
                </p>


            </div>

        </div>

    `;


            /*
             ジャンル
            */

            displayOutingProductCategories();


            /*
             商品
            */

            displayOutingProductOptions();


            /*
             ジャンル変更
            */

            const categorySelect =
                document.getElementById(
                    "cmbOutingProductCategory"
                );


            if (categorySelect) {

                categorySelect.addEventListener(

                    "change",

                    displayOutingProductOptions

                );

            }


            /*
             商品を決定
            */

            const addProductButton =
                document.getElementById(
                    "btnAddProductToOuting"
                );


            if (addProductButton) {

                addProductButton.addEventListener(

                    "click",

                    addProductToCurrentOuting

                );

            }


            /*
             商品コード読取
            */

            const scanOutingJanButton =
                document.getElementById(
                    "btnScanOutingJan"
                );


            if (scanOutingJanButton) {

                scanOutingJanButton.addEventListener(

                    "click",

                    () => {

                        barcodeReadMode =
                            "outingAddJan";


                        /*
                         コード読取画面の戻るボタンを
                         お出かけチェック用へ変更
                        */

                        const btnBackCode =
                            document.getElementById(
                                "btnBackCode"
                            );

                        const btnCancelScan =
                            document.getElementById(
                                "btnCancelScan"
                            );

                        if (btnBackCode) {

                            btnBackCode.textContent =
                                t(
                                    "outing.scan.back"
                                );
                        }

                        if (btnCancelScan) {

                            btnCancelScan.textContent =
                                t(
                                    "outing.scan.back"
                                );
                        }


                        /*
                         お出かけチェック画面を閉じる
                        */

                        const outingCheckScreen =
                            document.getElementById(
                                "outingCheckScreen"
                            );


                        if (outingCheckScreen) {

                            outingCheckScreen.hidden =
                                true;

                        }


                        const codeScreen =
                            document.getElementById(
                                "codeScreen"
                            );


                        if (codeScreen) {

                            codeScreen.hidden =
                                false;

                        }


                        readCode();

                    }

                );

            }


            /*
             戻る
            */

            const cancelButton =
                document.getElementById(
                    "btnCancelOutingProductSelect"
                );


            if (cancelButton) {

                cancelButton.addEventListener(

                    "click",

                    () => {

                        outingProductSelectMode =
                            false;

                        outingLinkTargetItemId =
                            null;

                        outingViewMode =
                            "pending";


                        updateOutingViewButtons();

                        displayOutingItemsByCurrentView();

                    }

                );

            }


            return;

        }

        list.innerHTML = `

    <div class="outing-add-item-area">


        <div class="outing-add-section">

            <h3 class="outing-add-section-title">

                ${t("outing.add.homeTitle")}

            </h3>


            <label for="txtOutingAddItemName">

                ${t("outing.add.item")}

            </label>


            <input
                type="text"
                id="txtOutingAddItemName"
                placeholder="${t("outing.add.itemPlaceholder")}">


            <button
                type="button"
                id="btnAddItemToCurrentOuting"
                class="btn-main">

                ${t("outing.add.itemButton")}

            </button>


            <p
                id="outingAddItemMessage"
                class="shopping-detail-guide">
            </p>

        </div>


        <div class="outing-add-section">

            <h3 class="outing-add-section-title">

                ${t("outing.add.purchaseTitle")}

            </h3>


            <p class="outing-add-help">

                ${t("outing.add.purchaseGuide")}

            </p>


            <label for="txtOutingPurchaseItemName">

                ${t("outing.add.purchase")}

            </label>


            <input
                type="text"
                id="txtOutingPurchaseItemName"
                placeholder="${t("outing.add.purchasePlaceholder")}">


            <button
                type="button"
                id="btnAddPurchaseItemToOuting"
                class="btn-main">

                ${t("outing.add.purchaseButton")}

            </button>


            <p
                id="outingAddPurchaseMessage"
                class="shopping-detail-guide">
            </p>


            <div class="outing-product-choice-divider">

    ${t("outing.add.selectMethod")}

</div>


<div class="outing-product-select-method">

    <h4 class="outing-product-method-title">

        ${t("outing.add.registered")}

    </h4>


    <label for="cmbOutingProductCategory">

        ${t("outing.add.category")}

    </label>


    <select id="cmbOutingProductCategory">

        <option value="">
            ${t("outing.add.allCategories")}
        </option>

    </select>


    <label for="cmbOutingAddProduct">

        ${t("outing.add.product")}

    </label>


    <select id="cmbOutingAddProduct">

        <option value="">
            ${t("outing.add.selectProduct")}
        </option>

    </select>

</div>


<div class="outing-product-method-separator">
</div>


<div class="outing-product-select-method">

    <h4 class="outing-product-method-title">

        ${t("outing.add.code")}

    </h4>


    <p class="outing-product-method-guide">

        ${t("outing.add.codeGuide")}

    </p>


    <button
        type="button"
        id="btnScanOutingJan"
        class="btn-main">

        ${t("outing.add.scan")}

    </button>

</div>


            <label for="txtOutingProductQuantity">

                ${t("outing.add.quantity")}

            </label>


            <input
                type="number"
                id="txtOutingProductQuantity"
                min="1"
                value="1">


            <button
                type="button"
                id="btnAddProductToOuting"
                class="btn-main">

                ${t("outing.add.productButton")}

            </button>


            <p
                id="outingAddProductMessage"
                class="shopping-detail-guide">
            </p>

        </div>


    </div>

`;


        const addButton =
            document.getElementById(
                "btnAddItemToCurrentOuting"
            );


        if (addButton) {

            addButton.addEventListener(

                "click",

                addItemToCurrentOuting

            );

        }

        /*
         購入する物を
         仮の名前で追加
        */

        const addPurchaseButton =
            document.getElementById(
                "btnAddPurchaseItemToOuting"
            );


        if (addPurchaseButton) {

            addPurchaseButton.addEventListener(

                "click",

                addPurchaseItemToCurrentOuting

            );

        }
        /*
         商品ジャンルを
         お出かけの商品選択へ表示
        */

        displayOutingProductCategories();


        /*
         登録済み商品を
         お出かけの商品選択へ表示
        */

        displayOutingProductOptions();

        /*
         商品ジャンルを変更したら
         登録済み商品を絞り込む
        */

        const outingProductCategorySelect =
            document.getElementById(
                "cmbOutingProductCategory"
            );


        if (outingProductCategorySelect) {

            outingProductCategorySelect.addEventListener(

                "change",

                () => {

                    displayOutingProductOptions();

                }

            );

        }

        /*
         登録済み商品を
         お出かけ持ち物へ追加
        */

        const addProductButton =
            document.getElementById(
                "btnAddProductToOuting"
            );


        if (addProductButton) {

            addProductButton.addEventListener(

                "click",

                addProductToCurrentOuting

            );

        }

        /*
         お出かけ＋追加
         商品コードをカメラで読み取る
        */

        const scanOutingJanButton =
            document.getElementById(
                "btnScanOutingJan"
            );


        if (scanOutingJanButton) {

            scanOutingJanButton.addEventListener(

                "click",

                () => {

                    /*
                     お出かけ＋追加から
                     商品コード読取を開いたことを記録
                    */

                    barcodeReadMode =
                        "outingAddJan";


                    /*
                     コード読取画面の戻るボタンを
                     お出かけチェック用へ変更
                    */

                    const btnBackCode =
                        document.getElementById(
                            "btnBackCode"
                        );

                    const btnCancelScan =
                        document.getElementById(
                            "btnCancelScan"
                        );

                    if (btnBackCode) {

                        btnBackCode.textContent =
                            t(
                                "outing.scan.back"
                            );
                    }

                    if (btnCancelScan) {

                        btnCancelScan.textContent =
                            t(
                                "outing.scan.back"
                            );
                    }


                    /*
                     お出かけチェック画面を閉じる
                    */

                    const outingCheckScreen =
                        document.getElementById(
                            "outingCheckScreen"
                        );


                    if (outingCheckScreen) {

                        outingCheckScreen.hidden =
                            true;

                    }


                    /*
                     コード読取画面を表示
                    */

                    const codeScreen =
                        document.getElementById(
                            "codeScreen"
                        );


                    if (codeScreen) {

                        codeScreen.hidden =
                            false;

                    }


                    /*
                     カメラ開始
                    */

                    readCode();

                }

            );

        }

        return;

    }


    const filteredItems =
        outing.items.filter(

            item =>
                item.status ===
                outingViewMode

        );

    /*
     家にある物
    */

    const homeItems =
        filteredItems.filter(

            item =>
                (
                    item.itemType ||
                    "home"
                ) ===
                "home"

        );


    /*
     購入する物
    */

    const purchaseItems =
        filteredItems.filter(

            item =>
                item.itemType ===
                "purchase"

        );

    if (
        filteredItems.length === 0
    ) {

        list.innerHTML = `

            <p class="check-empty-message">

                ${t("outing.items.emptyStatus")}

            </p>

        `;

        return;

    }


    /*
     家にある物 / 購入する物
     のグループ
    */

    const groups = [

        {

            title:
                t("outing.items.home"),

            className:
                "home",

            items:
                homeItems

        },

        {

            title:
                outingViewMode === "ready"
                    ? t("outing.items.purchased")
                    : t("outing.items.purchase"),

            className:
                "purchase",

            items:
                purchaseItems

        }

    ];


    for (
        const group of
        groups
    ) {

        /*
         グループ全体
        */

        const groupSection =
            document.createElement(
                "section"
            );


        groupSection.className =
            "outing-item-group " +
            "outing-item-group-" +
            group.className;


        /*
         グループ見出し
        */

        groupSection.innerHTML = `

        <div class="outing-item-group-header">

            <strong>
                ${group.title}
            </strong>

            <span>
                ${group.items.length}
            </span>

        </div>

        <div class="outing-item-group-list">
        </div>

    `;


        const groupList =
            groupSection.querySelector(
                ".outing-item-group-list"
            );


        /*
         この種類に持ち物がない場合
        */

        if (
            group.items.length === 0
        ) {

            groupList.innerHTML = `

                <p class="outing-group-empty">

                    ${t("outing.items.emptyGroup")}

                </p>

            `;

        }


        /*
         このグループの持ち物を表示
        */

        for (
            const item of
            group.items
        ) {

            const card =
                document.createElement(
                    "div"
                );


            card.className =
                "outing-check-card";


            /*
             関連付け済みの正式商品
            */

            const linkedProduct =
                item.productId
                    ? products.find(

                        product =>
                            product.id ===
                            item.productId

                    )
                    : null;


            /*
             正式商品名と数量の表示
            */

            const linkedProductHtml =
                linkedProduct
                    ? `
        <p class="shopping-detail-guide">

            🔗 ${linkedProduct.name}
            ・${t("outing.items.quantity")
                        .replace(
                            "{quantity}",
                            item.quantity || 1
                        )}

        </p>
      `
                    : "";


            let actionButtonsHtml =
                "";


            /*
             購入して準備済になった
             持ち物の表示
            */

            const shoppingReadyMark =
                item.readySource ===
                    "shopping"

                    ? "🛒 "
                    : "";


            const shoppingReadyText =
                item.readySource ===
                    "shopping"

                    ? `
        <span class="outing-ready-source">

            ${t("outing.items.readyFromShopping")}

        </span>
      `
                    : "";


            /*
             準備前
            */

            if (
                outingViewMode ===
                "pending"
            ) {

                /*
                 購入する物
                */

                if (
                    item.itemType ===
                    "purchase"
                ) {

                    /*
                     商品がまだ決まっていない場合
                     「商品を選ぶ」を表示
                    */

                    const chooseProductButtonHtml =

                        !item.productId

                            ? `
                    <button
                        type="button"
                        class="outing-item-product-button"
                        onclick="startOutingItemProductLink('${item.id}')">

                        ${t("outing.items.chooseProduct")}

                    </button>
                    `
                            : "";


                    /*
                     商品が決まっている場合だけ
                     購入予定へ追加できる
                    */

                    const shoppingButtonHtml =

                        item.productId

                            ? `
                    <button
                        type="button"
                        class="outing-item-shopping-button"
                        onclick="addOutingItemToShopping('${item.id}')">

                        ${t("outing.items.addToShopping")}

                    </button>
                    `
                            : `
                    <p class="outing-product-guide">

                        ${t("outing.items.selectProductGuide")}

                    </p>
                    `;


                    actionButtonsHtml = `

            ${chooseProductButtonHtml}


            ${shoppingButtonHtml}


            <button
                type="button"
                class="outing-item-hold-button"
                onclick="changeOutingItemStatus('${item.id}', 'hold')">

                ${t("outing.items.hold")}

            </button>

        `;

                }


                /*
                 家にある物
                */

                else {

                    actionButtonsHtml = `

            <button
                type="button"
                class="outing-item-ready-button"
                onclick="changeOutingItemStatus('${item.id}', 'ready')">

                ${t("outing.items.ready")}

            </button>


            <button
                type="button"
                class="outing-item-hold-button"
                onclick="changeOutingItemStatus('${item.id}', 'hold')">

                ${t("outing.items.hold")}

            </button>

        `;

                }

            }


            /*
             準備済
            */

            else if (
                outingViewMode ===
                "ready"
            ) {

                actionButtonsHtml = `

                <button
                    type="button"
                    class="outing-item-pending-button"
                    onclick="changeOutingItemStatus('${item.id}', 'pending')">

                    ${t("outing.items.backPending")}

                </button>


                <button
                    type="button"
                    class="outing-item-hold-button"
                    onclick="changeOutingItemStatus('${item.id}', 'hold')">

                    ${t("outing.items.hold")}

                </button>

            `;

            }


            /*
             保留
            */

            else if (
                outingViewMode ===
                "hold"
            ) {

                actionButtonsHtml = `

                <button
                    type="button"
                    class="outing-item-pending-button"
                    onclick="changeOutingItemStatus('${item.id}', 'pending')">

                    ${t("outing.items.backPending")}

                </button>


                <button
                    type="button"
                    class="outing-item-ready-button"
                    onclick="changeOutingItemStatus('${item.id}', 'ready')">

                    ${t("outing.items.ready")}

                </button>

            `;

            }


            /*
             持ち物カード
            */

            card.innerHTML = `

            <strong class="outing-check-item-name">

                ${shoppingReadyMark}${item.name}

            </strong>


            ${linkedProductHtml}


            ${shoppingReadyText}


            <div class="outing-check-item-buttons">

                ${actionButtonsHtml}


                <button
                   type="button"
                   class="outing-item-delete-button"
                   onclick="deleteOutingItem('${item.id}')">

                   ${t("outing.items.delete")}

                </button>

            </div>

        `;


            /*
             カードを
             家にある物 / 購入する物
             の中へ追加
            */

            groupList.appendChild(
                card
            );

        }


        /*
         グループ全体を
         一覧へ追加
        */

        list.appendChild(
            groupSection
        );

    }

}

/* ==========================================
   お出かけ
   登録済み商品候補
   ========================================== */

function displayOutingProductOptions() {

    const productSelect =
        document.getElementById(
            "cmbOutingAddProduct"
        );


    if (!productSelect) {

        return;

    }


    const categorySelect =
        document.getElementById(
            "cmbOutingProductCategory"
        );


    /*
     選択中ジャンル

     ""なら
     すべての商品
    */

    const categoryId =
        categorySelect
            ? categorySelect.value
            : "";


    /*
     商品欄を初期化
    */

    productSelect.innerHTML = `

    <option value="">
        ${t("outing.add.selectProduct")}
    </option>

`;


    /*
     有効な商品だけ取得

     ジャンル選択中なら
     そのジャンルだけに絞る
    */

    const targetProducts =
        products.filter(

            product => {

                if (
                    product.active ===
                    false
                ) {

                    return false;

                }


                if (
                    categoryId !== "" &&
                    product.categoryId !==
                    categoryId
                ) {

                    return false;

                }


                return true;

            }

        );


    /*
     商品名順
    */

    targetProducts.sort(

        (a, b) =>

            String(
                a.name || ""
            ).localeCompare(
                String(
                    b.name || ""
                ),
                "ja"
            )

    );


    /*
     商品をプルダウンへ追加
    */

    for (
        const product of
        targetProducts
    ) {

        const option =
            document.createElement(
                "option"
            );


        option.value =
            product.id;


        /*
         商品に登録されている
         購入先から国を取得
        */

        const store =
            product.storeId
                ? findStore(
                    product.storeId
                )
                : null;

        const country =
            store
                ? store.country || "JP"
                : "JP";

        const countryFlag =
            getCountryFlag(
                country
            );


        /*
         現在は日本語表示なので
         日本の商品は国旗を表示しない
        
         海外商品の場合だけ
         国旗を表示する
        */

        const countryPrefix =
            country === "JP"
                ? ""
                : `${countryFlag} `;


        option.textContent =
            `${countryPrefix}${product.name}`;


        productSelect.appendChild(
            option
        );

    }

}

/* ==========================================
   標準商品ジャンル
   表示言語に合わせた名前を取得
   ========================================== */

function getCategoryDisplayName(
    category
) {

    if (
        !category ||
        !category.name
    ) {

        return "";

    }

    const defaultCategoryTranslationKeys = {

        "食品":
            "category.default.food",

        "飲料":
            "category.default.drink",

        "お菓子":
            "category.default.snack",

        "お酒":
            "category.default.alcohol",

        "日用品":
            "category.default.daily",

        "洗剤・衛生用品":
            "category.default.hygiene",

        "医薬品":
            "category.default.medicine",

        "化粧品・美容":
            "category.default.beauty",

        "衣料品":
            "category.default.clothing",

        "ベビー用品":
            "category.default.baby",

        "ペット用品":
            "category.default.pet",

        "文具・雑貨":
            "category.default.stationery",

        "家電・電池":
            "category.default.electronics",

        "その他":
            "category.default.other"

    };


    const translationKey =
        defaultCategoryTranslationKeys[
        category.name
        ];


    return translationKey
        ? t(translationKey)
        : category.name;

}

/* ==========================================
   お出かけ
   商品選択 ジャンル候補
   ========================================== */

function displayOutingProductCategories() {

    const select =
        document.getElementById(
            "cmbOutingProductCategory"
        );


    if (!select) {

        return;

    }


    /*
     先頭
    */

    select.innerHTML = `

    <option value="">
        ${t("outing.add.allCategories")}
    </option>

`;


    /*
     有効なジャンルだけ表示
    */

    for (
        const category of
        categories
    ) {

        if (
            category.active ===
            false
        ) {

            continue;

        }


        const option =
            document.createElement(
                "option"
            );


        option.value =
            category.id;


        option.textContent =
            getCategoryDisplayName(
                category
            );


        select.appendChild(
            option
        );

    }

}

/* ==========================================
   お出かけ
   登録済み商品を持ち物へ追加
   ========================================== */

function addProductToCurrentOuting() {

    if (!currentOutingId) {

        return;

    }


    const outing =
        outings.find(

            outing =>
                outing.id ===
                currentOutingId

        );


    if (!outing) {

        return;

    }


    const select =
        document.getElementById(
            "cmbOutingAddProduct"
        );


    const quantityInput =
        document.getElementById(
            "txtOutingProductQuantity"
        );


    const message =
        document.getElementById(
            "outingAddProductMessage"
        );


    if (
        !select ||
        !quantityInput
    ) {

        return;

    }


    const productId =
        select.value;


    const quantity =
        Number(
            quantityInput.value
        );


    /*
     商品未選択
    */

    if (productId === "") {

        if (message) {

            message.textContent =
                t(
                    "outing.message.productRequired"
                );

        }

        return;

    }


    /*
     数量チェック
    */

    if (
        !Number.isFinite(quantity) ||
        quantity < 1
    ) {

        if (message) {

            message.textContent =
                t(
                    "outing.message.quantityInvalid"
                );

        }

        quantityInput.focus();

        return;

    }


    /*
     商品マスターから取得
    */

    const product =
        products.find(

            product =>
                product.id ===
                productId

        );


    if (!product) {

        if (message) {

            message.textContent =
                t(
                    "outing.message.productNotFound"
                );

        }

        return;

    }

    /*
     自由入力の持ち物へ
     後から商品を関連付ける場合
    */

    if (outingLinkTargetItemId) {

        const targetItem =
            outing.items.find(

                item =>
                    item.id ===
                    outingLinkTargetItemId

            );


        if (!targetItem) {

            outingLinkTargetItemId =
                null;

            return;

        }


        /*
         同じ商品が別の持ち物に
         すでに関連付いていないか確認
        */

        const duplicateLinkedProduct =
            outing.items.some(

                item =>

                    item.id !==
                    targetItem.id &&

                    item.productId ===
                    product.id

            );


        if (duplicateLinkedProduct) {

            if (message) {

                message.textContent =
                    t(
                        "outing.message.productLinkedDuplicate"
                    );

            }

            return;

        }


        const now =
            new Date().toISOString();


        /*
         元の持ち物名はそのまま残して
         正式商品だけ関連付ける
        */

        targetItem.productId =
            product.id;


        /*
         正式商品を選んだので
         購入する物として扱う
        */

        targetItem.itemType =
            "purchase";


        targetItem.quantity =
            quantity;

        targetItem.updatedAt =
            now;

        outing.updatedAt =
            now;


        /*
         保存
        */

        saveOutings();


        /*
         関連付け対象を解除
        */

        outingLinkTargetItemId =
            null;

        /*
         商品選択専用モード終了
        */

        outingProductSelectMode =
            false;

        /*
         準備前へ戻す
        */

        outingViewMode =
            "pending";


        updateOutingCounts();

        updateOutingViewButtons();

        displayOutingItemsByCurrentView();


        return;

    }

    /*
     同じ商品がすでに
     このお出かけにあるか確認
    */

    const duplicate =
        outing.items.some(

            item =>
                item.productId ===
                product.id

        );


    if (duplicate) {

        if (message) {

            message.textContent =
                t(
                    "outing.message.productDuplicate"
                );

        }

        return;

    }


    const now =
        new Date().toISOString();


    /*
     商品として持ち物へ追加
    */

    outing.items.push({

        id:
            "outing-item-" +
            Date.now() +
            "-" +
            Math.random()
                .toString(36)
                .slice(2, 7),

        name:
            product.name,

        productId:
            product.id,

        /*
         購入する物
        */

        itemType:
            "purchase",

        quantity:
            quantity,

        status:
            "pending",

        createdAt:
            now,

        updatedAt:
            now

    });


    outing.updatedAt =
        now;


    /*
     保存
    */

    saveOutings();


    /*
     準備前へ戻す
    */

    outingViewMode =
        "pending";


    /*
     件数・タブ・一覧を更新
    */

    updateOutingCounts();

    updateOutingViewButtons();

    displayOutingItemsByCurrentView();

}

/* ==========================================
   お出かけ
   自由入力の持ち物へ商品を関連付け開始
   ========================================== */

function startOutingItemProductLink(
    itemId
) {

    if (!currentOutingId) {

        return;

    }


    const outing =
        outings.find(

            outing =>
                outing.id ===
                currentOutingId

        );


    if (!outing) {

        return;

    }


    const item =
        outing.items.find(

            item =>
                item.id ===
                itemId

        );


    if (!item) {

        return;

    }


    /*
     関連付け対象を記憶
    */

    outingLinkTargetItemId =
        item.id;


    /*
     「商品を選ぶ」から
     開いたことを記録
    */

    outingProductSelectMode =
        true;


    /*
     商品選択専用画面を表示するため
     addモードへ移動
    */

    outingViewMode =
        "add";


    updateOutingViewButtons();

    displayOutingItemsByCurrentView();

}

/* ==========================================
   お出かけ
   現在のお出かけへ持ち物追加
   ========================================== */

function addItemToCurrentOuting() {

    if (!currentOutingId) {

        return;

    }


    const outing =
        outings.find(

            outing =>
                outing.id ===
                currentOutingId

        );


    if (!outing) {

        return;

    }


    const input =
        document.getElementById(
            "txtOutingAddItemName"
        );

    const message =
        document.getElementById(
            "outingAddItemMessage"
        );


    if (!input) {

        return;

    }


    const itemName =
        input.value.trim();


    /*
     未入力
    */

    if (itemName === "") {

        if (message) {

            message.textContent =
                t(
                    "outing.message.itemRequired"
                );
        }

        input.focus();
        return;
    }


    /*
     同じお出かけ内で
     持ち物重複チェック
    */

    const duplicate =
        outing.items.some(

            item =>
                item.name ===
                itemName

        );


    if (duplicate) {

        if (message) {

            message.textContent =
                t(
                    "outing.message.itemRegisteredDuplicate"
                );
        }

        input.focus();
        return;
    }


    const now =
        new Date().toISOString();


    outing.items.push({

        id:
            "outing-item-" +
            Date.now() +
            "-" +
            Math.random()
                .toString(36)
                .slice(2, 7),

        name:
            itemName,


        /*
         家にある物
        */

        itemType:
            "home",


        status:
            "pending",

        createdAt:
            now,

        updatedAt:
            now

    });


    outing.updatedAt =
        now;


    /*
     保存
    */

    saveOutings();


    /*
     準備前へ戻す
    */

    outingViewMode =
        "pending";


    /*
     件数・タブ・一覧を更新
    */

    updateOutingCounts();

    updateOutingViewButtons();

    displayOutingItemsByCurrentView();

}

/* ==========================================
   お出かけ
   購入する物を仮の名前で追加
   ========================================== */

function addPurchaseItemToCurrentOuting() {

    if (!currentOutingId) {

        return;

    }


    const outing =
        outings.find(

            outing =>
                outing.id ===
                currentOutingId

        );


    if (!outing) {

        return;

    }


    const input =
        document.getElementById(
            "txtOutingPurchaseItemName"
        );


    const message =
        document.getElementById(
            "outingAddPurchaseMessage"
        );


    if (!input) {

        return;

    }


    const itemName =
        input.value.trim();


    if (itemName === "") {

        if (message) {

            message.textContent =
                t(
                    "outing.message.purchaseRequired"
                );

        }

        input.focus();

        return;

    }


    /*
     同名チェック
    */

    const duplicate =
        outing.items.some(

            item =>
                item.name.trim() ===
                itemName

        );


    if (duplicate) {

        if (message) {

            message.textContent =
                t(
                    "outing.message.itemRegisteredDuplicate"
                );

        }

        input.focus();

        return;

    }


    const now =
        new Date().toISOString();


    outing.items.push({

        id:
            "outing-item-" +
            Date.now() +
            "-" +
            Math.random()
                .toString(36)
                .slice(2, 7),

        name:
            itemName,


        /*
         購入する物
        */

        itemType:
            "purchase",


        /*
         商品はまだ決まっていない
        */

        productId:
            null,

        quantity:
            1,

        status:
            "pending",

        createdAt:
            now,

        updatedAt:
            now

    });


    outing.updatedAt =
        now;


    saveOutings();


    input.value =
        "";


    if (message) {

        message.textContent =
            t(
                "outing.message.purchaseAdded"
            );

    }


    outingViewMode =
        "pending";


    updateOutingCounts();

    updateOutingViewButtons();

    displayOutingItemsByCurrentView();

}

/* ==========================================
   お出かけ
   持ち物の状態変更
   ========================================== */

function changeOutingItemStatus(
    itemId,
    newStatus
) {

    if (!currentOutingId) {

        return;

    }


    const outing =
        outings.find(

            outing =>
                outing.id ===
                currentOutingId

        );


    if (!outing) {

        return;

    }


    const item =
        outing.items.find(

            item =>
                item.id ===
                itemId

        );


    if (!item) {

        return;

    }


    /*
     状態変更
    */

    item.status =
        newStatus;

    /*
     手動で状態を変更した場合は
     購入由来の準備済情報を解除
    */

    item.readySource =
        null;

    item.updatedAt =
        new Date().toISOString();


    outing.updatedAt =
        new Date().toISOString();


    /*
     localStorageへ保存
    */

    saveOutings();


    /*
     準備前・準備済・保留の
     件数をその場で更新
    */

    updateOutingCounts();


    /*
     上部タブの選択状態を更新
    */

    updateOutingViewButtons();


    /*
     現在表示している一覧を再表示
    */

    displayOutingItemsByCurrentView();

}

/* ==========================================
   お出かけ
   持ち物をこのお出かけから削除
   ========================================== */

function deleteOutingItem(
    itemId
) {

    if (!currentOutingId) {

        return;

    }


    const outing =
        outings.find(

            outing =>
                outing.id ===
                currentOutingId

        );


    if (!outing) {

        return;

    }


    const outingItem =
        outing.items.find(

            item =>
                item.id ===
                itemId

        );


    if (!outingItem) {

        return;

    }


    /*
     購入予定と連携しているか確認
    */

    const linkedShoppingItem =
        shoppingItems.find(

            shoppingItem =>

                Array.isArray(
                    shoppingItem.outingLinks
                ) &&

                shoppingItem.outingLinks.some(

                    link =>

                        link.outingId ===
                        outing.id &&

                        link.outingItemId ===
                        outingItem.id

                )

        );


    /*
     確認メッセージ
    */

    let confirmMessage =

        t(
            "outing.confirm.deleteItem"
        ).replace(
            "{name}",
            outingItem.name
        );

    if (linkedShoppingItem) {

        confirmMessage +=

            t(
                "outing.confirm.deleteLinkedItem"
            );
    }


    const confirmed =
        confirm(
            confirmMessage
        );


    if (!confirmed) {

        return;

    }


    /*
     購入予定との関連を解除
    */

    if (linkedShoppingItem) {

        linkedShoppingItem.outingLinks =
            linkedShoppingItem.outingLinks.filter(

                link =>

                    !(
                        link.outingId ===
                        outing.id &&

                        link.outingItemId ===
                        outingItem.id
                    )

            );


        linkedShoppingItem.updatedAt =
            new Date().toISOString();


        saveShoppingItems();

    }


    /*
     お出かけから
     この持ち物だけ削除
    */

    outing.items =
        outing.items.filter(

            item =>
                item.id !==
                outingItem.id

        );


    outing.updatedAt =
        new Date().toISOString();


    saveOutings();


    /*
     表示更新
    */

    updateOutingCounts();

    updateOutingViewButtons();

    displayOutingItemsByCurrentView();

}

/* ==========================================
   お出かけ
   持ち物を購入予定へ追加
   ========================================== */

function addOutingItemToShopping(
    itemId
) {

    if (!currentOutingId) {

        return;

    }


    const outing =
        outings.find(

            outing =>
                outing.id ===
                currentOutingId

        );


    if (!outing) {

        return;

    }


    const outingItem =
        outing.items.find(

            item =>
                item.id ===
                itemId

        );


    if (!outingItem) {

        return;

    }


    /*
     すでにこの持ち物から
     購入予定へ追加済みか確認
    */

    const alreadyLinked =
        shoppingItems.find(

            shoppingItem =>

                Array.isArray(
                    shoppingItem.outingLinks
                ) &&

                shoppingItem.outingLinks.some(

                    link =>

                        link.outingId ===
                        outing.id &&

                        link.outingItemId ===
                        outingItem.id

                )

        );


    if (alreadyLinked) {

        alert(
            t(
                "outing.message.alreadyAddedToShopping"
            )
        );

        return;
    }

    /*
     登録済み商品を取得

     productIdがある場合
     → productIdを最優先

     productIdがない古いデータなど
     → 商品名一致で探す
    */

    const registeredProduct =

        outingItem.productId

            ? products.find(

                product =>

                    product.active !== false &&

                    product.id ===
                    outingItem.productId

            )

            : products.find(

                product =>

                    product.active !== false &&

                    product.name.trim() ===
                    outingItem.name.trim()

            );

    /*
     同じ商品がすでに購入予定にあるか
     確認
    */

    let shoppingItem =
        null;


    if (registeredProduct) {

        shoppingItem =
            shoppingItems.find(

                item =>
                    item.productId ===
                    registeredProduct.id

            );

    } else {

        shoppingItem =
            shoppingItems.find(

                item =>

                    !item.productId &&

                    (
                        item.productName ||
                        ""
                    ).trim() ===
                    outingItem.name.trim()

            );

    }


    const now =
        new Date().toISOString();


    /*
     すでに購入予定にある場合
     お出かけとの関連だけ追加
    */

    if (shoppingItem) {

        if (
            !Array.isArray(
                shoppingItem.outingLinks
            )
        ) {

            shoppingItem.outingLinks =
                [];

        }


        shoppingItem.outingLinks.push({

            outingId:
                outing.id,

            outingItemId:
                outingItem.id

        });


        /*
         購入前へ戻す
        */

        shoppingItem.status =
            "pending";

        shoppingItem.holdStartedAt =
            null;


        /*
         お出かけ側の数量を反映
        */

        shoppingItem.quantity =
            outingItem.quantity ||
            shoppingItem.quantity ||
            1;


        shoppingItem.updatedAt =
            now;

    } else {

        /*
         新しく購入予定へ追加
        */

        shoppingItem = {

            id:
                "shopping-" +
                Date.now() +
                "-" +
                Math.random()
                    .toString(36)
                    .slice(2, 8),

            productId:
                registeredProduct
                    ? registeredProduct.id
                    : null,

            productName:
                registeredProduct
                    ? undefined
                    : outingItem.name,

            quantity:
                outingItem.quantity ||
                1,

            needsRegistration:
                registeredProduct
                    ? false
                    : true,

            status:
                "pending",

            holdStartedAt:
                null,

            outingLinks: [

                {

                    outingId:
                        outing.id,

                    outingItemId:
                        outingItem.id

                }

            ],

            createdAt:
                now,

            updatedAt:
                now

        };


        shoppingItems.push(
            shoppingItem
        );

    }


    /*
     購入予定保存
    */

    saveShoppingItems();


    /*
     お出かけ側にも
     購入予定IDを記録
    */

    outingItem.shoppingItemId =
        shoppingItem.id;

    outingItem.updatedAt =
        now;

    outing.updatedAt =
        now;


    saveOutings();


    alert(

        t(
            "outing.message.addedToShopping"
        ).replace(
            "{name}",
            outingItem.name
        )
    );

}

/* ==========================================
   お出かけ
   購入完了した持ち物を準備済へ
   ========================================== */

function completeOutingItemsFromShopping(
    shoppingItem
) {

    if (!shoppingItem) {

        return;

    }


    let changed =
        false;

    const now =
        new Date().toISOString();


    /*
     方法①
     outingLinksから
     元のお出かけ持ち物を探す
    */

    if (
        Array.isArray(
            shoppingItem.outingLinks
        )
    ) {

        for (
            const link of
            shoppingItem.outingLinks
        ) {

            const outing =
                outings.find(

                    outing =>
                        outing.id ===
                        link.outingId

                );


            if (!outing) {

                continue;

            }


            const outingItem =
                outing.items.find(

                    item =>
                        item.id ===
                        link.outingItemId

                );


            if (!outingItem) {

                continue;

            }


            /*
             購入完了
             → 準備済
            */

            outingItem.status =
                "ready";

            outingItem.readySource =
                "shopping";

            outingItem.shoppingItemId =
                null;

            outingItem.updatedAt =
                now;

            outing.updatedAt =
                now;


            changed =
                true;

        }

    }


    /*
     方法②
     shoppingItemIdからも探す

     outingLinksが取れない場合の
     安全対策
    */

    for (
        const outing of
        outings
    ) {

        if (
            !Array.isArray(
                outing.items
            )
        ) {

            continue;

        }


        for (
            const outingItem of
            outing.items
        ) {

            if (
                outingItem.shoppingItemId !==
                shoppingItem.id
            ) {

                continue;

            }


            /*
             購入完了
             → 準備済
            */

            outingItem.status =
                "ready";

            outingItem.readySource =
                "shopping";

            outingItem.shoppingItemId =
                null;

            outingItem.updatedAt =
                now;

            outing.updatedAt =
                now;


            changed =
                true;

        }

    }


    /*
     お出かけデータ保存
    */

    if (changed) {

        saveOutings();

    }

}

/* ==========================================
   お出かけ
   終了して過去へ移動
   ========================================== */

function completeCurrentOuting() {

    if (!currentOutingId) {

        return;

    }


    const outing =
        outings.find(

            outing =>
                outing.id ===
                currentOutingId

        );


    if (!outing) {

        return;

    }


    /*
     「いつも使う」は
     過去へ移動させない
    */

    if (
        outing.status ===
        "routine"
    ) {

        return;

    }


    /*
     すでに終了している場合も
     何もしない
    */

    if (
        outing.status ===
        "completed"
    ) {

        return;

    }


    const confirmed =
        confirm(

            t(
                "outing.confirm.complete"
            ).replace(
                "{name}",
                outing.name
            )
        );


    if (!confirmed) {

        return;

    }


    /*
     過去へ移動
    */

    outing.status =
        "completed";


    /*
     終了日時も保存
    */

    outing.completedAt =
        new Date().toISOString();


    outing.updatedAt =
        new Date().toISOString();


    saveOutings();


    /*
     開いているお出かけを解除
    */

    currentOutingId =
        null;


    /*
     過去タブへ自動で切り替える
    */

    outingListViewMode =
        "past";


    openCheck();

}

/* ==========================================
   マスター管理画面から戻る
   ========================================== */

function backFromMasterScreen() {

    hideAllScreens();

    /*
     商品登録画面から管理画面を開いた場合
    */

    if (masterReturnScreen === "product") {

        document.getElementById("productScreen").hidden =
            false;

        /*
         管理画面で追加・変更した候補を
         選択ボックスへ再表示
        */

        displayCategorySelect();

        populateStoreCombo();

        /*
         商品ジャンルの選択値を戻す
        */

        const cmbCategory =
            document.getElementById("cmbCategory");

        if (
            cmbCategory &&
            productSelectedCategoryId
        ) {

            const categoryExists =
                Array.from(cmbCategory.options).some(

                    option =>
                        option.value ===
                        productSelectedCategoryId

                );

            if (categoryExists) {

                cmbCategory.value =
                    productSelectedCategoryId;

            }

        }

        /*
         購入先の選択値を戻す
        */

        const cmbStore =
            document.getElementById("cmbStore");

        if (
            cmbStore &&
            productSelectedStoreId
        ) {

            const storeExists =
                Array.from(cmbStore.options).some(

                    option =>
                        option.value ===
                        productSelectedStoreId

                );

            if (storeExists) {

                cmbStore.value =
                    productSelectedStoreId;

            }

        }

        return;

    }


    /*
     今回の購入画面から
     購入先管理を開いた場合
    */

    if (
        masterReturnScreen ===
        "priceRecord"
    ) {

        if (!recordingProductId) {

            openHome();

            return;

        }


        /*
         今回の購入画面を再表示
        */

        openPriceRecord(
            recordingProductId
        );


        /*
         今回の購入
         購入先候補を作り直す
        */
        displayPriceRecordStoreOptions();


        /*
         購入先を戻す
        */

        const cmbPriceRecordStore =
            document.getElementById(
                "cmbPriceRecordStore"
            );

        if (
            cmbPriceRecordStore &&
            priceRecordTempStoreId
        ) {

            const storeExists =
                Array.from(
                    cmbPriceRecordStore.options
                ).some(

                    option =>
                        option.value ===
                        priceRecordTempStoreId

                );

            if (storeExists) {

                cmbPriceRecordStore.value =
                    priceRecordTempStoreId;

            }

        }


        /*
         今回価格を戻す
        */

        const txtPriceRecordPrice =
            document.getElementById(
                "txtPriceRecordPrice"
            );

        if (txtPriceRecordPrice) {

            txtPriceRecordPrice.value =
                priceRecordTempPrice;

        }


        /*
         購入数量を戻す
        */

        const txtPriceRecordQuantity =
            document.getElementById(
                "txtPriceRecordQuantity"
            );

        if (txtPriceRecordQuantity) {

            txtPriceRecordQuantity.value =
                priceRecordTempQuantity || "1";

        }


        return;

    }


    /*
     設定画面から管理画面を開いた場合
    */

    document.getElementById(
        "settingsScreen"
    ).hidden = false;

}

function saveProduct() {

    const txtProductName = document.getElementById("txtProductName");
    const txtJanCode = document.getElementById("txtJanCode");
    const productMessage = document.getElementById("productMessage");

    const productName = txtProductName.value.trim();
    const janCode = txtJanCode.value.trim();

    const cmbCategory = document.getElementById("cmbCategory");
    const txtVolume = document.getElementById("txtVolume");
    const cmbUnit = document.getElementById("cmbUnit");


    const categoryId = cmbCategory.value;
    const volume = txtVolume.value.trim();
    const unit = cmbUnit.value;

    const txtPrice = document.getElementById("txtPrice");
    const price = txtPrice.value.trim();
    const cmbStore = document.getElementById("cmbStore");
    const storeId = cmbStore.value;
    /*
     内容量と単位の組み合わせチェック
    */

    if (
        volume !== "" &&
        unit === ""
    ) {

        productMessage.textContent =
            t(
                "product.validation.volumeNeedsUnit"
            );

        cmbUnit.focus();

        return;

    }

    if (
        volume === "" &&
        unit !== ""
    ) {

        productMessage.textContent =
            t(
                "product.validation.unitNeedsVolume"
            );

        txtVolume.focus();

        return;

    }

    if (productName === "") {

        productMessage.textContent =
            t(
                "product.validation.nameRequired"
            );

        return;

    }

    if (categoryId === "") {

        productMessage.textContent =
            t(
                "product.validation.categoryRequired"
            );

        document.getElementById(
            "cmbCategory"
        ).focus();

        return;

    }

    if (storeId === "") {

        productMessage.textContent =
            t(
                "product.validation.storeRequired"
            );

        document.getElementById(
            "cmbStore"
        ).focus();

        return;

    }

    if (
        price !== "" &&
        Number(price) < 0
    ) {

        productMessage.textContent =
            t(
                "product.validation.priceInvalid"
            );

        txtPrice.focus();

        return;

    }

    let product;

    if (editingProductId === null) {

        /*
         商品コード重複チェック
        */


        /*
         登録済み商品に
         同じ商品コードがあるか確認
        */

        const existingProduct =
            products.find(

                product =>

                    product.active &&

                    janCode !== "" &&

                    (
                        product.janCode === janCode ||
                        product.jan === janCode
                    )

            );


        if (existingProduct) {

            const message =
                document.getElementById(
                    "productMessage"
                );

            if (message) {

                message.textContent =
                    t(
                        "product.message.alreadyRegistered"
                    );

            }

            alert(
                t(
                    "product.confirm.alreadyRegistered"
                ).replace(
                    "{name}",
                    existingProduct.name
                )
            );

            return;

        }


        /*
         削除済み商品に
         同じ商品コードがあるか確認
        */

        const deletedProduct =
            products.find(

                product =>

                    !product.active &&

                    janCode !== "" &&

                    (
                        product.janCode === janCode ||
                        product.jan === janCode
                    )

            );


        if (deletedProduct) {

            const restoreConfirmed =
                confirm(

                    t(
                        "product.confirm.deletedFound"
                    ).replace(
                        "{name}",
                        deletedProduct.name
                    )

                );


            if (!restoreConfirmed) {

                const message =
                    document.getElementById(
                        "productMessage"
                    );

                if (message) {

                    message.textContent =
                        t(
                            "product.message.registrationCancelled"
                        );

                }

                return;

            }


            /*
             削除済み商品を復元
            */

            restoreProduct(
                deletedProduct.id
            );


            /*
             復元した商品の判断画面へ
            */

            openRegisteredJanInfo(

                deletedProduct.id,

                "product"

            );

            return;

        }


        product = {

            id: createProductId(),

            name: productName,

            janCode: janCode,

            categoryId: categoryId,

            volume: volume,

            unit: unit,

            favorite:
                false,

            price: price,

            storeId: storeId,

            createdAt: new Date().toISOString(),

            updatedAt: new Date().toISOString(),

            active: true

        };

        addProduct(product);


        /*
         商品登録の価格計算情報をリセット
        */

        productPriceCalculation =
            null;

    } else {

        const oldProduct =
            products.find(

                p =>
                    p.id ===
                    editingProductId

            );


        if (!oldProduct) {

            return;

        }


        /*
         更新する前に
         変更前の価格・購入先と比較する
        */

        const priceChanged =
            Number(oldProduct.price) !==
            Number(price);

        const storeChanged =
            oldProduct.storeId !==
            storeId;


        /*
         新しい商品データを作る
        */

        product = {

            ...oldProduct,

            name:
                productName,

            janCode:
                janCode,

            categoryId:
                categoryId,

            volume:
                volume,

            unit:
                unit,

            price:
                price,

            storeId:
                storeId,

            updatedAt:
                new Date().toISOString()

        };


        /*
         商品データを更新
        */

        updateProduct(
            product
        );


        /*
         商品登録から価格・購入先を変更した場合

         履歴がある
         → 最新履歴を更新

         履歴がない
         → 新しい履歴を1件作成
        */

        if (
            price !== "" &&
            storeId !== "" &&
            (
                priceChanged ||
                storeChanged
            )
        ) {

            const latestHistory =
                getLatestPriceHistory(
                    product.id
                );


            /*
             すでに購入履歴がある場合
             最新履歴を更新する
            */

            if (latestHistory) {

                const updateData = {

                    storeId:
                        storeId,

                    price:
                        price

                };


                /*
                 価格計算を使用した場合
                 税抜価格・税率なども更新
                */

                if (productPriceCalculation) {

                    updateData.originalPrice =
                        productPriceCalculation.originalPrice;

                    updateData.priceType =
                        productPriceCalculation.priceType;

                    updateData.taxRate =
                        productPriceCalculation.taxRate;

                    updateData.discountType =
                        productPriceCalculation.discountType;

                    updateData.discountValue =
                        productPriceCalculation.discountValue;

                }


                updatePriceHistory(

                    latestHistory.id,

                    updateData

                );

            }

        }


        /*
         商品登録の価格計算情報をリセット
        */

        productPriceCalculation =
            null;


        editingProductId = null;

    }

    document.getElementById("txtProductName").value = "";

    document.getElementById("cmbCategory").selectedIndex = 0;

    document.getElementById("txtVolume").value = "";

    document.getElementById("cmbUnit").selectedIndex = 0;

    document.getElementById("txtPrice").value = "";

    document.getElementById("cmbStore").selectedIndex = 0;

    const volumeExample =
        document.getElementById(
            "volumeExample"
        );

    if (volumeExample) {

        volumeExample.hidden = true;

    }

    if (navigator.onLine) {

        productMessage.textContent =
            t(
                "product.message.saved"
            );

    } else {

        productMessage.textContent =
            t(
                "product.message.savedOffline"
            );

    }

    displayProducts();

    /*
     購入予定の商品選択から
     商品登録へ来た場合
    */

    if (
        productRegistrationReturnMode ===
        "shoppingAdd"
    ) {

        /*
         商品登録画面を閉じる
        */

        const productScreen =
            document.getElementById(
                "productScreen"
            );

        if (productScreen) {

            productScreen.hidden =
                true;

        }


        /*
         購入予定画面へ戻す
        */

        const shoppingScreen =
            document.getElementById(
                "shoppingScreen"
            );

        if (shoppingScreen) {

            shoppingScreen.hidden =
                false;

        }


        /*
         ＋追加画面を表示
        */

        changeShoppingView(
            "add"
        );


        /*
         登録した商品が
         現在の絞り込みで消えないように
         全商品表示へ戻す
        */

        shoppingProductFilter =
            "all";


        /*
         商品プルダウンを
         最新状態へ作り直す
        */

        displayShoppingProductOptions();


        /*
         登録した商品を
         自動選択
        */

        const productSelect =
            document.getElementById(
                "cmbShoppingProduct"
            );

        if (
            productSelect &&
            product
        ) {

            productSelect.value =
                product.id;

        }


        /*
         商品コード欄にも反映
        */

        const janInput =
            document.getElementById(
                "txtShoppingJanCode"
            );

        if (
            janInput &&
            product
        ) {

            janInput.value =
                product.janCode ||
                product.jan ||
                "";

        }


        /*
         メッセージ表示
        */

        const shoppingMessage =
            document.getElementById(
                "shoppingMessage"
            );

        if (
            shoppingMessage &&
            product
        ) {

            shoppingMessage.textContent =
                t(
                    "product.message.registeredSelected"
                ).replace(
                    "{name}",
                    product.name
                );

        }


        /*
         次回の商品登録のため
         商品登録フォームを初期化
        */

        cancelProductEdit();


        /*
         戻り先を通常へ戻す
        */

        productRegistrationReturnMode =
            "normal";


        /*
         商品選択欄へ移動
        */

        if (productSelect) {

            productSelect.scrollIntoView({

                behavior:
                    "smooth",

                block:
                    "center"

            });

        }


        return;

    }

    /*
     お出かけの商品選択から
     商品登録へ来た場合
    */

    if (
        productRegistrationReturnMode ===
        "outingAdd"
    ) {

        /*
         商品登録画面を閉じる
        */

        const productScreen =
            document.getElementById(
                "productScreen"
            );


        if (productScreen) {

            productScreen.hidden =
                true;

        }


        /*
         お出かけチェック画面へ戻す
        */

        const outingCheckScreen =
            document.getElementById(
                "outingCheckScreen"
            );


        if (outingCheckScreen) {

            outingCheckScreen.hidden =
                false;

        }


        /*
         既存の「購入する物」から
         商品を選んでいた場合は
         商品選択専用モードを維持
        */

        if (outingLinkTargetItemId) {

            outingProductSelectMode =
                true;

        }


        /*
         商品選択画面を再表示
        */

        outingViewMode =
            "add";


        updateOutingViewButtons();

        displayOutingItemsByCurrentView();


        /*
         登録した商品のジャンルを
         自動選択
        */

        const categorySelect =
            document.getElementById(
                "cmbOutingProductCategory"
            );


        if (
            categorySelect &&
            product &&
            product.categoryId
        ) {

            categorySelect.value =
                product.categoryId;

        }


        /*
         選択したジャンルの商品だけに
         商品プルダウンを作り直す
        */

        displayOutingProductOptions();


        /*
         登録した商品を
         自動選択
        */

        const productSelect =
            document.getElementById(
                "cmbOutingAddProduct"
            );


        if (
            productSelect &&
            product
        ) {

            productSelect.value =
                product.id;

        }


        /*
         メッセージ表示
        */

        const outingMessage =
            document.getElementById(
                "outingAddProductMessage"
            );


        if (
            outingMessage &&
            product
        ) {

            outingMessage.textContent =
                t(
                    "product.message.registeredSelected"
                ).replace(
                    "{name}",
                    product.name
                );

        }


        /*
         次回の商品登録のため
         商品登録フォームを初期化
        */

        cancelProductEdit();


        /*
         戻り先を通常へ戻す
        */

        productRegistrationReturnMode =
            "normal";


        /*
         商品選択欄へ移動
        */

        if (productSelect) {

            productSelect.scrollIntoView({

                behavior:
                    "smooth",

                block:
                    "center"

            });

        }


        return;

    }

}

/* ==========================================
   登録済み商品 ジャンル絞り込み
   ========================================== */

function displayProductFilterCategories() {

    const select =
        document.getElementById(
            "cmbProductFilterCategory"
        );

    if (!select) {

        return;

    }

    const currentValue =
        productFilterCategoryId;

    select.innerHTML = `

        <option value="">
            ${t("product.list.allCategories")}
        </option>

    `;

    for (const category of categories) {

        if (!category.active) {

            continue;

        }

        const option =
            document.createElement(
                "option"
            );

        option.value =
            category.id;

        const categoryDisplayName =
            typeof getCategoryDisplayName ===
                "function"

                ? getCategoryDisplayName(
                    category
                )

                : category.name;

        option.textContent =
            t(
                "product.list.categoryPrefix"
            ) +
            categoryDisplayName;

        select.appendChild(
            option
        );

    }

    select.value =
        currentValue;

}

/* ==========================================
   よく使う商品 ON / OFF
   ========================================== */

function toggleFavoriteProduct(
    productId
) {

    const product =
        products.find(

            product =>
                product.id ===
                productId

        );


    if (!product) {

        return;

    }


    /*
     true ⇔ false
    */

    product.favorite =
        product.favorite === true
            ? false
            : true;


    product.updatedAt =
        new Date().toISOString();


    /*
     商品データを保存
    */

    saveProducts();


    /*
     商品登録一覧を更新
    */

    displayProducts();


    /*
     履歴・価格比較の商品ごとも
     表示中なら更新
    */

    if (
        typeof displayPriceHistoryByProduct ===
        "function"
    ) {

        displayPriceHistoryByProduct();

    }


    /*
     購入予定で
     「よく使う」を表示中なら更新
    */

    if (
        typeof shoppingProductFilter !==
        "undefined" &&
        shoppingProductFilter ===
        "favorite"
    ) {

        displayShoppingProductOptions();

    }

}

/* ==========================================
   登録済み商品一覧
   年月別・開閉表示
   ========================================== */

function displayProducts() {

    const productList =
        document.getElementById("productList");

    if (!productList) {

        return;

    }


    /* ==========================================
       削除済み商品 件数更新
       ========================================== */

    const btnDeletedProducts =
        document.getElementById(
            "btnDeletedProducts"
        );

    const deletedProductCount =
        products.filter(

            product =>
                !product.active

        ).length;

    if (btnDeletedProducts) {

        btnDeletedProducts.textContent =
            t(
                "product.deleted.button"
            ).replace(
                "{count}",
                deletedProductCount
            );

    }


    productList.innerHTML = "";

    /*
     有効な商品だけを取得し、
     登録日の新しい順へ並べる
    */

    const activeProducts = products

        .filter(

            product => {

                if (!product.active) {

                    return false;

                }

                if (
                    productFilterCategoryId !== "" &&
                    product.categoryId !==
                    productFilterCategoryId
                ) {

                    return false;

                }

                return true;

            }

        )

        .sort((productA, productB) => {

            return new Date(productB.createdAt) -
                new Date(productA.createdAt);

        });

    const productFilterCount =
        document.getElementById(
            "productFilterCount"
        );

    if (productFilterCount) {

        const productCountKey =
            activeProducts.length === 1
                ? "product.list.countOne"
                : "product.list.count";

        productFilterCount.textContent =
            t(
                productCountKey
            ).replace(
                "{count}",
                activeProducts.length
            );

    }

    if (activeProducts.length === 0) {

        productList.innerHTML = `

        <p class="empty-product-message">

            ${t(
            "product.list.empty"
        )}

        </p>

    `;

        return;

    }

    /*
     登録年月ごとに商品をまとめる
     キーの例：2026-08
    */

    const monthlyProducts = {};

    for (const product of activeProducts) {

        const createdDate =
            new Date(product.createdAt);

        /*
         登録日のない旧データにも対応
        */

        const validDate =
            Number.isNaN(createdDate.getTime())
                ? new Date()
                : createdDate;

        const monthKey =

            validDate.getFullYear() +

            "-" +

            String(
                validDate.getMonth() + 1
            ).padStart(2, "0");

        if (!monthlyProducts[monthKey]) {

            monthlyProducts[monthKey] = [];

        }

        monthlyProducts[monthKey].push(product);

    }

    /*
     年月を新しい順に並べる
    */

    const monthKeys =
        Object.keys(monthlyProducts).sort().reverse();

    monthKeys.forEach((monthKey, monthIndex) => {

        const [year, month] =
            monthKey.split("-");

        const monthProducts =
            monthlyProducts[monthKey];

        /*
         最新月だけ最初から開く
        */

        const isOpen =
            monthIndex === 0;

        const monthSection =
            document.createElement("section");

        monthSection.className =
            "product-month-group";

        monthSection.innerHTML = `

            <button
                type="button"
                class="product-month-header"
                aria-expanded="${isOpen}"
                onclick="toggleProductMonth(this)">

                <span class="product-month-arrow">

                    ${isOpen ? "▼" : "▶"}

                </span>

                <span class="product-month-title">

        ${t(
            "product.list.month"
        )
                .replace(
                    "{year}",
                    Number(year)
                )
                .replace(
                    "{month}",
                    Number(month)
                )}

        </span>

        <span class="product-month-count">

            ${t(
                    monthProducts.length === 1
                        ? "product.list.countOne"
                        : "product.list.count"
                ).replace(
                    "{count}",
                    monthProducts.length
                )}

        </span>

            </button>

            <div
                class="product-month-content"
                ${isOpen ? "" : "hidden"}>

            </div>

            `;

        const monthContent =
            monthSection.querySelector(
                ".product-month-content"
            );

        for (const product of monthProducts) {

            const createdDate =
                new Date(product.createdAt);

            const validDate =
                Number.isNaN(createdDate.getTime())
                    ? new Date()
                    : createdDate;

            const dateText =

                validDate.getFullYear() +

                "/" +

                String(
                    validDate.getMonth() + 1
                ).padStart(2, "0") +

                "/" +

                String(
                    validDate.getDate()
                ).padStart(2, "0");

            const productCard =
                document.createElement("div");

            productCard.className =
                "product-card";


            /*
             商品の購入先から
             国・通貨を取得
            */

            const productStore =
                product.storeId
                    ? findStore(
                        product.storeId
                    )
                    : null;

            const productCountry =
                productStore
                    ? productStore.country || "JP"
                    : getCocartlyBaseCountry();

            const productCurrency =
                getCurrencyByCountry(
                    productCountry
                );


            /*
             通貨記号
            */

            const currencySymbols = {

                JPY: "¥",
                USD: "$",
                CAD: "C$",
                AUD: "A$",
                KRW: "₩",
                CNY: "¥",
                TWD: "NT$"

            };

            const productCurrencySymbol =
                currencySymbols[
                productCurrency
                ] || "¥";


            productCard.innerHTML = `

    <h3>${product.name}</h3>

    <p>
        <span class="product-label">
            ${t("product.list.code")}
        </span>
        <span class="product-value">
            ${product.janCode || "-"}
        </span>
    </p>

        <p>
        <span class="product-label">
            ${t("product.list.category")}
        </span>
        <span class="product-value">
            ${(() => {

                    const category =
                        categories.find(
                            category =>
                                category.id ===
                                product.categoryId
                        );

                    return category
                        ? getCategoryDisplayName(
                            category
                        )
                        : "-";

                })()}
        </span>
    </p>

    <p>
        <span class="product-label">
            ${t("product.list.store")}
        </span>
        <span class="product-value">
            ${getStoreDisplayName(product.storeId)}
        </span>
    </p>

    <p>
        <span class="product-label">
            ${t("product.list.volume")}
        </span>
        <span class="product-value">
            ${product.volume || "-"}${product.unit || ""}
        </span>
    </p>

    <p>
        <span class="product-label">
            ${t("product.list.price")}
        </span>
        <span class="product-value">
            ${product.price !== undefined &&
                    product.price !== null &&
                    product.price !== ""
                    ? productCurrencySymbol +
                    product.price
                    : "-"
                }
        </span>
    </p>

    <p>
        <span class="product-label">
            ${t("product.list.registeredDate")}
        </span>
        <span class="product-value">
            ${dateText}
        </span>
    </p>
                <div class="product-buttons">

                    <button
                        type="button"
                        class="favorite-product-button ${product.favorite === true ? "active" : ""}"
                        onclick="toggleFavoriteProduct('${product.id}')">

                       ${product.favorite === true
                    ? "⭐ " +
                    t(
                        "product.list.favorite"
                    )
                    : "☆ " +
                    t(
                        "product.list.favorite"
                    )}

                    </button>


                    <button
                        type="button"
                        class="product-price-button"
                        onclick="openRegisteredJanInfo('${product.id}', 'product')">

                        ${t(
                        "product.list.checkPrice"
                    )}

                    </button>


                    <div class="product-sub-buttons">

                        <button
                            type="button"
                            class="product-edit-button"
                            onclick="editProduct('${product.id}')">

                            ${t(
                        "product.list.edit"
                    )}

                        </button>

                    <button
                        type="button"
                        class="product-delete-button"
                        onclick="deleteProduct('${product.id}')">

                        ${t(
                        "product.list.delete"
                    )}

                    </button>

                </div>

            </div>

            `;

            monthContent.appendChild(productCard);

        }

        productList.appendChild(monthSection);

    });

}

/* ==========================================
   商品年月グループ開閉
   ========================================== */

function toggleProductMonth(button) {

    const monthGroup =
        button.closest(".product-month-group");

    if (!monthGroup) {

        return;

    }

    const content =
        monthGroup.querySelector(
            ".product-month-content"
        );

    const arrow =
        button.querySelector(
            ".product-month-arrow"
        );

    if (!content || !arrow) {

        return;

    }

    const willOpen =
        content.hidden;

    content.hidden =
        !willOpen;

    arrow.textContent =
        willOpen ? "▼" : "▶";

    button.setAttribute(
        "aria-expanded",
        String(willOpen)
    );

}

/* ==========================================
   削除済みアイテム画面
   ========================================== */

function openDeletedProducts() {

    document.getElementById("productScreen").hidden = true;

    document.getElementById("deletedProductScreen").hidden = false;


    /*
     削除済みアイテム画面
     表示言語更新
    */
    const deletedProductScreen =
        document.getElementById(
            "deletedProductScreen"
        );

    if (deletedProductScreen) {

        const title =
            deletedProductScreen.querySelector(
                "header h2"
            );

        if (title) {

            title.textContent =
                "🗑 " +
                t(
                    "product.deleted.title"
                );

        }

    }


    const btnBackProduct =
        document.getElementById(
            "btnBackProduct"
        );

    if (btnBackProduct) {

        btnBackProduct.textContent =
            t(
                "product.deleted.back"
            );

    }


    displayDeletedProducts();

}

function backProduct() {

    document.getElementById("deletedProductScreen").hidden = true;

    document.getElementById("productScreen").hidden = false;

    displayProducts();

}

/* ==========================================
   コード読取画面
   表示言語更新
   ========================================== */

function updateCodeScreenLanguage() {

    const screen =
        document.getElementById(
            "codeScreen"
        );

    if (!screen) {
        return;
    }

    const backButton =
        document.getElementById(
            "btnBackCode"
        );

    if (backButton) {
        backButton.textContent =
            t("code.backHome");
    }

    const title =
        screen.querySelector(
            "header h2"
        );

    if (title) {
        title.textContent =
            t("code.title");
    }

    const guide =
        screen.querySelector(
            "header + p"
        );

    if (guide) {
        guide.textContent =
            t("code.guide");
    }

    const successText =
        screen.querySelector(
            "#scanSuccess p"
        );

    if (successText) {
        successText.textContent =
            t("code.success");
    }

    const scanStatus =
        document.getElementById(
            "scanStatus"
        );

    if (scanStatus) {
        scanStatus.textContent =
            t("code.waiting");
    }

    const flashButton =
        document.getElementById(
            "btnFlash"
        );

    if (flashButton) {
        flashButton.textContent =
            t("code.flash");
    }

    const inputButton =
        document.getElementById(
            "btnJanInput"
        );

    if (inputButton) {
        inputButton.textContent =
            t("code.manualInput");
    }

    const cancelButton =
        document.getElementById(
            "btnCancelScan"
        );

    if (cancelButton) {
        cancelButton.textContent =
            t("code.backHome");
    }

}

/* ==========================================
   コード読取画面
   ========================================== */

function openCode() {

    /*
     ホームから開く場合は
     必ず通常のバーコード読取
    */

    barcodeReadMode =
        "normal";


    /*
     一度すべての画面を閉じる
    */

    hideAllScreens();


    /*
     コード読取画面だけ表示
    */

    const codeScreen =
        document.getElementById(
            "codeScreen"
        );

    if (!codeScreen) {

        return;

    }

    codeScreen.hidden =
        false;


    /*
     コード読取画面を
     現在の表示言語へ更新
    */

    if (
        typeof updateCodeScreenLanguage ===
        "function"
    ) {

        updateCodeScreenLanguage();

    }


    /*
     ホームから開いた場合の
     戻るボタンを確実に更新
    */

    const btnBackCode =
        document.getElementById(
            "btnBackCode"
        );

    const btnCancelScan =
        document.getElementById(
            "btnCancelScan"
        );


    if (btnBackCode) {

        btnBackCode.textContent =
            t("code.backHome");

    }


    if (btnCancelScan) {

        btnCancelScan.textContent =
            t("code.backHome");

    }


    /*
     カメラ開始
    */

    readCode();

}

function backHomeFromCode() {

    /*
     カメラを停止
    */

    if (
        typeof stopBarcodeReader ===
        "function"
    ) {

        stopBarcodeReader();

    }


    /*
 今回の購入から
 商品コード読取を開いた場合
*/

    if (
        barcodeReadMode ===
        "priceRecordJan"
    ) {

        /*
         通常読取へ戻す
        */

        barcodeReadMode =
            "normal";


        /*
         一度すべての画面を閉じる
        */

        hideAllScreens();


        /*
         今回の購入画面へ戻る
        */

        const priceRecordScreen =
            document.getElementById(
                "priceRecordScreen"
            );

        if (!priceRecordScreen) {

            return;

        }

        priceRecordScreen.hidden =
            false;

        return;

    }


    /*
     購入予定＋追加から
     商品コード読取を開いた場合
    */

    if (
        barcodeReadMode ===
        "shoppingAddJan"
    ) {

        /*
         通常読取へ戻す
        */

        barcodeReadMode =
            "normal";


        /*
         一度すべての画面を閉じる
        */

        hideAllScreens();


        /*
         購入予定画面へ戻る
        */

        const shoppingScreen =
            document.getElementById(
                "shoppingScreen"
            );

        if (!shoppingScreen) {

            return;

        }

        shoppingScreen.hidden =
            false;


        /*
         ＋追加画面を表示
        */

        changeShoppingView(
            "add"
        );


        return;

    }


    /*
     お出かけチェックから
     商品コード読取を開いた場合
    */

    if (
        barcodeReadMode ===
        "outingAddJan"
    ) {

        /*
         通常読取へ戻す
        */

        barcodeReadMode =
            "normal";


        /*
         一度すべての画面を閉じる
        */

        hideAllScreens();


        /*
         お出かけチェック画面へ戻る
        */

        const outingCheckScreen =
            document.getElementById(
                "outingCheckScreen"
            );

        if (!outingCheckScreen) {
            return;
        }

        outingCheckScreen.hidden =
            false;


        /*
         追加画面を維持
        */

        outingViewMode =
            "add";

        updateOutingViewButtons();

        displayOutingItemsByCurrentView();


        return;
    }


    /*
     通常のコード読取の場合
     通常読取へ戻す
    */

    barcodeReadMode =
        "normal";


    /*
     ホーム画面へ戻る
    
     openHome() 内で
     表示言語も更新する
    */

    openHome();

}

/* ==========================================
   商品情報表示
   ========================================== */

function setProductInfo(product) {

    console.log("txtProductName =", document.getElementById("txtProductName"));
    console.log("txtVolume =", document.getElementById("txtVolume"));
    console.log("cmbUnit =", document.getElementById("cmbUnit"));

    console.log(product);

    document.getElementById("txtProductName").value =
        product.name;

    document.getElementById("txtJanCode").value =
        product.janCode ||
        product.jan ||
        "";

    document.getElementById("txtVolume").value =
        product.volume;

    document.getElementById("cmbUnit").value =
        product.unit;

}

function toggleFlash() {

    alert(
        t(
            "code.message.flashComingSoon"
        )
    );

}

function openJanInput() {

    alert(
        t(
            "code.message.manualInputComingSoon"
        )
    );

}

/* ==========================================
   全画面を閉じる
   ========================================== */

function hideAllScreens() {

    const screens =
        document.querySelectorAll("#app > section");

    for (const screen of screens) {

        screen.hidden = true;

    }

}

/* ==========================================
   設定画面
   ========================================== */

function openSettings() {

    hideAllScreens();

    /*
     現在の表示言語を
     設定画面へ反映
    */

    updateSettingsScreenLanguage();

    document.getElementById(
        "settingsScreen"
    ).hidden = false;


    /*
     保存されている表示言語を
     設定画面へ反映
    */

    const cmbSettingsLanguage =
        document.getElementById(
            "cmbSettingsLanguage"
        );

    if (cmbSettingsLanguage) {

        cmbSettingsLanguage.value =
            getCocartlyLanguage();

    }


    /*
     普段買い物する国・地域を
     設定画面へ反映
    */

    const cmbSettingsBaseCountry =
        document.getElementById(
            "cmbSettingsBaseCountry"
        );

    if (cmbSettingsBaseCountry) {

        cmbSettingsBaseCountry.value =
            getCocartlyBaseCountry();

    }

}

/* ==========================================
   About・プライバシー
   表示言語更新
   ========================================== */

function updateInformationScreenLanguage() {

    document
        .querySelectorAll(
            "#aboutCocartlyScreen [data-i18n], " +
            "#privacyPolicyScreen [data-i18n]"
        )
        .forEach(
            element => {

                const key =
                    element.getAttribute(
                        "data-i18n"
                    );

                if (key) {

                    element.textContent =
                        t(key);

                }

            }
        );
}


/* ==========================================
   Cocartlyについて
   ========================================== */

function openAboutCocartly() {

    hideAllScreens();

    updateInformationScreenLanguage();


    const screen =
        document.getElementById(
            "aboutCocartlyScreen"
        );


    if (screen) {

        screen.hidden =
            false;

    }


    window.scrollTo({

        top: 0,

        behavior:
            "smooth"

    });

}

/* ==========================================
   プライバシーポリシー
   ========================================== */

function openPrivacyPolicy() {

    hideAllScreens();

    updateInformationScreenLanguage();


    const screen =
        document.getElementById(
            "privacyPolicyScreen"
        );


    if (screen) {

        screen.hidden =
            false;

    }


    window.scrollTo({

        top: 0,

        behavior:
            "smooth"

    });

}

/* ==========================================
   Cocartly
   データバックアップ
   ========================================== */

function backupCocartlyData() {

    try {

        /*
         localStorageに保存されている
         Cocartlyデータもまとめて保存
        */

        const storageData = {};


        for (
            let i = 0;
            i < localStorage.length;
            i++
        ) {

            const key =
                localStorage.key(i);


            if (!key) {

                continue;

            }


            storageData[key] =
                localStorage.getItem(key);

        }

        /*
         Cocartlyで使用している
         データをまとめる
        */

        const backupData = {

            appName:
                "Cocartly",

            backupVersion:
                1,

            exportedAt:
                new Date().toISOString(),

            data: {

                /*
                 商品
                */

                products:
                    typeof products !== "undefined"
                        ? products
                        : [],


                /*
                 商品ジャンル
                */

                categories:
                    typeof categories !== "undefined"
                        ? categories
                        : [],


                /*
                 購入先
                */

                stores:
                    typeof stores !== "undefined"
                        ? stores
                        : [],


                /*
                 購入履歴・価格履歴
                */

                priceHistory:
                    typeof priceHistory !== "undefined"
                        ? priceHistory
                        : [],


                /*
                 購入予定
                */

                shoppingItems:
                    typeof shoppingItems !== "undefined"
                        ? shoppingItems
                        : [],


                /*
                 店頭確認履歴
                */

                storeChecks:
                    typeof storeChecks !== "undefined"
                        ? storeChecks
                        : [],


                /*
                 お出かけ
                */

                outings:
                    typeof outings !== "undefined"
                        ? outings
                        : [],


                /*
                 現在の買い物状態
                */

                shoppingSession:
                    typeof shoppingSession !== "undefined"
                        ? shoppingSession
                        : null

            },


            /*
             実際にブラウザへ
             保存されているデータ
            */

            localStorage:
                storageData

        };


        /*
         JSONファイルへ変換
        */

        const json =
            JSON.stringify(
                backupData,
                null,
                2
            );


        const blob =
            new Blob(
                [json],
                {
                    type:
                        "application/json"
                }
            );


        const url =
            URL.createObjectURL(
                blob
            );


        /*
         バックアップ日時
        */

        const now =
            new Date();


        const dateText =
            now.getFullYear() +
            "-" +
            String(
                now.getMonth() + 1
            ).padStart(
                2,
                "0"
            ) +
            "-" +
            String(
                now.getDate()
            ).padStart(
                2,
                "0"
            );


        const timeText =
            String(
                now.getHours()
            ).padStart(
                2,
                "0"
            ) +
            "-" +
            String(
                now.getMinutes()
            ).padStart(
                2,
                "0"
            );


        /*
         ダウンロード
        */

        const link =
            document.createElement(
                "a"
            );


        link.href =
            url;


        link.download =
            "cocartly-backup-" +
            dateText +
            "_" +
            timeText +
            ".json";


        document.body.appendChild(
            link
        );


        link.click();


        link.remove();


        /*
         一時URLを解放
        */

        setTimeout(
            () => {

                URL.revokeObjectURL(
                    url
                );

            },
            1000
        );


        alert(
            t("backup.message.created")
        );


    } catch (error) {

        console.error(
            "バックアップエラー：",
            error
        );


        alert(
            t("backup.message.failed")
        );

    }

}

/* ==========================================
   Cocartly
   バックアップから復元
   ========================================== */

function restoreCocartlyData() {

    /*
     ファイル選択欄を作る
    */

    const input =
        document.createElement(
            "input"
        );


    input.type =
        "file";


    input.accept =
        ".json,application/json";


    input.addEventListener(
        "change",
        () => {

            const file =
                input.files &&
                input.files[0];


            if (!file) {

                return;

            }


            const reader =
                new FileReader();


            reader.onload =
                () => {

                    try {

                        const backupData =
                            JSON.parse(
                                reader.result
                            );


                        /*
                         Cocartlyの
                         バックアップか確認
                        */

                        if (
                            !backupData ||
                            backupData.appName !==
                            "Cocartly" ||
                            !backupData.localStorage
                        ) {

                            alert(
                                t("restore.message.invalidFile")
                            );

                            return;

                        }


                        /*
                         最終確認
                        */

                        const ok =
                            confirm(
                                t("restore.confirm")
                            );


                        if (!ok) {

                            return;

                        }


                        /*
                         現在の保存データを消す
                        */

                        localStorage.clear();


                        /*
                         バックアップの
                         localStorageを復元
                        */

                        for (
                            const key in
                            backupData.localStorage
                        ) {

                            const value =
                                backupData.localStorage[
                                key
                                ];


                            if (
                                typeof value !==
                                "string"
                            ) {

                                continue;

                            }


                            localStorage.setItem(
                                key,
                                value
                            );

                        }


                        alert(
                            t("restore.message.completed")
                        );


                        /*
                         復元したデータを
                         読み直す
                        */

                        location.reload();


                    } catch (error) {

                        console.error(
                            "復元エラー：",
                            error
                        );


                        alert(
                            t("restore.message.readFailed")
                        );

                    }

                };


            reader.readAsText(
                file
            );

        }
    );


    /*
     ファイル選択を開く
    */

    input.click();

}

/* ==========================================
   ホーム画面
   表示言語更新
   ========================================== */

function updateHomeScreenLanguage() {

    /*
     PC・タブレット表示
    */

    const desktopOnlyMessage =
        document.querySelector(
            '[data-i18n="device.desktop.only"]'
        );

    const desktopUnavailableMessage =
        document.querySelector(
            '[data-i18n="device.desktop.unavailable"]'
        );

    if (desktopOnlyMessage) {

        desktopOnlyMessage.textContent =
            t("device.desktop.only");

    }

    if (desktopUnavailableMessage) {

        desktopUnavailableMessage.textContent =
            t("device.desktop.unavailable");

    }


    /*
     横向き案内
    */

    const orientationTitle =
        document.querySelector(
            '[data-i18n="device.orientation.title"]'
        );

    const orientationGuide =
        document.querySelector(
            '[data-i18n="device.orientation.guide"]'
        );

    if (orientationTitle) {

        orientationTitle.textContent =
            t("device.orientation.title");

    }

    if (orientationGuide) {

        orientationGuide.textContent =
            t("device.orientation.guide");

    }


    /*
     キャッチコピー
    */

    const homeCatch =
        document.querySelector(
            "#homeScreen .home-catch"
        );

    if (homeCatch) {
        homeCatch.textContent =
            t("home.catch");
    }


    /*
     コード読取
    */

    const codeLabel =
        document.querySelector(
            "#btnCode .home-code-label"
        );

    if (codeLabel) {
        codeLabel.textContent =
            t("home.code");
    }


    /*
     商品登録
    */

    const productLabel =
        document.querySelector(
            "#btnProduct .home-main-label"
        );

    if (productLabel) {
        productLabel.textContent =
            t("home.product");
    }


    /*
     購入予定
    */

    const shoppingLabel =
        document.querySelector(
            "#btnShopping .home-main-label"
        );

    if (shoppingLabel) {
        shoppingLabel.textContent =
            t("home.shopping");
    }


    /*
     履歴・価格比較
     ホームボタン用表示
    */

    const historyLabel =
        document.querySelector(
            "#btnHistory .home-main-label"
        );

    if (historyLabel) {

        const currentLanguage =
            getCocartlyLanguage();

        switch (currentLanguage) {

            case "ja":

                /*
                 日本語は元のデザインを維持
                */

                historyLabel.innerHTML =
                    "履歴・<br>価格比較";

                break;


            case "en":

                historyLabel.innerHTML =
                    "History<br>& Prices";

                break;


            case "ko":

                historyLabel.innerHTML =
                    "구매 내역<br>가격 비교";

                break;


            case "zh-CN":

                historyLabel.innerHTML =
                    "购买记录<br>价格比较";

                break;


            case "zh-TW":

                historyLabel.innerHTML =
                    "購買紀錄<br>價格比較";

                break;


            case "fr":

                historyLabel.innerHTML =
                    "Historique<br>des prix";

                break;


            default:

                historyLabel.textContent =
                    t("home.history");

                break;
        }
    }


    /*
     お出かけチェック
    */

    const outingLabel =
        document.querySelector(
            "#btnCheck .home-main-label"
        );

    if (outingLabel) {

        const currentLanguage =
            getCocartlyLanguage();

        if (currentLanguage === "ja") {

            /*
             日本語は元のデザインを維持
            */

            outingLabel.innerHTML =
                "お出かけ<br>チェック";

        } else {

            outingLabel.textContent =
                t("home.outing");

        }
    }


    /*
     お知らせ
    */

    const noticeButton =
        document.getElementById(
            "btnNotice"
        );

    if (noticeButton) {

        noticeButton.setAttribute(
            "aria-label",
            t("home.notice")
        );

        const noticeLabel =
            noticeButton
                .closest(
                    ".home-utility-item"
                )
                ?.querySelector(
                    ".home-utility-label"
                );

        if (noticeLabel) {
            noticeLabel.textContent =
                t("home.notice");
        }
    }


    /*
     設定
    */

    const settingsButton =
        document.getElementById(
            "btnSettings"
        );

    if (settingsButton) {

        settingsButton.setAttribute(
            "aria-label",
            t("home.settings")
        );

        const settingsLabel =
            settingsButton
                .closest(
                    ".home-utility-item"
                )
                ?.querySelector(
                    ".home-utility-label"
                );

        if (settingsLabel) {
            settingsLabel.textContent =
                t("home.settings");
        }
    }
}

/* ==========================================
   設定画面の表示言語を反映
   ========================================== */

function updateSettingsScreenLanguage() {

    const settingsScreen =
        document.getElementById(
            "settingsScreen"
        );

    if (!settingsScreen) {
        return;
    }


    /*
     戻るボタン
    */

    const backButton =
        document.getElementById(
            "btnBackSettings"
        );

    if (backButton) {

        backButton.textContent =
            t("settings.backHome");

    }


    /*
     画面タイトル
    */

    const title =
        settingsScreen.querySelector(
            ".screen-header h2"
        );

    if (title) {

        title.textContent =
            "⚙ " +
            t("settings.title");

    }


    /*
     設定グループタイトル
    */

    const groupTitles =
        settingsScreen.querySelectorAll(
            ".settings-group-title"
        );

    if (groupTitles[0]) {
        groupTitles[0].textContent =
            t("settings.basic");
    }

    if (groupTitles[1]) {
        groupTitles[1].textContent =
            t("settings.management");
    }

    if (groupTitles[2]) {
        groupTitles[2].textContent =
            t("settings.data");
    }

    if (groupTitles[3]) {
        groupTitles[3].textContent =
            t("settings.app");
    }


    /*
     基本設定
     表示言語
    */

    const languageLabel =
        settingsScreen.querySelector(
            'label[for="cmbSettingsLanguage"]'
        );

    if (languageLabel) {

        languageLabel.textContent =
            t("settings.language");

    }


    /*
     基本設定
     普段買い物する国・地域
    */

    const baseCountryLabel =
        settingsScreen.querySelector(
            'label[for="cmbSettingsBaseCountry"]'
        );

    if (baseCountryLabel) {

        baseCountryLabel.textContent =
            t("settings.baseCountry");

    }


    /*
     表示言語の選択肢
    */

    const languageSelect =
        document.getElementById(
            "cmbSettingsLanguage"
        );

    if (languageSelect) {

        /*
         現在の表示言語と
         プルダウンの選択状態を同期
        */

        languageSelect.value =
            getCocartlyLanguage();


        const languageOptions = [
            "ja",
            "en",
            "ko",
            "zh-CN",
            "zh-TW",
            "fr"
        ];

        for (
            const language
            of languageOptions
        ) {

            const option =
                languageSelect.querySelector(
                    `option[value="${language}"]`
                );

            if (option) {

                option.textContent =
                    t(
                        "language." +
                        language
                    );

            }

        }

    }


    /*
     国・地域の選択肢
    */

    const countrySelect =
        document.getElementById(
            "cmbSettingsBaseCountry"
        );

    if (countrySelect) {

        const countryOptions = [
            "JP",
            "US",
            "CA",
            "AU",
            "KR",
            "CN",
            "TW",
            "OTHER"
        ];

        const countryIcons = {

            JP: "🇯🇵 ",
            US: "🇺🇸 ",
            CA: "🇨🇦 ",
            AU: "🇦🇺 ",
            KR: "🇰🇷 ",
            CN: "🇨🇳 ",
            TW: "🇹🇼 ",
            OTHER: "🌐 "

        };


        for (
            const country
            of countryOptions
        ) {

            const option =
                countrySelect.querySelector(
                    `option[value="${country}"]`
                );

            if (option) {

                option.textContent =
                    countryIcons[country] +
                    t(
                        "country." +
                        country
                    );

            }

        }

    }


    /*
     商品ジャンル管理
    */

    const categoryTitle =
        document.querySelector(
            "#btnCategoryMaster strong"
        );

    const categoryGuide =
        document.querySelector(
            "#btnCategoryMaster small"
        );

    if (categoryTitle) {

        categoryTitle.textContent =
            t(
                "settings.category.title"
            );

    }

    if (categoryGuide) {

        categoryGuide.textContent =
            t(
                "settings.category.guide"
            );

    }


    /*
     購入先管理
    */

    const storeTitle =
        document.querySelector(
            "#btnStoreMaster strong"
        );

    const storeGuide =
        document.querySelector(
            "#btnStoreMaster small"
        );

    if (storeTitle) {

        storeTitle.textContent =
            t(
                "settings.store.title"
            );

    }

    if (storeGuide) {

        storeGuide.textContent =
            t(
                "settings.store.guide"
            );

    }


    /*
     データをバックアップ
    */

    const backupTitle =
        document.querySelector(
            "#btnBackupData strong"
        );

    const backupGuide =
        document.querySelector(
            "#btnBackupData small"
        );

    if (backupTitle) {

        backupTitle.textContent =
            t(
                "settings.backup.title"
            );

    }

    if (backupGuide) {

        backupGuide.textContent =
            t(
                "settings.backup.guide"
            );

    }


    /*
     バックアップから復元
    */

    const restoreTitle =
        document.querySelector(
            "#btnRestoreData strong"
        );

    const restoreGuide =
        document.querySelector(
            "#btnRestoreData small"
        );

    if (restoreTitle) {

        restoreTitle.textContent =
            t(
                "settings.restore.title"
            );

    }

    if (restoreGuide) {

        restoreGuide.textContent =
            t(
                "settings.restore.guide"
            );

    }


    /*
     Cocartlyについて
    */

    const aboutTitle =
        document.querySelector(
            "#btnAboutCocartly strong"
        );

    const aboutGuide =
        document.querySelector(
            "#btnAboutCocartly small"
        );

    if (aboutTitle) {

        aboutTitle.textContent =
            t(
                "settings.about.title"
            );

    }

    if (aboutGuide) {

        aboutGuide.textContent =
            t(
                "settings.about.guide"
            );

    }


    /*
     バージョン
    */

    const versionLabel =
        settingsScreen.querySelector(
            ".settings-version-label"
        );

    if (versionLabel) {

        versionLabel.textContent =
            t("settings.version");

    }


    /*
     プライバシーポリシー
    */

    const privacyTitle =
        document.querySelector(
            "#btnPrivacyPolicy strong"
        );

    const privacyGuide =
        document.querySelector(
            "#btnPrivacyPolicy small"
        );

    if (privacyTitle) {

        privacyTitle.textContent =
            t(
                "settings.privacy.title"
            );

    }

    if (privacyGuide) {

        privacyGuide.textContent =
            t(
                "settings.privacy.guide"
            );

    }

}

function updateProductScreenLanguage() {

    const productScreen =
        document.getElementById(
            "productScreen"
        );

    if (!productScreen) {
        return;
    }


    /*
     ホームへ戻る
    */

    const backButton =
        document.getElementById(
            "btnHomeFromProduct"
        );

    if (backButton) {
        backButton.textContent =
            t("product.backHome");
    }


    /*
     画面タイトル
    */

    const title =
        productScreen.querySelector(
            ".screen-title-row h2"
        );

    if (title) {
        title.textContent =
            "📦 " +
            t("product.title");
    }


    /*
     使い方
    */

    const helpButton =
        document.getElementById(
            "btnProductHelp"
        );

    if (helpButton) {
        helpButton.textContent =
            t("product.help");
    }


    /*
     商品名
    */

    const productNameLabel =
        productScreen.querySelector(
            'label[for="txtProductName"]'
        );

    if (productNameLabel) {
        productNameLabel.textContent =
            t("product.name");
    }


    const productNameInput =
        document.getElementById(
            "txtProductName"
        );

    if (productNameInput) {
        productNameInput.placeholder =
            t(
                "product.namePlaceholder"
            );
    }


    /*
     商品コード
    */

    const productCodeLabel =
        productScreen.querySelector(
            'label[for="txtJanCode"]'
        );

    if (productCodeLabel) {
        productCodeLabel.textContent =
            t("product.code");
    }


    const productCodeInput =
        document.getElementById(
            "txtJanCode"
        );

    if (productCodeInput) {
        productCodeInput.placeholder =
            t(
                "product.codePlaceholder"
            );
    }


    /*
     商品コード案内文
    */

    const codeGuide =
        productScreen.querySelector(
            ".jan-code-guide"
        );

    if (codeGuide) {
        codeGuide.textContent =
            t("product.codeGuide");
    }

    /*
     商品ジャンル
    */

    const categoryLabel =
        productScreen.querySelector(
            'label[for="cmbCategory"]'
        );

    if (categoryLabel) {
        categoryLabel.textContent =
            t("product.category");
    }

    /*
     商品ジャンルの初期表示
    */

    const categorySelect =
        document.getElementById(
            "cmbCategory"
        );

    if (categorySelect) {

        const emptyOption =
            categorySelect.querySelector(
                'option[value=""]'
            );

        if (emptyOption) {
            emptyOption.textContent =
                t("product.select");
        }

    }

    /*
     商品ジャンル管理
    */

    const categoryManageButton =
        document.getElementById(
            "btnManageCategories"
        );

    if (categoryManageButton) {
        categoryManageButton.textContent =
            t("product.categoryManage");
    }


    /*
     国・地域
    */

    const countryLabel =
        productScreen.querySelector(
            'label[for="cmbStoreCountry"]'
        );

    if (countryLabel) {
        countryLabel.textContent =
            t("product.country");
    }


    /*
     国・地域の選択肢
    */

    const countrySelect =
        document.getElementById(
            "cmbStoreCountry"
        );

    if (countrySelect) {

        const countryOptions = [
            "JP",
            "US",
            "CA",
            "AU",
            "KR",
            "CN",
            "TW",
            "OTHER"
        ];

        const countryIcons = {
            JP: "🇯🇵 ",
            US: "🇺🇸 ",
            CA: "🇨🇦 ",
            AU: "🇦🇺 ",
            KR: "🇰🇷 ",
            CN: "🇨🇳 ",
            TW: "🇹🇼 ",
            OTHER: "🌐 "
        };

        for (
            const country
            of countryOptions
        ) {

            const option =
                countrySelect.querySelector(
                    `option[value="${country}"]`
                );

            if (option) {
                option.textContent =
                    countryIcons[country] +
                    t(
                        "country." +
                        country
                    );
            }

        }

    }


    /*
     購入先
    */

    const storeLabel =
        productScreen.querySelector(
            'label[for="cmbStore"]'
        );

    if (storeLabel) {
        storeLabel.textContent =
            t("product.store");
    }


    /*
     購入先の初期表示
    */

    const storeSelect =
        document.getElementById(
            "cmbStore"
        );

    if (storeSelect) {

        const emptyOption =
            storeSelect.querySelector(
                'option[value=""]'
            );

        if (emptyOption) {
            emptyOption.textContent =
                t("product.select");
        }

    }


    /*
     購入先管理
    */

    const storeManageButton =
        document.getElementById(
            "btnManageStores"
        );

    if (storeManageButton) {
        storeManageButton.textContent =
            t("product.storeManage");
    }

    /*
     商品登録
     価格計算
    */

    /*
     表示価格
    */

    const productTaxDisplayPriceLabel =
        productScreen.querySelector(
            'label[for="txtProductTaxExcluded"]'
        );

    if (productTaxDisplayPriceLabel) {
        productTaxDisplayPriceLabel.textContent =
            t(
                "product.tax.displayPrice"
            );
    }


    /*
     表示価格の種類
    */

    const productPriceTypeLabel =
        productScreen.querySelector(
            'label[for="cmbProductPriceType"]'
        );

    if (productPriceTypeLabel) {
        productPriceTypeLabel.textContent =
            t(
                "product.tax.priceType"
            );
    }


    /*
     税込・税抜
    */

    const productPriceTypeSelect =
        document.getElementById(
            "cmbProductPriceType"
        );

    if (productPriceTypeSelect) {

        const taxIncludedOption =
            productPriceTypeSelect.querySelector(
                'option[value="taxIncluded"]'
            );

        const taxExcludedOption =
            productPriceTypeSelect.querySelector(
                'option[value="taxExcluded"]'
            );

        if (taxIncludedOption) {
            taxIncludedOption.textContent =
                t(
                    "product.tax.taxIncluded"
                );
        }

        if (taxExcludedOption) {
            taxExcludedOption.textContent =
                t(
                    "product.tax.taxExcluded"
                );
        }

    }


    /*
     税率
    */

    const productTaxRateLabel =
        productScreen.querySelector(
            'label[for="cmbProductTaxRate"]'
        );

    if (productTaxRateLabel) {
        productTaxRateLabel.textContent =
            t(
                "product.tax.taxRate"
            );
    }


    /*
     海外用 税率
    */

    const productCustomTaxRateLabel =
        productScreen.querySelector(
            'label[for="txtProductCustomTaxRate"]'
        );

    if (productCustomTaxRateLabel) {
        productCustomTaxRateLabel.textContent =
            t(
                "product.tax.customTaxRate"
            );
    }


    /*
     端数処理
    */

    const productRoundingLabel =
        productScreen.querySelector(
            'label[for="cmbProductRoundingMode"]'
        );

    if (productRoundingLabel) {
        productRoundingLabel.textContent =
            t(
                "product.tax.rounding"
            );
    }


    /*
     端数処理の選択肢
    */

    const productRoundingSelect =
        document.getElementById(
            "cmbProductRoundingMode"
        );

    if (productRoundingSelect) {

        const roundOption =
            productRoundingSelect.querySelector(
                'option[value="round"]'
            );

        const floorOption =
            productRoundingSelect.querySelector(
                'option[value="floor"]'
            );

        const ceilOption =
            productRoundingSelect.querySelector(
                'option[value="ceil"]'
            );

        if (roundOption) {
            roundOption.textContent =
                t(
                    "product.tax.round"
                );
        }

        if (floorOption) {
            floorOption.textContent =
                t(
                    "product.tax.floor"
                );
        }

        if (ceilOption) {
            ceilOption.textContent =
                t(
                    "product.tax.ceil"
                );
        }

    }


    /*
     値引き
    */

    const productDiscountLabel =
        productScreen.querySelector(
            'label[for="cmbProductDiscountType"]'
        );

    if (productDiscountLabel) {
        productDiscountLabel.textContent =
            t(
                "product.tax.discount"
            );
    }


    /*
     値引きの選択肢
    */

    const productDiscountSelect =
        document.getElementById(
            "cmbProductDiscountType"
        );

    if (productDiscountSelect) {

        const discountNoneOption =
            productDiscountSelect.querySelector(
                'option[value="none"]'
            );

        const discountPercentOption =
            productDiscountSelect.querySelector(
                'option[value="percent"]'
            );

        if (discountNoneOption) {
            discountNoneOption.textContent =
                t(
                    "product.tax.discountNone"
                );
        }

        if (discountPercentOption) {
            discountPercentOption.textContent =
                t(
                    "product.tax.discountPercent"
                );
        }

    }


    /*
     値引き値
    */

    const productDiscountValueLabel =
        productScreen.querySelector(
            'label[for="txtProductDiscountValue"]'
        );

    if (productDiscountValueLabel) {
        productDiscountValueLabel.textContent =
            t(
                "product.tax.discountValue"
            );
    }


    /*
     この価格を使用
    */

    const productUseTaxPriceButton =
        document.getElementById(
            "btnUseProductTaxPrice"
        );

    if (productUseTaxPriceButton) {
        productUseTaxPriceButton.textContent =
            t(
                "product.tax.usePrice"
            );
    }


    /*
     閉じる
    */

    const productCloseTaxCalcButton =
        document.getElementById(
            "btnCloseProductTaxCalc"
        );

    if (productCloseTaxCalcButton) {
        productCloseTaxCalcButton.textContent =
            t(
                "product.tax.close"
            );
    }

    /*
     値引き値の入力例
    */

    const productDiscountValueInput =
        document.getElementById(
            "txtProductDiscountValue"
        );

    if (productDiscountValueInput) {
        productDiscountValueInput.placeholder =
            t(
                "product.tax.discountValuePlaceholder"
            );
    }

    /*
     価格計算
     最終価格の初期表示
    */

    const productTaxResult =
        document.getElementById(
            "productTaxResult"
        );

    if (productTaxResult) {

        /*
         まだ価格計算していない場合だけ
         現在の表示言語で
         初期表示を更新する
        */

        if (
            !productPriceCalculation
        ) {

            productTaxResult.textContent =
                t(
                    "product.tax.finalPrice"
                ).replace(
                    "{price}",
                    "-"
                );

        }

    }

    /*
     保存
    */

    const saveButton =
        document.getElementById(
            "btnSaveProduct"
        );

    if (saveButton) {
        saveButton.textContent =
            t("product.save");
    }


    /*
     編集キャンセル
    */

    const cancelEditButton =
        document.getElementById(
            "btnCancelProductEdit"
        );

    if (cancelEditButton) {
        cancelEditButton.textContent =
            t("product.cancelEdit");
    }

    /*
     内容量
    */

    const volumeLabel =
        productScreen.querySelector(
            'label[for="txtVolume"]'
        );

    if (volumeLabel) {
        volumeLabel.textContent =
            t("product.volume");
    }


    /*
     内容量 入力例
    */

    const volumeExampleButton =
        document.getElementById(
            "btnVolumeExample"
        );

    if (volumeExampleButton) {
        volumeExampleButton.textContent =
            t("product.volumeExample");
    }


    /*
     内容量 プレースホルダー
    */

    const volumeInput =
        document.getElementById(
            "txtVolume"
        );

    if (volumeInput) {
        volumeInput.placeholder =
            t("product.volumePlaceholder");
    }

    /*
     単位
    */

    const unitLabel =
        productScreen.querySelector(
            'label[for="cmbUnit"]'
        );

    if (unitLabel) {
        unitLabel.textContent =
            t("product.unit");
    }


    /*
     単位の初期表示
    */

    const unitSelect =
        document.getElementById(
            "cmbUnit"
        );

    if (unitSelect) {

        const emptyOption =
            unitSelect.querySelector(
                'option[value=""]'
            );

        if (emptyOption) {
            emptyOption.textContent =
                t("product.select");
        }

    }

    /*
     単位の表示名
    */

    if (unitSelect) {

        const unitTranslations = {
            "個": "product.unit.each",
            "枚": "product.unit.sheet",
            "本": "product.unit.bottle",
            "袋": "product.unit.bag",
            "箱": "product.unit.box",
            "パック": "product.unit.pack",
            "ロール": "product.unit.roll"
        };

        for (
            const [
                unitValue,
                translationKey
            ]
            of Object.entries(
                unitTranslations
            )
        ) {

            const option =
                unitSelect.querySelector(
                    `option[value="${unitValue}"]`
                );

            if (option) {
                option.textContent =
                    t(translationKey);
            }

        }

    }

    /*
 内容量・単位の入力例
*/

    const volumeExample =
        document.getElementById(
            "volumeExample"
        );

    if (volumeExample) {

        /*
         入力例タイトル
        */

        const exampleTitle =
            volumeExample.querySelector(
                ".input-example-header strong"
            );

        if (exampleTitle) {
            exampleTitle.textContent =
                t(
                    "product.volumeExampleTitle"
                );
        }


        /*
         入力例4件
        */

        const exampleItems =
            volumeExample.querySelectorAll(
                "p"
            );

        const exampleTranslations = [
            [
                "product.volumeExampleEgg",
                "product.volumeExampleEggDetail"
            ],
            [
                "product.volumeExampleMilk",
                "product.volumeExampleMilkDetail"
            ],
            [
                "product.volumeExampleRice",
                "product.volumeExampleRiceDetail"
            ],
            [
                "product.volumeExampleBread",
                "product.volumeExampleBreadDetail"
            ]
        ];

        exampleTranslations.forEach(
            (
                translationKeys,
                index
            ) => {

                const item =
                    exampleItems[index];

                if (!item) {
                    return;
                }

                item.innerHTML =
                    t(
                        translationKeys[0]
                    ) +
                    "<br>" +
                    t(
                        translationKeys[1]
                    );

            }
        );

    }


    /*
     価格（税込）
    */

    const priceLabel =
        productScreen.querySelector(
            'label[for="txtPrice"]'
        );

    if (priceLabel) {
        priceLabel.textContent =
            t("product.price");
    }


    /*
     価格計算ボタン
    */

    const priceCalcButton =
        document.getElementById(
            "btnProductTaxCalc"
        );

    if (priceCalcButton) {
        priceCalcButton.textContent =
            t("product.priceCalc");
    }


    /*
     価格プレースホルダー
     国・地域に合わせる
    */
    const priceInput =
        document.getElementById("txtPrice");

    if (priceInput) {
        const countrySelect =
            document.getElementById("cmbStoreCountry");

        const country =
            countrySelect
                ? countrySelect.value ||
                getCocartlyBaseCountry()
                : getCocartlyBaseCountry();

        const displayPriceExamples = {
            JP: "198",
            US: "1.98",
            CA: "1.98",
            AU: "1.98",
            KR: "1980",
            CN: "19.8",
            TW: "198",
            OTHER: "1.98"
        };

        priceInput.placeholder =
            t("product.tax.examplePrefix") +
            (displayPriceExamples[country] || "198");
    }


    /*
     商品マスターへ共有
    */

    const shareTitle =
        productScreen.querySelector(
            ".shareTitle"
        );

    if (shareTitle) {
        shareTitle.textContent =
            t("product.shareTitle");
    }

    const shareDescription =
        productScreen.querySelector(
            ".shareDescription"
        );

    if (shareDescription) {
        shareDescription.textContent =
            t("product.shareDescription");
    }

    const shareThanks =
        productScreen.querySelector(
            ".shareThanks"
        );

    if (shareThanks) {
        shareThanks.textContent =
            t("product.shareThanks");
    }


    /*
     登録済み商品
    */

    const registeredTitle =
        productScreen.querySelector(
            ".productButtons + p + h3"
        );

    if (registeredTitle) {
        registeredTitle.textContent =
            t("product.list.title");
    }


    /*
     商品登録 使い方
    */

    const productHelp =
        document.getElementById(
            "productHelp"
        );

    if (productHelp) {

        const helpTitle =
            productHelp.querySelector(
                ".screen-help-panel-header strong"
            );

        if (helpTitle) {
            helpTitle.textContent =
                t("product.help.title");
        }

        const sections =
            productHelp.querySelectorAll(
                ".help-section"
            );

        if (sections[0]) {

            const heading =
                sections[0].querySelector(
                    "h3"
                );

            if (heading) {
                heading.textContent =
                    t(
                        "product.help.featureTitle"
                    );
            }

            const paragraphs =
                sections[0].querySelectorAll(
                    "p"
                );

            const keys = [
                "product.help.feature1",
                "product.help.feature2",
                "product.help.feature3",
                "product.help.feature4"
            ];

            paragraphs.forEach(
                (paragraph, index) => {

                    if (keys[index]) {
                        paragraph.textContent =
                            t(keys[index]);
                    }

                }
            );
        }

        if (sections[1]) {

            const heading =
                sections[1].querySelector(
                    "h3"
                );

            if (heading) {
                heading.textContent =
                    t(
                        "product.help.usefulTitle"
                    );
            }

            const paragraphs =
                sections[1].querySelectorAll(
                    "p"
                );

            const keys = [
                "product.help.useful1",
                "product.help.useful2",
                "product.help.useful3",
                "product.help.useful4"
            ];

            paragraphs.forEach(
                (paragraph, index) => {

                    if (keys[index]) {
                        paragraph.textContent =
                            t(keys[index]);
                    }

                }
            );
        }

        if (sections[2]) {

            const heading =
                sections[2].querySelector(
                    "h3"
                );

            if (heading) {
                heading.textContent =
                    t(
                        "product.help.howToTitle"
                    );
            }

            const items =
                sections[2].querySelectorAll(
                    "li"
                );

            const keys = [
                "product.help.step1",
                "product.help.step2",
                "product.help.step3",
                "product.help.step4",
                "product.help.step5",
                "product.help.step6",
                "product.help.step7",
                "product.help.step8"
            ];

            items.forEach(
                (item, index) => {

                    if (keys[index]) {
                        item.textContent =
                            t(keys[index]);
                    }

                }
            );
        }

        const closeButton =
            document.getElementById(
                "btnCloseProductHelpBottom"
            );

        if (closeButton) {
            closeButton.textContent =
                t("product.help.close");
        }

    }

}

/* ==========================================
   ホーム画面
   ========================================== */

function openHome() {

    hideAllScreens();

    /*
     現在の表示言語を
     ホーム画面へ反映
    */

    updateHomeScreenLanguage();

    document.getElementById(
        "homeScreen"
    ).hidden =
        false;

}

/* ==========================================
   商品ジャンル管理画面
   表示言語更新
   ========================================== */

function updateCategoryScreenLanguage() {

    const categoryScreen =
        document.getElementById(
            "categoryScreen"
        );

    if (!categoryScreen) {

        return;

    }


    /*
     画面タイトル
    */

    const title =
        categoryScreen.querySelector(
            "h2"
        );

    if (title) {

        title.textContent =
            t(
                "category.title"
            );

    }


    /*
     商品ジャンル名
    */

    const nameLabel =
        categoryScreen.querySelector(
            'label[for="txtCategoryName"]'
        );

    if (nameLabel) {

        nameLabel.textContent =
            t(
                "category.name"
            );

    }


    /*
     商品ジャンル名
     入力例
    */

    const nameInput =
        document.getElementById(
            "txtCategoryName"
        );

    if (nameInput) {

        nameInput.placeholder =
            t(
                "category.namePlaceholder"
            );

    }


    /*
     保存
    */

    const saveButton =
        document.getElementById(
            "btnSaveCategory"
        );

    if (saveButton) {

        saveButton.textContent =
            t(
                "category.save"
            );

    }

}


/* ==========================================
   カテゴリー管理画面
   ========================================== */

function openCategoryMaster() {

    /*
     設定画面から開く
    */

    masterReturnScreen =
        "settings";


    hideAllScreens();


    document.getElementById(
        "categoryScreen"
    ).hidden =
        false;


    /*
     商品ジャンル管理画面の
     表示言語を更新
    */

    updateCategoryScreenLanguage();


    editingCategoryId =
        null;


    document.getElementById(
        "txtCategoryName"
    ).value =
        "";


    const btnBackCategory =
        document.getElementById(
            "btnBackCategory"
        );


    if (btnBackCategory) {

        btnBackCategory.textContent =
            t("category.backSettings");

    }


    displayCategoryList();

}

/* ==========================================
   商品登録画面から商品ジャンル管理を開く
   ========================================== */

function openCategoryMasterFromProduct() {

    const cmbCategory =
        document.getElementById("cmbCategory");

    productSelectedCategoryId =
        cmbCategory ? cmbCategory.value : "";

    masterReturnScreen = "product";

    hideAllScreens();

    document.getElementById("categoryScreen").hidden =
        false;


    /*
     商品ジャンル管理画面の
     表示言語を更新
    */

    updateCategoryScreenLanguage();


    editingCategoryId = null;

    document.getElementById("txtCategoryName").value =
        "";

    const btnBackCategory =
        document.getElementById(
            "btnBackCategory"
        );

    if (btnBackCategory) {

        btnBackCategory.textContent =
            t("category.backProduct");

    }

    displayCategoryList();

}

/* ==========================================
   購入先管理画面
   表示言語更新
   ========================================== */

function updateStoreScreenLanguage() {

    const storeScreen =
        document.getElementById(
            "storeScreen"
        );

    if (!storeScreen) {

        return;

    }


    /*
     画面タイトル
    */

    const title =
        storeScreen.querySelector(
            "h2"
        );

    if (title) {

        title.textContent =
            t(
                "store.title"
            );

    }


    /*
     国・地域
    */

    const countryLabel =
        storeScreen.querySelector(
            'label[for="cmbStoreMasterCountry"]'
        );

    if (countryLabel) {

        countryLabel.textContent =
            t(
                "store.country"
            );

    }


    /*
     国・地域の選択肢
    */

    const countrySelect =
        document.getElementById(
            "cmbStoreMasterCountry"
        );

    if (countrySelect) {

        const countries = [
            "JP",
            "US",
            "CA",
            "AU",
            "KR",
            "CN",
            "TW",
            "OTHER"
        ];

        const countryIcons = {
            JP: "🇯🇵 ",
            US: "🇺🇸 ",
            CA: "🇨🇦 ",
            AU: "🇦🇺 ",
            KR: "🇰🇷 ",
            CN: "🇨🇳 ",
            TW: "🇹🇼 ",
            OTHER: "🌐 "
        };

        for (
            const country
            of countries
        ) {

            const option =
                countrySelect.querySelector(
                    `option[value="${country}"]`
                );

            if (option) {

                option.textContent =
                    countryIcons[country] +
                    t(
                        "country." +
                        country
                    );

            }

        }

    }


    /*
     購入先の種類
    */

    const typeLabel =
        storeScreen.querySelector(
            'label[for="cmbStoreType"]'
        );

    if (typeLabel) {

        typeLabel.textContent =
            t(
                "store.type"
            );

    }


    /*
     購入先の種類
     選択肢
    */

    const typeSelect =
        document.getElementById(
            "cmbStoreType"
        );

    if (typeSelect) {

        const emptyOption =
            typeSelect.querySelector(
                'option[value=""]'
            );

        if (emptyOption) {

            emptyOption.textContent =
                t(
                    "store.select"
                );

        }


        const storeTypes = [
            "supermarket",
            "convenience",
            "drugstore",
            "discount",
            "warehouse-club",
            "fixed-price",
            "home-center",
            "department-mall",
            "clothing",
            "electronics",
            "gas-station",
            "specialty",
            "online",
            "subscription",
            "vending",
            "other"
        ];

        for (
            const storeType
            of storeTypes
        ) {

            const option =
                typeSelect.querySelector(
                    `option[value="${storeType}"]`
                );

            if (option) {

                option.textContent =
                    t(
                        "store.type." +
                        storeType
                    );

            }

        }

    }


    /*
     都道府県・州など
    */

    const regionLabel =
        storeScreen.querySelector(
            'label[for="txtStoreRegion"]'
        );

    if (regionLabel) {

        regionLabel.textContent =
            t(
                "store.region"
            );

    }


    /*
     市区町村
    */

    const cityLabel =
        storeScreen.querySelector(
            'label[for="txtStoreCity"]'
        );

    if (cityLabel) {

        cityLabel.textContent =
            t(
                "store.city"
            );

    }


    /*
     購入先名
    */

    const nameLabel =
        storeScreen.querySelector(
            'label[for="txtStoreName"]'
        );

    if (nameLabel) {

        nameLabel.textContent =
            t(
                "store.name"
            );

    }


    /*
     保存
    */

    const saveButton =
        document.getElementById(
            "btnSaveStore"
        );

    if (saveButton) {

        saveButton.textContent =
            t(
                "store.save"
            );

    }


    /*
     登録済み購入先
    */

    const registeredTitle =
        storeScreen.querySelector(
            "#storeMessage + h3"
        );

    if (registeredTitle) {

        registeredTitle.textContent =
            t(
                "store.registered"
            );

    }

}


/* ==========================================
   店舗管理画面
   ========================================== */

function openStoreMaster() {

    masterReturnScreen = "settings";

    hideAllScreens();

    document.getElementById("storeScreen").hidden =
        false;


    /*
     購入先管理画面の
     表示言語を更新
    */

    updateStoreScreenLanguage();


    editingStoreId = null;

    document.getElementById("txtStoreName").value =
        "";

    document.getElementById("cmbStoreType").selectedIndex =
        0;

    document.getElementById("storeMessage").textContent =
        "";

    /*
     普段買い物する国・地域を
     購入先管理へ反映
    */

    const cmbStoreMasterCountry =
        document.getElementById(
            "cmbStoreMasterCountry"
        );

    if (cmbStoreMasterCountry) {

        cmbStoreMasterCountry.value =
            getCocartlyBaseCountry();

    }

    /*
     国・地域に合わせて
     入力例を更新
    */

    updateStoreLocationPlaceholders();

    const btnBackStore =
        document.getElementById(
            "btnBackStore"
        );

    if (btnBackStore) {

        btnBackStore.textContent =
            t("store.backSettings");

    }

    displayStores();

}

/* ==========================================
   商品登録画面から購入先管理を開く
   ========================================== */

function openStoreMasterFromProduct() {

    const cmbStore =
        document.getElementById("cmbStore");

    productSelectedStoreId =
        cmbStore ? cmbStore.value : "";

    masterReturnScreen = "product";

    hideAllScreens();

    document.getElementById("storeScreen").hidden =
        false;


    /*
     購入先管理画面の
     表示言語を更新
    */

    updateStoreScreenLanguage();


    editingStoreId = null;

    /*
     商品登録で選択している国・地域を
     購入先管理へ引き継ぐ
    */

    const cmbStoreCountry =
        document.getElementById(
            "cmbStoreCountry"
        );

    const cmbStoreMasterCountry =
        document.getElementById(
            "cmbStoreMasterCountry"
        );

    if (
        cmbStoreCountry &&
        cmbStoreMasterCountry
    ) {

        cmbStoreMasterCountry.value =
            cmbStoreCountry.value;

    }


    /*
     商品登録から開いた場合も
     選択した国・地域に合わせて
     入力例を更新
    */
    if (
        typeof updateStoreLocationPlaceholders ===
        "function"
    ) {

        updateStoreLocationPlaceholders();

    }


    document.getElementById("txtStoreName").value =
        "";

    document.getElementById("cmbStoreType").selectedIndex =
        0;


    /*
     都道府県・州、市区町村を初期化
    */

    const txtStoreRegion =
        document.getElementById(
            "txtStoreRegion"
        );

    const txtStoreCity =
        document.getElementById(
            "txtStoreCity"
        );

    if (txtStoreRegion) {

        txtStoreRegion.value = "";

    }

    if (txtStoreCity) {

        txtStoreCity.value = "";

    }


    document.getElementById("storeMessage").textContent =
        "";

    const btnBackStore =
        document.getElementById(
            "btnBackStore"
        );

    if (btnBackStore) {

        btnBackStore.textContent =
            t("store.backProduct");

    }

    displayStores();

}

/* ==========================================
   今回の購入 → 購入先管理
   ========================================== */

function openStoreMasterFromPriceRecord() {

    /*
     今回の購入で入力中の内容を保存
    */

    const cmbPriceRecordStore =
        document.getElementById(
            "cmbPriceRecordStore"
        );

    const txtPriceRecordPrice =
        document.getElementById(
            "txtPriceRecordPrice"
        );

    const txtPriceRecordQuantity =
        document.getElementById(
            "txtPriceRecordQuantity"
        );


    priceRecordTempStoreId =
        cmbPriceRecordStore
            ? cmbPriceRecordStore.value
            : "";

    priceRecordTempPrice =
        txtPriceRecordPrice
            ? txtPriceRecordPrice.value
            : "";

    priceRecordTempQuantity =
        txtPriceRecordQuantity
            ? txtPriceRecordQuantity.value
            : "1";


    masterReturnScreen =
        "priceRecord";

    hideAllScreens();

    document.getElementById(
        "storeScreen"
    ).hidden = false;


    /*
     購入先管理画面の
     表示言語を更新
    */

    updateStoreScreenLanguage();


    editingStoreId =
        null;


    /*
     今回の購入の商品から
     登録時の購入先を取得
    */

    const product =
        products.find(
            product =>
                product.id ===
                recordingProductId
        );

    const registeredStore =
        product &&
            product.storeId
            ? findStore(
                product.storeId
            )
            : null;


    /*
     商品の国・地域を
     購入先管理へ引き継ぐ
    */

    const cmbStoreMasterCountry =
        document.getElementById(
            "cmbStoreMasterCountry"
        );

    if (cmbStoreMasterCountry) {

        cmbStoreMasterCountry.value =
            registeredStore &&
                registeredStore.country
                ? registeredStore.country
                : getCocartlyBaseCountry();

    }


    document.getElementById(
        "txtStoreName"
    ).value = "";

    document.getElementById(
        "cmbStoreType"
    ).selectedIndex = 0;


    /*
     都道府県・州、市区町村を初期化
    */

    const txtStoreRegion =
        document.getElementById(
            "txtStoreRegion"
        );

    const txtStoreCity =
        document.getElementById(
            "txtStoreCity"
        );

    if (txtStoreRegion) {

        txtStoreRegion.value = "";

    }

    if (txtStoreCity) {

        txtStoreCity.value = "";

    }


    document.getElementById(
        "storeMessage"
    ).textContent = "";


    /*
     国・地域に合わせて
     購入先名・地域・市区町村の
     入力例を更新
    */

    updateStoreLocationPlaceholders();


    const btnBackStore =
        document.getElementById(
            "btnBackStore"
        );

    if (btnBackStore) {

        btnBackStore.textContent =
            t(
                "purchase.backPurchase"
            );

    }

    displayStores();

}

/* ==========================================
   削除済み商品一覧
   ========================================== */

function displayDeletedProducts() {

    const deletedProductList =
        document.getElementById("deletedProductList");

    deletedProductList.innerHTML = "";

    for (const product of products) {

        if (product.active) {

            continue;

        }

        const createdDate = new Date(product.createdAt);

        const dateText =
            createdDate.getFullYear() + "/" +
            String(createdDate.getMonth() + 1).padStart(2, "0") + "/" +
            String(createdDate.getDate()).padStart(2, "0");


        /*
         商品の購入先から
         国・通貨を取得
        */

        const productStore =
            product.storeId
                ? findStore(
                    product.storeId
                )
                : null;

        const productCountry =
            productStore
                ? productStore.country || "JP"
                : getCocartlyBaseCountry();

        const productCurrency =
            getCurrencyByCountry(
                productCountry
            );


        /*
         通貨記号
        */

        const currencySymbols = {

            JPY: "¥",
            USD: "$",
            CAD: "C$",
            AUD: "A$",
            KRW: "₩",
            CNY: "¥",
            TWD: "NT$"

        };

        const productCurrencySymbol =
            currencySymbols[
            productCurrency
            ] || "¥";


        deletedProductList.innerHTML += `

<div class="product-card">

    <h3>${product.name}</h3>

    <p>
        <span class="product-label">
            ${t("product.list.code")}
        </span>
        <span class="product-value">
            ${product.janCode || "-"}
        </span>
    </p>

    <p>
        <span class="product-label">
            ${t("product.list.category")}
        </span>
        <span class="product-value">
            ${product.categoryId}
        </span>
    </p>

    <p>
        <span class="product-label">
            ${t("product.list.volume")}
        </span>
        <span class="product-value">
            ${product.volume}${product.unit}
        </span>
    </p>

    <p>
        <span class="product-label">
            ${t("product.list.price")}
        </span>
        <span class="product-value">
            ${product.price !== undefined &&
                product.price !== null &&
                product.price !== ""
                ? productCurrencySymbol +
                product.price
                : "-"
            }
        </span>
    </p>

    <p>
        <span class="product-label">
            ${t("product.list.store")}
        </span>
        <span class="product-value">
            ${getStoreDisplayName(product.storeId)}
        </span>
    </p>

    <p>
        <span class="product-label">
            ${t("product.list.registeredDate")}
        </span>
        <span class="product-value">
            ${dateText}
        </span>
    </p>

    <div class="product-buttons">

        <button onclick="restoreProduct('${product.id}')">
            ${t(
                "product.deleted.restore"
            )}
        </button>

        <button onclick="permanentlyDeleteProduct('${product.id}')">
            ${t(
                "product.deleted.permanentDelete"
            )}
        </button>

    </div>

</div>

`;
    }

}

/* ==========================================
   商品編集
   ========================================== */

function editProduct(productId) {

    const product = products.find(

        p => p.id === productId

    );

    if (!product) {

        return;

    }

    editingProductId = product.id;


    /*
     編集キャンセルボタンを表示
    */

    const btnCancelProductEdit =
        document.getElementById(
            "btnCancelProductEdit"
        );

    if (btnCancelProductEdit) {

        btnCancelProductEdit.hidden =
            false;

    }


    /*
     商品登録画面を表示
    */

    hideAllScreens();

    document.getElementById(
        "productScreen"
    ).hidden = false;


    /*
     商品情報を入力欄へ反映
    */

    document.getElementById(
        "txtProductName"
    ).value =
        product.name || "";

    document.getElementById(
        "txtJanCode"
    ).value =
        product.janCode || product.jan || "";

    document.getElementById(
        "cmbCategory"
    ).value =
        product.categoryId || "";

    document.getElementById(
        "txtVolume"
    ).value =
        product.volume || "";

    document.getElementById(
        "cmbUnit"
    ).value =
        product.unit || "";

    document.getElementById(
        "txtPrice"
    ).value =
        product.price || "";


    /*
     商品に登録されている
     購入先を取得
    */

    const registeredStore =
        product.storeId
            ? findStore(
                product.storeId
            )
            : null;


    /*
     購入先の国・地域を
     商品登録画面へ復元
    */

    const cmbStoreCountry =
        document.getElementById(
            "cmbStoreCountry"
        );

    const productCountry =
        registeredStore
            ? registeredStore.country || "JP"
            : getCocartlyBaseCountry();


    if (cmbStoreCountry) {

        cmbStoreCountry.value =
            productCountry;

    }


    /*
     国・地域に合わせて
     税率入力・通貨表示を更新
    */

    if (cmbStoreCountry) {

        cmbStoreCountry.dispatchEvent(
            new Event("change")
        );

    }


    /*
     選択した国・地域の
     購入先一覧を作り直す
    */

    populateStoreCombo();


    /*
     登録されていた購入先を
     選択状態へ戻す
    */

    document.getElementById(
        "cmbStore"
    ).value =
        product.storeId || "";


    document.getElementById(
        "productMessage"
    ).textContent =
        t(
            "product.message.editMode"
        );


    /*
     商品登録画面の先頭へ
    */

    window.scrollTo({

        top: 0,

        behavior: "smooth"

    });

}

/* ==========================================
   商品編集キャンセル
   ========================================== */

function cancelProductEdit() {

    /*
     編集状態を解除
    */

    editingProductId =
        null;

    productPriceCalculation =
        null;


    /*
     価格計算の内容を
     新規登録状態へ戻す
    */

    const txtProductTaxExcluded =
        document.getElementById(
            "txtProductTaxExcluded"
        );

    if (txtProductTaxExcluded) {

        txtProductTaxExcluded.value =
            "";

    }


    const cmbProductPriceType =
        document.getElementById(
            "cmbProductPriceType"
        );

    if (cmbProductPriceType) {

        cmbProductPriceType.value =
            "taxExcluded";

    }


    const cmbProductTaxRate =
        document.getElementById(
            "cmbProductTaxRate"
        );

    if (cmbProductTaxRate) {

        cmbProductTaxRate.value =
            "10";

    }


    const txtProductCustomTaxRate =
        document.getElementById(
            "txtProductCustomTaxRate"
        );

    if (txtProductCustomTaxRate) {

        txtProductCustomTaxRate.value =
            "";

    }


    const cmbProductRoundingMode =
        document.getElementById(
            "cmbProductRoundingMode"
        );

    if (cmbProductRoundingMode) {

        cmbProductRoundingMode.value =
            "round";

    }


    const cmbProductDiscountType =
        document.getElementById(
            "cmbProductDiscountType"
        );

    if (cmbProductDiscountType) {

        cmbProductDiscountType.value =
            "none";

    }


    const txtProductDiscountValue =
        document.getElementById(
            "txtProductDiscountValue"
        );

    if (txtProductDiscountValue) {

        txtProductDiscountValue.value =
            "0";

    }


    const productTaxResult =
        document.getElementById(
            "productTaxResult"
        );

    if (productTaxResult) {

        productTaxResult.textContent =
            "";

    }


    /*
     価格計算パネルを閉じる
    */

    const productTaxCalc =
        document.getElementById(
            "productTaxCalc"
        );

    if (productTaxCalc) {

        productTaxCalc.hidden =
            true;

    }


    /*
     商品登録の入力内容を
     新規登録状態へ戻す
    */

    document.getElementById(
        "txtProductName"
    ).value = "";

    document.getElementById(
        "txtJanCode"
    ).value = "";

    document.getElementById(
        "cmbCategory"
    ).selectedIndex = 0;

    document.getElementById(
        "txtVolume"
    ).value = "";

    document.getElementById(
        "cmbUnit"
    ).selectedIndex = 0;

    document.getElementById(
        "txtPrice"
    ).value = "";


    /*
     普段買い物する国・地域へ戻す
    */

    const cmbStoreCountry =
        document.getElementById(
            "cmbStoreCountry"
        );

    if (cmbStoreCountry) {

        cmbStoreCountry.value =
            getCocartlyBaseCountry();

    }


    /*
     国・地域に合わせて
     税率・通貨表示を戻す
    */

    if (
        typeof updateProductTaxInput ===
        "function"
    ) {

        updateProductTaxInput();

    }


    /*
     日本の購入先一覧へ戻す
    */

    populateStoreCombo();

    const cmbStore =
        document.getElementById(
            "cmbStore"
        );

    if (cmbStore) {

        cmbStore.selectedIndex =
            0;

    }


    /*
     編集キャンセルボタンを
     非表示へ戻す
    */

    const btnCancelProductEdit =
        document.getElementById(
            "btnCancelProductEdit"
        );

    if (btnCancelProductEdit) {

        btnCancelProductEdit.hidden =
            true;

    }


    /*
     メッセージを戻す
    */

    const productMessage =
        document.getElementById(
            "productMessage"
        );

    if (productMessage) {

        productMessage.textContent =
            t(
                "product.message.editCancelled"
            );

    }


    /*
     内容量の説明を閉じる
    */

    const volumeExample =
        document.getElementById(
            "volumeExample"
        );

    if (volumeExample) {

        volumeExample.hidden =
            true;

    }


    /*
     商品登録画面の先頭へ
    */

    window.scrollTo({

        top: 0,

        behavior: "smooth"

    });

}

/* ==========================================
   商品削除
   ========================================== */

function deleteProduct(productId) {

    if (
        !confirm(
            t(
                "product.confirm.delete"
            )
        )
    ) {

        return;

    }

    const product = products.find(

        p => p.id === productId

    );

    if (!product) {

        return;

    }

    product.active = false;

    product.updatedAt = new Date().toISOString();

    saveProducts();

    displayProducts();

    displayDeletedProducts();

}

/* ==========================================
   商品復元
   ========================================== */

function restoreProduct(productId) {

    const product = products.find(

        p => p.id === productId

    );

    if (!product) {

        return;

    }

    product.active = true;

    product.updatedAt = new Date().toISOString();

    saveProducts();

    // 商品一覧更新
    displayProducts();

    // 削除済み一覧更新
    displayDeletedProducts();

}

/* ==========================================
   商品を完全削除
   ========================================== */

function permanentlyDeleteProduct(productId) {

    const product =
        products.find(

            product =>
                product.id === productId

        );

    if (!product) {

        return;

    }


    const confirmed =
        confirm(

            t(
                "product.confirm.permanentDelete"
            ).replace(
                "{name}",
                product.name
            )

        );


    if (!confirmed) {

        return;

    }


    const productIndex =
        products.findIndex(

            product =>
                product.id === productId

        );

    if (productIndex === -1) {

        console.error(
            "完全削除する商品が見つかりません。"
        );

        return;

    }


    /*
     商品を配列から完全削除
    */

    products.splice(
        productIndex,
        1
    );


    /*
     localStorageへ保存
    */

    saveProducts();


    /*
     通常の商品一覧更新
    */

    displayProducts();


    /*
     削除済み商品一覧更新
    */

    displayDeletedProducts();

}

/* ==========================================
   購入先管理
   国別の地域入力例
   ========================================== */
function updateStoreLocationPlaceholders() {

    const countrySelect =
        document.getElementById(
            "cmbStoreMasterCountry"
        );

    const storeNameInput =
        document.getElementById(
            "txtStoreName"
        );

    const regionInput =
        document.getElementById(
            "txtStoreRegion"
        );

    const cityInput =
        document.getElementById(
            "txtStoreCity"
        );

    if (
        !countrySelect ||
        !storeNameInput ||
        !regionInput ||
        !cityInput
    ) {
        return;
    }


    const selectedCountry =
        countrySelect.value ||
        "JP";


    const placeholderCountry =
        [
            "JP",
            "US",
            "CA",
            "AU",
            "KR",
            "CN",
            "TW"
        ].includes(
            selectedCountry
        )
            ? selectedCountry
            : "OTHER";


    storeNameInput.placeholder =
        t(
            "store.placeholder." +
            placeholderCountry +
            ".store"
        );

    regionInput.placeholder =
        t(
            "store.placeholder." +
            placeholderCountry +
            ".region"
        );

    cityInput.placeholder =
        t(
            "store.placeholder." +
            placeholderCountry +
            ".city"
        );
}

/* ==========================================
   購入先登録・更新
   ========================================== */

function saveStore() {

    const txtStoreName =
        document.getElementById("txtStoreName");

    const cmbStoreType =
        document.getElementById("cmbStoreType");

    const txtStoreRegion =
        document.getElementById(
            "txtStoreRegion"
        );

    const txtStoreCity =
        document.getElementById(
            "txtStoreCity"
        );

    const storeMessage =
        document.getElementById("storeMessage");

    const cmbStoreMasterCountry =
        document.getElementById(
            "cmbStoreMasterCountry"
        );

    const selectedCountry =
        cmbStoreMasterCountry
            ? cmbStoreMasterCountry.value
            : "JP";

    if (!txtStoreName || !cmbStoreType) {

        return;

    }

    const storeName =
        txtStoreName.value.trim();

    const storeTypeId =
        cmbStoreType.value;

    const storeRegion =
        txtStoreRegion
            ? txtStoreRegion.value.trim()
            : "";

    const storeCity =
        txtStoreCity
            ? txtStoreCity.value.trim()
            : "";

    if (storeTypeId === "") {

        storeMessage.textContent =
            t(
                "store.validation.typeRequired"
            );

        cmbStoreType.focus();

        return;

    }

    if (storeName === "") {

        storeMessage.textContent =
            t(
                "store.validation.nameRequired"
            );

        txtStoreName.focus();

        return;

    }

    const duplicateStore = stores.find(

        store => {

            const storeCountry =
                store.country || "JP";

            return (

                store.active &&

                store.name.toLowerCase() ===
                storeName.toLowerCase() &&

                storeCountry ===
                selectedCountry &&

                store.id !== editingStoreId

            );

        }

    );

    if (duplicateStore) {

        storeMessage.textContent =
            t(
                "store.validation.duplicate"
            );

        return;

    }

    if (editingStoreId === null) {

        const store =
            createStore(
                storeName,
                storeTypeId,
                selectedCountry,
                storeRegion,
                storeCity
            );

        addStore(store);

        storeMessage.textContent =
            t(
                "store.message.saved"
            );

    } else {

        const store =
            findStore(editingStoreId);

        if (!store) {

            storeMessage.textContent =
                t(
                    "store.message.editNotFound"
                );

            editingStoreId = null;

            return;

        }

        const updatedStore = {

            ...store,

            name: storeName,

            typeId: storeTypeId,

            country: selectedCountry,

            region: storeRegion,

            city: storeCity

        };

        updateStore(updatedStore);

        editingStoreId = null;

        storeMessage.textContent =
            t(
                "store.message.updated"
            );

    }

    txtStoreName.value = "";

    if (txtStoreRegion) {

        txtStoreRegion.value = "";

    }

    if (txtStoreCity) {

        txtStoreCity.value = "";

    }

    cmbStoreType.selectedIndex = 0;

    populateStoreCombo();

    displayStores();

}

/* ==========================================
   店舗編集
   ========================================== */

function editStore(storeId) {

    const store =
        findStore(storeId);

    if (!store) {

        return;
    }


    /*
     Cocartly標準の初期店舗は
     編集不可
    */
    if (
        store.isDefault === true
    ) {

        alert(
            t(
                "store.default.editBlocked"
            )
        );

        return;
    }


    editingStoreId = store.id;

    document.getElementById("txtStoreName").value =
        store.name;

    document.getElementById("cmbStoreType").value =
        store.typeId || "other";

    /*
     購入先の国・地域を復元
    */

    const cmbStoreMasterCountry =
        document.getElementById(
            "cmbStoreMasterCountry"
        );

    if (cmbStoreMasterCountry) {

        cmbStoreMasterCountry.value =
            store.country || "JP";

    }

    const txtStoreRegion =
        document.getElementById(
            "txtStoreRegion"
        );

    const txtStoreCity =
        document.getElementById(
            "txtStoreCity"
        );

    if (txtStoreRegion) {

        txtStoreRegion.value =
            store.region || "";

    }

    if (txtStoreCity) {

        txtStoreCity.value =
            store.city || "";

    }

    document.getElementById("storeMessage").textContent =
        t(
            "store.message.editMode"
        );

    document.getElementById("txtStoreName").focus();

}

/* ==========================================
   初期購入先
   表示言語に合わせた情報を取得
   ========================================== */

function getDefaultStoreDisplayInfo(
    store
) {

    if (
        !store ||
        store.isDefault !== true
    ) {

        return {
            name: store
                ? store.name
                : "",
            region: store
                ? store.region || ""
                : "",
            city: store
                ? store.city || ""
                : ""
        };

    }


    const defaultStoreTranslationKeys = {

        "JP|イオンスタイル品川シーサイド": {
            name:
                "store.default.jp.aeon.name",
            region:
                "store.default.jp.aeon.region",
            city:
                "store.default.jp.aeon.city"
        },

        "JP|ライフ セントラルスクエア恵比寿ガーデンプレイス店": {
            name:
                "store.default.jp.life.name",
            region:
                "store.default.jp.life.region",
            city:
                "store.default.jp.life.city"
        },

        "JP|万代 渋川店": {
            name:
                "store.default.jp.mandai.name",
            region:
                "store.default.jp.mandai.region",
            city:
                "store.default.jp.mandai.city"
        },

        "JP|業務スーパー 新宿大久保店": {
            name:
                "store.default.jp.gyomu.name",
            region:
                "store.default.jp.gyomu.region",
            city:
                "store.default.jp.gyomu.city"
        },

        "JP|コープみらい コープ戸山店": {
            name:
                "store.default.jp.coop.name",
            region:
                "store.default.jp.coop.region",
            city:
                "store.default.jp.coop.city"
        },

        "JP|ロピア 平井島忠ホームズ店": {
            name:
                "store.default.jp.lopia.name",
            region:
                "store.default.jp.lopia.region",
            city:
                "store.default.jp.lopia.city"
        },

        "JP|MEGAドン・キホーテ渋谷本店": {
            name:
                "store.default.jp.donki.name",
            region:
                "store.default.jp.donki.region",
            city:
                "store.default.jp.donki.city"
        },

        "JP|コストコ 川崎倉庫店": {
            name:
                "store.default.jp.costco.name",
            region:
                "store.default.jp.costco.region",
            city:
                "store.default.jp.costco.city"
        }

    };


    const storeCountry =
        store.country || "JP";

    const translation =
        defaultStoreTranslationKeys[
        storeCountry +
        "|" +
        store.name
        ];


    if (!translation) {

        return {
            name:
                store.name,
            region:
                store.region || "",
            city:
                store.city || ""
        };

    }


    return {
        name:
            t(translation.name),
        region:
            t(translation.region),
        city:
            t(translation.city)
    };

}

/* ==========================================
   店舗一覧表示
   ========================================== */

function displayStores() {

    const storeList =
        document.getElementById(
            "storeList"
        );

    if (!storeList) {

        return;

    }


    storeList.innerHTML =
        "";


    /*
     購入先管理で選択している
     国・地域を取得
    */

    const cmbStoreMasterCountry =
        document.getElementById(
            "cmbStoreMasterCountry"
        );


    const selectedCountry =
        cmbStoreMasterCountry
            ? cmbStoreMasterCountry.value
            : "JP";


    /*
     選択中の国・地域にある
     有効な購入先だけ取得
    */

    const countryStores =
        stores.filter(
            store => {

                const storeCountry =
                    store.country ||
                    "JP";

                return (
                    store.active !==
                    false &&
                    storeCountry ===
                    selectedCountry
                );

            }
        );


    /*
     初期店舗
     ユーザー追加店舗
     に分ける
    */

    const defaultStores =
        countryStores.filter(
            store =>
                store.isDefault ===
                true
        );


    const userStores =
        countryStores.filter(
            store =>
                store.isDefault !==
                true
        );


    /*
     ==========================================
     店舗カード作成
     ==========================================
    */

    function createStoreCard(
        store
    ) {

        /*
         地域情報
        */

        const storeDisplayInfo =
            getDefaultStoreDisplayInfo(
                store
            );


        const locationParts =
            [];


        if (storeDisplayInfo.region) {

            locationParts.push(
                storeDisplayInfo.region
            );

        }


        if (storeDisplayInfo.city) {

            locationParts.push(
                storeDisplayInfo.city
            );

        }


        const locationText =
            locationParts.join(
                " "
            );


        /*
         初期店舗かどうか
        */

        const buttonsHtml =
            store.isDefault === true

                ?

                `
                <span class="master-protected">
                    ${t(
                    "store.list.protected"
                )}
                </span>
                `

                :

                `
                <button
                    onclick="editStore('${store.id}')"
                >
                    ${t(
                    "product.list.edit"
                )}
                </button>

                <button
                    onclick="deleteStoreData('${store.id}')"
                >
                    ${t(
                    "product.list.delete"
                )}
                </button>
                `;


        return `
            <div class="master-card">

                <div class="store-master-info">

                    <strong>
                         ${storeDisplayInfo.name}
                    </strong>

                    <small>
                        🏷
                        ${getStoreTypeName(
            store.typeId ||
            "other"
        )}
                    </small>

                    ${locationText

                ?

                `
                            <small>
                                📍 ${locationText}
                            </small>
                            `

                :

                ""
            }

                </div>

                <div class="master-buttons">

                    ${buttonsHtml}

                </div>

            </div>
        `;

    }


    /*
     ==========================================
     ① 初期店舗を上側へ表示
     ==========================================
    */

    if (
        defaultStores.length >
        0
    ) {

        storeList.innerHTML += `
            <div class="store-list-section-title">
                ${t(
            "store.list.default"
        )}
            </div>
        `;


        for (
            const store
            of defaultStores
        ) {

            storeList.innerHTML +=
                createStoreCard(
                    store
                );

        }

    }


    /*
     ==========================================
     ② ユーザー追加店舗
     ==========================================
    */

    if (
        userStores.length >
        0
    ) {

        storeList.innerHTML += `
            <div class="store-list-section-title">
                ${t(
            "store.list.added"
        )}
            </div>
        `;


        /*
         店舗名から
         チェーン名を判定するルール

         上から順番に判定
        */

        const chainRules = [

            /*
             日本
            */

            {
                label: "イオン",
                keywords: [
                    "イオン",
                    "AEON"
                ]
            },

            {
                label: "ライフ",
                keywords: [
                    "ライフ"
                ]
            },

            {
                label: "万代",
                keywords: [
                    "万代"
                ]
            },

            {
                label: "業務スーパー",
                keywords: [
                    "業務スーパー"
                ]
            },

            {
                label: "コープ",
                keywords: [
                    "コープ"
                ]
            },

            {
                label: "ロピア",
                keywords: [
                    "ロピア"
                ]
            },

            {
                label: "ドン・キホーテ",
                keywords: [
                    "ドン・キホーテ",
                    "ドンキ"
                ]
            },

            {
                label: "コストコ",
                keywords: [
                    "コストコ"
                ]
            },


            /*
             海外
            */

            {
                label: "Walmart",
                keywords: [
                    "Walmart"
                ]
            },

            {
                label: "Target",
                keywords: [
                    "Target"
                ]
            },

            {
                label: "Costco",
                keywords: [
                    "Costco"
                ]
            },

            {
                label: "Woolworths",
                keywords: [
                    "Woolworths"
                ]
            },

            {
                label: "Coles",
                keywords: [
                    "Coles"
                ]
            },

            {
                label: "E-Mart",
                keywords: [
                    "E-Mart",
                    "Emart",
                    "E Mart"
                ]
            },

            {
                label: "Lotte Mart",
                keywords: [
                    "Lotte Mart"
                ]
            },

            {
                label: "PX Mart",
                keywords: [
                    "PX Mart"
                ]
            }

        ];


        /*
         チェーンごとの箱を作る
        */

        const groupedStores =
            new Map();


        /*
         その他は最後に表示するため
         最初には作らない
        */


        /*
 店舗名を比較しやすい形へ変換する

 ・半角スペースを無視
 ・全角スペースを無視
 ・英字の大文字小文字を無視
*/

        function normalizeStoreName(
            name
        ) {

            return String(
                name || ""
            )
                .normalize("NFKC")
                .replace(
                    /\s+/g,
                    ""
                )
                .toLowerCase();

        }


        /*
 ==========================================
 2つの店舗名から
 共通している先頭部分を取得する
 ==========================================
*/

        function getCommonStorePrefix(
            nameA,
            nameB
        ) {

            const normalizedA =
                normalizeStoreName(
                    nameA
                );

            const normalizedB =
                normalizeStoreName(
                    nameB
                );


            let commonPrefix =
                "";


            const maxLength =
                Math.min(
                    normalizedA.length,
                    normalizedB.length
                );


            for (
                let i = 0;
                i < maxLength;
                i++
            ) {

                if (
                    normalizedA[i] !==
                    normalizedB[i]
                ) {

                    break;

                }


                commonPrefix +=
                    normalizedA[i];

            }


            return commonPrefix;

        }


        /*
         ==========================================
         正規化した共通名から
         表示用の店名を作る
        
         例
        
         ○○スーパー A店
         ○○スーパー　B店
        
         ↓
        
         ○○スーパー
         ==========================================
        */

        function getStoreGroupDisplayName(
            originalName,
            normalizedPrefix
        ) {

            const normalizedOriginal =
                String(
                    originalName || ""
                )
                    .normalize("NFKC");


            let matchedLength =
                0;

            let displayName =
                "";


            for (
                const char
                of normalizedOriginal
            ) {

                /*
                 スペースは比較対象外だが
                 表示名には残す
                */

                if (
                    /\s/u.test(
                        char
                    )
                ) {

                    if (
                        matchedLength >
                        0
                    ) {

                        displayName +=
                            char;

                    }

                    continue;

                }


                const normalizedChar =
                    char.toLowerCase();


                if (
                    matchedLength >=
                    normalizedPrefix.length
                ) {

                    break;

                }


                if (
                    normalizedChar !==
                    normalizedPrefix[
                    matchedLength
                    ]
                ) {

                    break;

                }


                displayName +=
                    char;


                matchedLength++;

            }


            return displayName.trim();

        }


        /*
         ==========================================
         ユーザー追加店舗を
         自動グループ化
         ==========================================
        */

        for (
            const store
            of userStores
        ) {

            let groupName =
                "";


            const storeNameNormalized =
                normalizeStoreName(
                    store.name
                );


            /*
             ==========================================
             ① 既知チェーンを優先
             ==========================================
            */

            for (
                const rule
                of chainRules
            ) {

                const matched =
                    rule.keywords.some(
                        keyword => {

                            const normalizedKeyword =
                                normalizeStoreName(
                                    keyword
                                );


                            return (
                                normalizedKeyword !==
                                "" &&
                                storeNameNormalized.includes(
                                    normalizedKeyword
                                )
                            );

                        }
                    );


                if (matched) {

                    groupName =
                        rule.label;

                    break;

                }

            }


            /*
             ==========================================
             ② 未知の店舗は
                他の店舗との共通部分から
                チェーン名を自動判定
             ==========================================
            */

            if (
                groupName ===
                ""
            ) {

                let bestPrefix =
                    "";


                for (
                    const otherStore
                    of userStores
                ) {

                    if (
                        otherStore.id ===
                        store.id
                    ) {

                        continue;

                    }


                    const commonPrefix =
                        getCommonStorePrefix(
                            store.name,
                            otherStore.name
                        );


                    /*
                     3文字未満の共通部分は
                     偶然一致の可能性が高いため
                     グループ化しない
                    */

                    if (
                        commonPrefix.length <
                        3
                    ) {

                        continue;

                    }


                    /*
                     最も長く一致した
                     共通部分を採用
                    */

                    if (
                        commonPrefix.length >
                        bestPrefix.length
                    ) {

                        bestPrefix =
                            commonPrefix;

                    }

                }


                if (
                    bestPrefix !==
                    ""
                ) {

                    groupName =
                        getStoreGroupDisplayName(
                            store.name,
                            bestPrefix
                        );

                }

            }


            /*
             ==========================================
             ③ 判定できなければ
                その他
             ==========================================
            */

            if (
                groupName ===
                ""
            ) {

                groupName =
                    "その他";

            }


            /*
             グループを作成
            */

            if (
                !groupedStores.has(
                    groupName
                )
            ) {

                groupedStores.set(
                    groupName,
                    []
                );

            }


            /*
             店舗をグループへ追加
            */

            groupedStores
                .get(
                    groupName
                )
                .push(
                    store
                );

        }


        /*
         各グループ内を
         店舗名順に並べる
        */

        for (
            const groupStores
            of groupedStores.values()
        ) {

            groupStores.sort(
                (a, b) =>
                    a.name.localeCompare(
                        b.name,
                        "ja"
                    )
            );

        }


        /*
         自動作成されたグループも含めて
         すべてのグループを表示する
        */

        const userStoreGroups =
            Array.from(
                groupedStores.entries()
            );


        /*
         「その他」は
         必ず最後に表示する
        */

        userStoreGroups.sort(
            (a, b) => {

                if (
                    a[0] ===
                    "その他"
                ) {

                    return 1;

                }


                if (
                    b[0] ===
                    "その他"
                ) {

                    return -1;

                }


                return a[0].localeCompare(
                    b[0],
                    "ja"
                );

            }
        );


        /*
         グループ見出しと
         店舗を表示
        */

        for (
            const [
                groupName,
                groupStores
            ]
            of userStoreGroups
        ) {

            if (
                !groupStores ||
                groupStores.length ===
                0
            ) {

                continue;

            }


            storeList.innerHTML += `
        <div class="store-chain-title">
            ${groupName}
        </div>
    `;


            for (
                const store
                of groupStores
            ) {

                storeList.innerHTML +=
                    createStoreCard(
                        store
                    );

            }

        }

    }


    /*
     店舗が1件もない場合
    */

    if (
        defaultStores.length ===
        0 &&
        userStores.length ===
        0
    ) {

        storeList.innerHTML = `
            <p class="master-empty">
                ${t(
            "store.list.empty"
        )}
            </p>
        `;

    }

}

/* ==========================================
   店舗削除
   ========================================== */

function deleteStoreData(storeId) {

    const store =
        findStore(
            storeId
        );


    if (!store) {

        return;

    }


    /*
     Cocartly標準の初期店舗は
     削除不可
    */
    if (
        store.isDefault === true
    ) {

        alert(
            t(
                "store.default.deleteBlocked"
            )
        );

        return;

    }


    if (
        !confirm(
            t(
                "store.confirm.delete"
            )
        )
    ) {

        return;

    }


    deleteStore(
        storeId
    );

    populateStoreCombo();

    displayStores();

}

/* ==========================================
   簡易追加欄の表示切替
   ========================================== */

function toggleQuickAdd(targetId, inputId) {

    const target =
        document.getElementById(targetId);

    const input =
        document.getElementById(inputId);

    if (!target) {

        return;

    }

    target.hidden = !target.hidden;

    if (!target.hidden && input) {

        input.focus();

    }

}

/* ==========================================
   メーカー簡易追加
   ========================================== */

function quickAddMaker() {

    const input =
        document.getElementById("txtQuickMaker");

    const message =
        document.getElementById("productMessage");

    const name =
        input ? input.value.trim() : "";

    if (name === "") {

        if (message) {

            message.textContent =
                t(
                    "product.quick.makerRequired"
                );

        }

        return;

    }

    const duplicate = makers.find(

        maker =>

            maker.active &&

            maker.name.toLowerCase() ===
            name.toLowerCase()

    );

    /*
     登録済みメーカーの場合
    */

    if (duplicate) {

        displayMakerSelect();

        document.getElementById("cmbMaker").value =
            duplicate.id;

        input.value = "";

        document.getElementById(
            "makerQuickAdd"
        ).hidden = true;

        if (message) {

            message.textContent =
                t(
                    "product.quick.makerExisting"
                );

        }

        return;

    }

    /*
     addMakerへ文字列だけ渡す
    */

    addMaker(name);

    displayMakerSelect();

    /*
     追加されたメーカーを探して自動選択
    */

    const addedMaker = makers.find(

        maker =>

            maker.active &&

            maker.name.toLowerCase() ===
            name.toLowerCase()

    );

    if (addedMaker) {

        document.getElementById("cmbMaker").value =
            addedMaker.id;

    }

    input.value = "";

    document.getElementById(
        "makerQuickAdd"
    ).hidden = true;

    if (message) {

        message.textContent =
            t(
                "product.quick.makerAdded"
            );

    }

}

/* ==========================================
   カテゴリー簡易追加
   ========================================== */

function quickAddCategory() {

    const input =
        document.getElementById("txtQuickCategory");

    const message =
        document.getElementById("productMessage");

    const name =
        input ? input.value.trim() : "";

    if (name === "") {

        message.textContent =
            t(
                "product.quick.categoryRequired"
            );

        return;

    }

    const duplicate = categories.find(

        category =>

            category.active &&

            category.name.toLowerCase() ===
            name.toLowerCase()

    );

    if (duplicate) {

        displayCategorySelect();

        document.getElementById("cmbCategory").value =
            duplicate.id;

        input.value = "";

        document.getElementById(
            "categoryQuickAdd"
        ).hidden = true;

        message.textContent =
            t(
                "product.quick.categoryExisting"
            );

        return;

    }

    addCategory(name);

    displayCategorySelect();

    const addedCategory = categories.find(

        category =>
            category.active &&
            category.name.toLowerCase() ===
            name.toLowerCase()

    );

    if (addedCategory) {

        document.getElementById("cmbCategory").value =
            addedCategory.id;

    }

    input.value = "";

    document.getElementById(
        "categoryQuickAdd"
    ).hidden = true;

    message.textContent =
        t(
            "product.quick.categoryAdded"
        );

}

/* ==========================================
   購入先簡易追加
   ========================================== */

function quickAddStore() {

    const input =
        document.getElementById("txtQuickStore");

    const typeSelect =
        document.getElementById("cmbQuickStoreType");

    const message =
        document.getElementById("productMessage");

    const name =
        input ? input.value.trim() : "";

    const typeId =
        typeSelect ? typeSelect.value : "";

    if (typeId === "") {

        message.textContent =
            t(
                "product.quick.storeTypeRequired"
            );

        if (typeSelect) {

            typeSelect.focus();

        }

        return;

    }

    if (name === "") {

        message.textContent =
            t(
                "product.quick.storeNameRequired"
            );

        if (input) {

            input.focus();

        }

        return;

    }

    const duplicate = stores.find(

        store =>

            store.active &&

            store.name.toLowerCase() ===
            name.toLowerCase()

    );

    /*
     同じ購入先が登録済みの場合
    */

    if (duplicate) {

        populateStoreCombo();

        document.getElementById("cmbStore").value =
            duplicate.id;

        input.value = "";

        typeSelect.selectedIndex = 0;

        document.getElementById(
            "storeQuickAdd"
        ).hidden = true;

        message.textContent =
            t(
                "product.quick.storeExisting"
            );

        return;

    }

    /*
     新しい購入先を作成
    */

    const store =
        createStore(
            name,
            typeId
        );

    addStore(store);

    /*
     商品登録画面の選択枠を更新
    */

    populateStoreCombo();

    document.getElementById("cmbStore").value =
        store.id;

    /*
     入力欄を初期化
    */

    input.value = "";

    typeSelect.selectedIndex = 0;

    document.getElementById(
        "storeQuickAdd"
    ).hidden = true;

    message.textContent =
        t(
            "product.quick.storeAdded"
        );

}

/* ==========================================
   標準商品ジャンル判定
   ========================================== */

function isDefaultCategory(categoryName) {

    const defaultCategoryNames = [

        "食品",
        "飲料",
        "お菓子",
        "お酒",
        "日用品",
        "洗剤・衛生用品",
        "医薬品",
        "化粧品・美容",
        "衣料品",
        "ベビー用品",
        "ペット用品",
        "文具・雑貨",
        "家電・電池",
        "その他"

    ];

    return defaultCategoryNames.includes(
        categoryName
    );

}

/* ==========================================
   初期購入先判定
   ========================================== */

function isDefaultStore(
    storeName,
    country = "JP"
) {

    const defaultStores = [

        /* 日本 */
        { name: "イオン", country: "JP" },
        { name: "ライフ", country: "JP" },
        { name: "万代", country: "JP" },
        { name: "業務スーパー", country: "JP" },
        { name: "コープ", country: "JP" },
        { name: "ロピア", country: "JP" },
        { name: "ドン・キホーテ", country: "JP" },
        { name: "コストコ", country: "JP" },

        /* アメリカ */
        { name: "Walmart", country: "US" },
        { name: "Target", country: "US" },
        { name: "Costco", country: "US" },

        /* カナダ */
        { name: "Walmart", country: "CA" },
        { name: "Costco", country: "CA" },

        /* オーストラリア */
        { name: "Woolworths", country: "AU" },
        { name: "Coles", country: "AU" },
        { name: "Costco", country: "AU" },

        /* 韓国 */
        { name: "E-Mart", country: "KR" },
        { name: "Lotte Mart", country: "KR" },
        { name: "Costco", country: "KR" },

        /* 中国 */
        { name: "Walmart", country: "CN" },
        { name: "Costco", country: "CN" },

        /* 台湾 */
        { name: "PX Mart", country: "TW" },
        { name: "Costco", country: "TW" }

    ];

    return defaultStores.some(
        (defaultStore) => {

            return (
                defaultStore.name ===
                storeName &&
                defaultStore.country ===
                country
            );

        }
    );

}
